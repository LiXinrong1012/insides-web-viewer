	...
	, m_ribbonStyle(qobject_cast<Qtitan::RibbonStyle*>(qApp->style()))
{
	...
	QAction* actionAbout = ribbonBar()->addAction(QIcon(":/resources/qtnres/about.png"), tr("About"), Qt::ToolButtonIconOnly);
	actionAbout->setToolTip(tr("Display program<br />information, version number and copyright"));
	connect(actionAbout, SIGNAL(triggered()), this, SLOT(about()));

	m_actionRibbonMinimize = ribbonBar()->addAction(QIcon(":/resources/qtnres/ribbonMinimize.png"), tr("Minimize the Ribbon"), Qt::ToolButtonIconOnly);
...
void RibbonWindow::about()
{
	...
	/*
	char tempstr[4096];
	int major, minor,tick;

	major = 1;
	minor = 0;
	tick=0;

	std::string appName="INSIDES Viewer";

	Base::ApplicationInfo info;
	std::string str=info.getBuildTime().toStdString();
	sprintf( tempstr, 
		"<font color=red>%s</font> %d.%d.%d<br>"
		"Build on %s<br>"
		"<br>"
		"Built against version %s of Qt, from Nokia Corporation<br>"
		"HOOPS 3D Framework v19.00, from TechSoft 3D<br>"
		"QCustomPlot v1.2<br>"
		"Xerces C++ v1.39<br>"
		"OSG v2.8.5<br>"
		"<br>For information about the <font color=red>%s</font> Application<br>"
		"Send email to Jerry He (hejw06@gmail.com)",
		appName.c_str(),
		major,
		minor,
		tick,
	...
	QMessageBox::information( this, QString("about ")+appName.c_str(), tempstr );
	*/
}
...
void RibbonWindow::minimizationChanged(bool minimized)
{
	...
	m_actionRibbonMinimize->setIcon(minimized ? QIcon(":/resources/qtnres/ribbonMaximize.png") :  
		QIcon(":/resources/qtnres/ribbonMinimize.png"));
}
