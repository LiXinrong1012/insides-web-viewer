...
// The ctor.
LexerTDY::LexerTDY(QObject *parent, bool caseInsensitiveKeywords)
	: QsciLexer(parent),
...
const char *LexerTDY::language() const
{
	return "C++";
}
...
const char *LexerTDY::lexer() const
{
...
// words.
QStringList LexerTDY::autoCompletionWordSeparators() const
{
	QStringList wl;
...
const char *LexerTDY::blockStartKeyword(int *style) const
{
	if (style)
		*style = Keyword;
...
const char *LexerTDY::blockStart(int *style) const
{
	if (style)
		*style = Operator;
...
const char *LexerTDY::blockEnd(int *style) const
{
	if (style)
		*style = Operator;
...
const char *LexerTDY::wordCharacters() const
{
...
QColor LexerTDY::defaultColor(int style) const
{
	switch (style)
	{
	case Default:
...
bool LexerTDY::defaultEolFill(int style) const
{
	switch (style)
	{
	case UnclosedString:
...
QFont LexerTDY::defaultFont(int style) const
{
	QFont f;
...
const char *LexerTDY::keywords(int set) const
{
	if (set == 1)
		//return
		...
		return
		"Rigid6 rigid6 RIGID6 Marker Assembly FEMPart Constraint FNode6 MNFPart";
...
QString LexerTDY::description(int style) const
{
	switch (style)
	{
	case Default:
...
QColor LexerTDY::defaultPaper(int style) const
{
	switch (style)
	{
	case UnclosedString:
...
void LexerTDY::refreshProperties()
{
	setAtElseProp();
	setCommentProp();
	setCompactProp();
...
bool LexerTDY::readProperties(QSettings &qs,const QString &prefix)
{
	int rc = true;
...
bool LexerTDY::writeProperties(QSettings &qs,const QString &prefix) const
{
	int rc = true;
...
void LexerTDY::setFoldAtElse(bool fold)
{
	fold_atelse = fold;
...
void LexerTDY::setFoldComments(bool fold)
{
	fold_comments = fold;
...
void LexerTDY::setFoldCompact(bool fold)
{
	fold_compact = fold;
...
void LexerTDY::setFoldPreprocessor(bool fold)
{
	fold_preproc = fold;
...
void LexerTDY::setStylePreprocessor(bool style)
{
	style_preproc = style;
...
void LexerTDY::setStylePreprocProp()
{
...
void LexerTDY::setDollarsAllowed(bool allowed)
{
	dollars = allowed;
...
void LexerTDY::setHighlightTripleQuotedStrings(bool enabled)
{
...
void LexerTDY::setHighlightTripleProp()
{
...
void LexerTDY::setHighlightHashQuotedStrings(bool enabled)
{
...
void LexerTDY::setHighlightHashProp()
{
	emit propertyChanged("lexer.cpp.hashquoted.strings",
		(highlight_hash ? "1" : "0"));
}

bool LexerTDY::caseSensitive() const
{
	return false;
}
