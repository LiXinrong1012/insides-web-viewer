...
{
	return QVariant();//_HQWidget->getTreeItemData(baseItem,index);
}

bool Render3DTreeItemProperties::setData( BaseItem* baseItem,int index,const QVariant& data )
{
	return true;//_HQWidget->setTreeItemData(baseItem,index,data);
}
...
RenderView::RenderView(void)
{
	...
	RenderWidget* w = new RenderWidget(main,NULL);

	_widget=main;
	_actualWidget=w;
	_treeProperties._OSGWidget=w;
