// SoccerHQWidget.cpp - Implementation of the HOOPS/Qt class SoccerHQWidget
// 
...
SoccerHQWidget::SoccerHQWidget(QWidget* parent, const char* name , const char * filename) 
    : HQWidget( parent, name ) 
{ 


	// Create and initialize HOOPS/MVO Model and View objects 
	m_pHBaseModel = new HSoccerModel(); 
	m_pHBaseModel->Init(); 
...
{ 
	...
	m_pHView->RenderWireframe();  // set the render mode to wireframe 
	...
	HC_Set_Color_By_Index("text", 4);
	HC_Set_Color_By_Index("edges, lines", 4); 
	HC_Set_Color_By_Index("markers", 3); 
	...
	HC_Set_Selectability("everything = off, faces = on, lines=on, text=on"); 
...
void SoccerHQWidget::Init() 
{ 
	// setup our HOOPS/MVO Base View, Selection object and current operator 
	// This must be called after SoccerHQWidget's constructor has executed 

	// create and initialize MVO HBaseView object 
	m_pHView = new HSoccerView(m_pHBaseModel, NULL, NULL, NULL,  
...
void SoccerHQWidget::load(const char * filename)
{
	m_pHView->Flush(true);
...
void SoccerHQWidget::OnSaveFileAs()
{
...
// window zoomer
void SoccerHQWidget::OnZoomToWindow() 
{
	if (m_pHView->GetCurrentOperator())
	    delete m_pHView->GetCurrentOperator();
...
void SoccerHQWidget::OnZoomToExtents() 
{
    m_pHView->ZoomToExtents();
}
  
// panner
void SoccerHQWidget::OnPan() 
{
...
// create sphere
void SoccerHQWidget::OnCreateSphere() 
{
	// turn off snapping for this operator
...
// create cone
void SoccerHQWidget::OnCreateCone() 
{
	// turn off snapping for this operator
	((HSoccerView *)m_pHView)->SetSnap(false);
...
// create cylinder
void SoccerHQWidget::OnCreateCylinder() 
{
	// turn off snapping for this operator
	((HSoccerView *)m_pHView)->SetSnap(false);
...
// select by window
void SoccerHQWidget::OnSelectByWindow()
{
    if (m_pHView->GetCurrentOperator())
	    delete m_pHView->GetCurrentOperator();
...
void SoccerHQWidget::OnDeleteLayers() 
{
	((HSoccerView *)m_pHView)->DeleteLayers();
}
 
 
void SoccerHQWidget::OnCreateRectangle() 
{
    if (m_pHView->GetCurrentOperator())
	    delete m_pHView->GetCurrentOperator();
	
    m_pHView->SetCurrentOperator(new HOpCreateRectangle(m_pHView));	

	// turn on snapping for this operator
	((HSoccerView *)m_pHView)->SetSnap(true);
}

void SoccerHQWidget::OnRunMyCode() 
{
