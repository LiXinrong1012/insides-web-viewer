...
#include "images/deletelayer.xpm" 
#include "images/rectanglemkp.xpm" 

// HOOPS/Qt includes 
#include "HQApplication.h" 
#include "SoccerHQWidget.h" 
#include "HQDeleter.h" 
...
QPixmap * HQApplication::deletelayerIcon = 0;
...
HQApplication::HQApplication(QApplication * p, const char * filename) : QMainWindow()
{
	...
	this->setWindowIconText("Soccer");
	this->setWindowTitle(QString("qt_soccer"));
	this->load(filename);
}
...
{ 
	...
	qlayout->setMargin(0);
	SoccerHQWidget * my_widget = new SoccerHQWidget( qframe, "SoccerHQWidget", (const char *)0);
...
void HQApplication::load()
{
	...
	if (qfd->exec() == QDialog::Accepted){
...
#ifdef USE_MDI
		{
			...
			QHBoxLayout * qlayout = new QHBoxLayout(qframe);
			qlayout->setMargin(0);
			SoccerHQWidget * my_widget = new SoccerHQWidget( qframe, "SoccerHQWidget", (const char *)fname);
			qlayout->addWidget(my_widget);
...
{
	QMessageBox::information( this, "Soccer",
		"Soccer Version 1.0");
...
void HQApplication::load(const char * filename)
{
	...
	QHBoxLayout * qlayout = new QHBoxLayout(qframe);
	qlayout->setMargin(0);
	SoccerHQWidget * my_widget = new SoccerHQWidget( qframe, "SoccerHQWidget", filename);
	qlayout->addWidget(my_widget);
	my_widget->setMinimumSize( 50, 50 ); 
	qframe->show();
	vhash_insert_item(wh, (void*)qframe,(void*)my_widget);
	...
	SoccerHQWidget * my_widget = new SoccerHQWidget( this, "SoccerHQWidget", filename);
	my_widget->setMinimumSize( 50, 50 ); 
...
selectAct = new QAction(*selectIcon, tr("&Select"), this) ;
	connect(sphereAct, SIGNAL(triggered()), this, SLOT(MDIOnSelect())) ;
...
#else
selectAct = new QAction(*selectIcon, tr("&Select"), my_widget) ;
	connect(selectAct, SIGNAL(triggered()), my_widget, SLOT(OnSelect())) ;
	tools->addAction(selectAct) ;
	...
	deleteLayerAct = new QAction(*deletelayerIcon, tr("&Delete Layers"), my_widget) ;
	connect(deleteLayerAct, SIGNAL(triggered()), my_widget, SLOT(OnDeleteLayers())) ;
	tools->addAction(deleteLayerAct) ;
	
	tools->addSeparator ();

	rectMarkupAct = new QAction(*rectanglemkpIcon, tr("&Create Rectangle"), my_widget) ;
	connect(rectMarkupAct, SIGNAL(triggered()), my_widget, SLOT(OnCreateRectangle())) ;
	tools->addAction(rectMarkupAct) ;
	...
	select->addAction(selectAct) ;
	select->addAction(selectWindowAct) ;

	QMenu *deleteLayer = this->menuBar()->addMenu(tr("&Delete Layers")) ;
	deleteLayer->addAction(deleteLayerAct) ;
...
void HQApplication::CreatePixmaps()
{
	...
	deletelayerIcon = new QPixmap( (const char**)deletelayer_xpm );
	deletelayerIcon->setMask( deletelayerIcon->createHeuristicMask() );
...
void HQApplication::MDIOnSelect()
{
#if USE_MDI
	QFrame* f=(QFrame*)qws->activeWindow();
	if(f) {
