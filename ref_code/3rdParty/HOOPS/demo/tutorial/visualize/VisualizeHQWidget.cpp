// VisualizeHQWidget.cpp - Implementation of the HOOPS/Qt class VisualizeHQWidget
// 
...
VisualizeHQWidget::VisualizeHQWidget(QWidget* parent, const char* name , const char * filename) 
    : HQWidget( parent, name ) 
{ 


	// Create and initialize HOOPS/MVO Model and View objects 
	m_pHBaseModel = new HVisualizeModel(); 
	m_pHBaseModel->Init(); 

	// Initialize View object to null ; gets created in VisualizeHQWidget::Init 
	m_pHView = 0; 
...
VisualizeHQWidget::~VisualizeHQWidget() 
{ 
 // Destructor  
...
void VisualizeHQWidget::SetupView() 
{ 
...
void VisualizeHQWidget::Init() 
{ 
	// setup our HOOPS/MVO Base View, Selection object and current operator 
	// This must be called after VisualizeHQWidget's constructor has executed 

	// create and initialize MVO HBaseView object 
	m_pHView = new HVisualizeView(m_pHBaseModel, NULL, NULL, NULL,  
...
void VisualizeHQWidget::load(const char * filename)
{
	m_pHView->Flush(true);
...
// print
void VisualizeHQWidget::OnPrint()
{ 
...
void VisualizeHQWidget::OnSaveFileAs()
{
...
void VisualizeHQWidget::OnOrbit()  
{ 
...
// window zoomer
void VisualizeHQWidget::OnZoomToWindow() 
{
	if (m_pHView->GetCurrentOperator())
	    delete m_pHView->GetCurrentOperator();
...
void VisualizeHQWidget::OnZoomToExtents() 
{
    m_pHView->ZoomToExtents();
}
  
// panner
void VisualizeHQWidget::OnPan() 
{
    if (m_pHView->GetCurrentOperator())
	    delete m_pHView->GetCurrentOperator();
...
// create sphere
void VisualizeHQWidget::OnCreateSphere() 
{
    if (m_pHView->GetCurrentOperator())
	    delete m_pHView->GetCurrentOperator();
...
// create cone
void VisualizeHQWidget::OnCreateCone() 
{
    if (m_pHView->GetCurrentOperator())
		delete m_pHView->GetCurrentOperator();
...
// create cylinder
void VisualizeHQWidget::OnCreateCylinder() 
{
    if (m_pHView->GetCurrentOperator())
	    delete m_pHView->GetCurrentOperator();
...
void VisualizeHQWidget::OnRunMyCode() 
{
	// this is a function in which developers can
	// write their own simple HOOPS functions
	QMessageBox::information( this, "Visualize", "I love HOOPS!!!!");		
}
  
// create cylinder
void VisualizeHQWidget::OnBuildLODs() 
{
	((HVisualizeView *)m_pHView)->OnBuildLODS();
}

void VisualizeHQWidget::OnLODOriginal() 
{
	((HVisualizeView *)m_pHView)->OnLODOriginal();
}


void VisualizeHQWidget::OnLOD1() 
{
	((HVisualizeView *)m_pHView)->OnLOD1();
}

void VisualizeHQWidget::OnLOD2() 
{
...
void VisualizeHQWidget::OnEnableFramerate() 
{
    ((HVisualizeView *)m_pHView)->OnEnableFramerate();

}

void VisualizeHQWidget::OnDisableFramerate() 
{
...
void VisualizeHQWidget::OnWalkOperator() 
{
    if (m_pHView->GetCurrentOperator())
	    delete m_pHView->GetCurrentOperator();


    m_pHView->SetCurrentOperator(new HOpCameraWalk(m_pHView));		
}
