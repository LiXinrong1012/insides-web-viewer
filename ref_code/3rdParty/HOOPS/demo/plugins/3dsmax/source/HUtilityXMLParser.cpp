...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/3dsmax/source/HUtilityXMLParser.cpp,v 1.3 2007-08-30 20:00:38 stage Exp $
//
...
{
	m_pTextParser->InsertBuffer((char *)buffer, strlen(buffer));
}
...
int HUtilityXMLParser::SkipSpecialTags()
{
...
{	
	...
	void *stack_data = (preturn->m_pTagCallback)(xmltag, openclose, preturn->m_pExtraData);
...
Start_Parsing:
	if (m_pTextParser->ParseBuffer(result, dlum, find_tagname_td, 1))
	{
		...
		if (SkipSpecialTags() == STATUS_SPECIAL_TAG_SKIPPED)
 			goto Start_Parsing;
		else
		{
...
void HUtilityXMLGenerator::WriteToBufferInternal(const char *text, int len)
{
	if (m_BufferSize <= m_BufferPos + len + 1)
	{
		...
		delete [] m_pTextBuffer;
		int newsize = m_BufferSize + strlen(text) + 200;
...
{
	WriteToBufferInternal(text, strlen(text));
}
