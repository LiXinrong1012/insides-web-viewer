	...
	if (my_dlg.DoModal())
	{
		...
		if (!_tcseq(pathname, _T("")))
		{
			if( _tcseq(extension, _T("hsf")) )
			{
				...
				result = pDoc->m_pHoopsModel->Write(pathname, m_pHView, width, height);
				if(result != InputOK){
					char result_str[32];
					...
					swprintf(message, MVO_BUFFER_SIZE-1, L"Saving %ls failed with error %ls", pathname, result_wstr);
