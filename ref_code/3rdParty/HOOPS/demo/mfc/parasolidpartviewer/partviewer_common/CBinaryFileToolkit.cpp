...
#ifdef PARASOLID
long CModellerInfo::Compute_Entity_Index(HC_KEY key, KERN_CLASS_TYPEDEF paraClass)
{
	PK_ENTITY_t entity;
	...
	if (!ENTITY_ask_class(entity, entityClass))
 		return 0;
	switch (entityClass)
	{
	case PK_CLASS_edge:
...
#ifdef GRANITE
long CModellerInfo::Compute_Entity_Index(HC_KEY key, KERN_CLASS_TYPEDEF eclass)
{
	bool error;
	long entity;
	KERN_CLASS_TYPEDEF entityClass;
	long body;
	...
	if (HC_Show_Existence_By_Key(key, "self"))
	{
		...
		if (!found)
		{
			if (eclass == KERN_BODY_TYPE ) // || eclass == PK_CLASS_instance)
			{
				HC_KEY ancestorSegment;
				// be more forgiving if they are interested in a body pointer; 
				// in case this entity has been rendered in merge_faces mode we need
	...
	if( entityClass == KERN_EDGE_TYPE )
	{
		if( eclass == KERN_BODY_TYPE )
		{
			...
			return entity;
		else if( eclass == KERN_ENTITY_TYPE )
			return entity; 	  
		else
			return 0;
	}
	else if( entityClass == KERN_FACE_TYPE )
	{
		if( eclass == KERN_BODY_TYPE )
		{
			error = FACE_ask_body(entity, &body);
	...
	else if( entityClass == KERN_BODY_TYPE )
	{
		if( eclass == KERN_BODY_TYPE )
			return entity;
		else
...
#endif //ACIS
#ifdef PARASOLID
TK_PSUser_Data::TK_PSUser_Data (CModellerInfo **mi, PK_PARTITION_t part, bool psf) : TK_User_Data(), m_partition(part)
#endif //PARASOLID
#ifdef GRANITE
TK_PSUser_Data::TK_PSUser_Data (CModellerInfo **mi, KComponentList_ptr componentList, bool psf) : TK_User_Data(), m_componentList(componentList)
#endif //GRANITE
{
...
TK_Status TK_PSComment::Execute (BStreamFileToolkit & tk) 
{
	...
	{
#ifdef PARASOLID
		tk.SetOpcodeHandler( TKE_Start_User_Data, new TK_PSUser_Data(&m_pHModel->m_mi, m_pHModel->GetPartition()));
#endif // PARASOLID		
#ifdef ACIS
		tk.SetOpcodeHandler( TKE_Start_User_Data,new TK_PSUser_Data(m_pHModel, &m_pHModel->m_mi, m_pHModel->GetEntityList()));
#endif // ACIS
#ifdef GRANITE
		tk.SetOpcodeHandler (TKE_Start_User_Data,new TK_PSUser_Data(&m_pHModel->m_mi, m_pHModel->GetComponentList()));
#endif // GRANITE
	}
...
TK_Status   TK_PSUser_Data::ExecutePS(BStreamFileToolkit & tk)
{
	...
	result = PK_PART_receive_u((unsigned short const*)H_UTF16(m_psf_tempfilename).encodedText(), &options, &numbodies, &bodiesPS);
	// In the case where the file contains assemblies, the bodiesPS array is not what the code
	//   below is expecting, hence just read in the file and then ask the partition for the bodies
	//   since this *is* what the code below expects.
	PK_MEMORY_free(bodiesPS);
	result = PK_PARTITION_ask_bodies(m_partition, &numbodies, &bodiesPS);

	if( result == PK_ERROR_schema_access_error )
	{
		TCHAR str[MVO_BUFFER_SIZE];
		CSolidHoopsApp* ptheApp = (CSolidHoopsApp*) AfxGetApp();
		_stprintf(str, _T("Could not find schema files at specified path:  %s"), CAppSettings::SchemaPath);
		AfxMessageBox( str,	MB_ICONINFORMATION | MB_OK );	
		return TK_Error;
	}
	else if( result != PK_ERROR_no_errors )
	{
		AfxMessageBox( _T("Failed to read embedded Parasolid file"),	MB_ICONINFORMATION | MB_OK );	
		return TK_Error;
	}
	...
	{
		int partnum;
		m_mi->ENTITY_ask_identifier(bodies[i],&partnum);
		if (partnum == numbodies)
			partnum = i;
		HC_KEY key;
		m_mi->Compute_Geometry_Keys(bodies[i], 1, &key);
		HP_Associate_Key_To_Entity(bodiesPS[partnum],key); 
		long *faces;
		int numfaces;
		...
		for (int j=0;j<numfaces;j++)
		{
			...
			PK_PART_find_entity_by_ident(bodiesPS[partnum],PK_CLASS_face,ident,&entity);
			HC_KEY keys[1000];
			int numkeys = m_mi->Compute_Geometry_Keys(faces[j], 1000, keys);
			for (int knum=0;knum<numkeys;knum++)
				HP_Associate_Key_To_Entity(entity,keys[knum]);
			...
			for (int k=0;k<numedges;k++)
			{
				...
				PK_ERROR_t err = PK_PART_find_entity_by_ident(bodiesPS[partnum],PK_CLASS_edge,ident,&entity);
				HC_KEY keys[1000];
...
//======================================================================
//	Author :Rajesh B			Created : 7/31/2002 2:16:57 PM
//======================================================================
//	Function :		TK_PSUser_Data::ExecutePS
//	Description :	Assumes that at this point there is a ProE .prt file
//					dumped, with it's size info. Extracts that prt data 
//					creates a temp ProE file and reads the model.
//	Return :		TK_Status	-	
//	Parameters :
//			BStreamFileToolkit & tk	-	
//	Note :	THE PROE FILE IS OF .PRT TYPE AND HAS ONLY ONE MODEL. NO 
...
TK_Status   TK_PSUser_Data::ExecutePS(BStreamFileToolkit & tk)
{
	...
	// we should already have a temporary file written out in the Read function
	// read this file using ProE api and populate component list
	KComponent_ptr component;
	{
		// NOTE: Limiting the scope of saveFile, this will prevent the temp_file
		// from getting locked and thus allow its deletion before we return - Rajesh B
		KSession_ptr session = KGetSession();						assert(session);
		KCStream_ptr stream = session->OpenFileStream(H_ASCII_TEXT(m_psf_tempfilename));	assert(stream);
		if( !stream)
		{
			AfxMessageBox(_T("Failed to read ProE data. The data may be corrupt"));
			return TK_Error;
		}

		component = session->LoadProEModel(stream);	assert(component);
		if( !component)
		{
			AfxMessageBox(_T("Failed to read ProE data. The data may be corrupt"));
			return TK_Error;
	...
	m_componentList->append(component);
	KModel_ptr model = component->GetModel();					assert(model);
	KBody_ptr body = model->GetSolidBody();						assert(body);
	KBodyList_ptr bodiesPS = KBodyList::create();
	bodiesPS->append(body);
	...
	for (int i=0;i<numbodies;i++)
	{
		int partnum;
		m_mi->ENTITY_ask_identifier(bodies[i],&partnum);
		partnum=numbodies-1-i;
		int partnum2=numbodies-1-i;
		HC_KEY key;
		m_mi->Compute_Geometry_Keys(bodies[i], 1, &key);
		HG_Associate_Key_To_Entity(KObj::cast(bodiesPS->get(partnum)),key); 
		long *faces;
		int numfaces;
		m_mi->BODY_ask_faces(bodies[i],&numfaces,&faces);
		for (int j=0;j<numfaces;j++)
		{
			m_mi->ENTITY_ask_identifier(faces[j],&ident);
			KFace_ptr face = model->GetFace(ident);
			HC_KEY keys[1000];
			int numkeys = m_mi->Compute_Geometry_Keys(faces[j], 1000, keys);
			for (int knum=0;knum<numkeys;knum++)
				HG_Associate_Key_To_Entity(KObj::cast(face),keys[knum]);
			...
			{
				m_mi->ENTITY_ask_identifier(edges[k],&ident);
				KEdge_ptr edge = model->GetEdge(ident);
				HC_KEY keys[1000];
				int numkeys = m_mi->Compute_Geometry_Keys(edges[k], 1000, keys);
				for (int knum=0;knum<numkeys;knum++)
					HG_Associate_Key_To_Entity(KObj::cast(edge),keys[knum]);
			}
			delete edges;
		}
		delete faces;
...
{ 
	...
	err=PK_PARTITION_ask_bodies(part,&numbodies,&bodies);
	int index = -1;
	...
	{		
 		HP_Compute_Geometry_Keys(bodies[i], 1, &key, "bodies");		
		tk_stat = tk.KeyToIndex(key,index);
		assert( tk_stat == TK_Normal );
		if (m_mi->m_bPSF)
		{
 			putdata(&i,sizeof(int));					//store:part number if PSF file
		}
 		putdata(&index,sizeof(long));					//store:index to HOOPS entity for body
 	 	PK_BODY_ask_faces(bodies[i],&numfaces,&faces);
 		putdata(&numfaces,sizeof(long));				//store:number of faces in body
		counter++;
		for (int j=0;j<numfaces;j++)
		{

			if (HP_Compute_Geometry_Keys(faces[j], 1, &key, "faces"))
			{
				tk_stat = tk.KeyToIndex(key,index);
				assert( tk_stat == TK_Normal );
			}
			...
			{
				int ident;
				PK_ENTITY_ask_identifier(faces[j],&ident);
			...
			putdata(&index,sizeof(long));					//store:index to HOOPS entity for face			
  			PK_FACE_ask_edges(faces[j],&num_edges,&edges);
			...
			for (int k=0;k<num_edges;k++)
			{
				...
				int *bid;
				int numints;
				PK_ERROR_t err = PK_ENTITY_ask_attribs(edges[k], att, &n_atts, &at);
				if (n_atts)
				{
					err = PK_ATTRIB_ask_ints(at[0], 0, &numints, &bid);
					assert(numints);
					char dummy = -1;
					putdata(&dummy,sizeof(char));
					putdata(&bid[0],sizeof(long));
					PK_MEMORY_free(bid);
					PK_MEMORY_free(at);
				}
				else
				{
					...
					HC_KEY keys[128];
					char num_keys = (char)HP_Compute_Geometry_Keys(edges[k], 128, keys, "edges");					
					putdata(&num_keys,sizeof(char));
					if (m_mi->m_bPSF)
					{
						int ident;
						PK_ENTITY_ask_identifier(edges[k],&ident);
						putdata(&ident,sizeof(int));					//store:identifier number if PSF file
					}
					...
					PK_ATTRIB_t attr;
					PK_ATTRIB_create_empty(edges[k],att,&attr);
					PK_ERROR_t err=PK_ATTRIB_set_ints(attr, 0, 1,(const int *)&counter);
					counter++;
				}
	...
	{		
		...
		for (int j=0;j<numfaces;j++)
		{
			PK_FACE_ask_edges(faces[j],&num_edges,&edges);
			for (int k=0;k<num_edges;k++)
				PK_ENTITY_delete_attribs(edges[k], att, &numdel);
			PK_MEMORY_free(edges);  
		}
		PK_MEMORY_free(faces);
...
TK_Status   TK_PSUser_Data::InterpretPS(BStreamFileToolkit & tk, HC_KEY dummy, int lod)
{
	...
	result = PK_PART_transmit_u(n_parts, parts, (unsigned short const*)H_UTF16(m_psf_tempfilename).encodedText(), &options);
	if( result == PK_ERROR_schema_access_error )
	{
		TCHAR str[MVO_BUFFER_SIZE];
		CSolidHoopsApp* ptheApp = (CSolidHoopsApp*) AfxGetApp();
	...
	{
		AfxMessageBox( _T("Failed to write temporary Parasolid file for embedding"), MB_ICONINFORMATION | MB_OK );
		return TK_Error;
	}
...
{ 
	...
	for (int i=0;i<numbodies;i++)
	{		
		KBody_ptr body = bodies->get(i);
		assert(body);

		HG_Compute_Geometry_Keys(KObj::cast(body), 1, &key, "bodies");		
		tk_stat = tk.KeyToIndex(key,index);
		assert( tk_stat == TK_Normal );
		if (m_mi->m_bPSF)
		{
 			putdata(&numbodies,sizeof(int));			//store:part number if PSF file
		}
 		putdata(&index,sizeof(long));					//store:index to HOOPS entity for body

		KFaceList_ptr faces;
		faces = body->GetFaceList();
		numfaces = faces->getarraysize();
 		putdata(&numfaces,sizeof(long));				//store:number of faces in body
		counter++;
		for (int j=0;j<numfaces;j++)
		{
			KFace_ptr face = faces->get(j);
			assert(face);

			if (HG_Compute_Geometry_Keys(KObj::cast(face), 1, &key, "faces"))
			{
				tk_stat = tk.KeyToIndex(key,index);
				assert( tk_stat == TK_Normal );
			}
			...
			num_edges = edges->getarraysize();
			putdata(&num_edges,sizeof(long));
			counter++;
			for (int k=0;k<num_edges;k++)
			{
				{
					KEdge_ptr edge = edges->get(k);
					assert(edge);

					HC_KEY keys[128];
					char num_keys = (char)HG_Compute_Geometry_Keys(KObj::cast(edge), 128, keys, "edges");					
					putdata(&num_keys,sizeof(char));
					if (m_mi->m_bPSF)
					{
						int ident;
						ident = (int) edge->GetId();
						putdata(&ident,sizeof(int));					//store:identifier number if PSF file
					}
...
{
	// for now set up a limitation of being able to store only one model
	if( m_componentList->getarraysize() != 1 )
	{
		AfxMessageBox(_T("Zero or more than one components present in this model. Only one model")
					  _T(" can be exported in gsf file"), MB_OK|MB_ICONSTOP);
		return TK_Error;
	}

	KComponent_ptr component = m_componentList->get(0);
	// no assemblies supported
	if( component && component->GetAssembly() )
	{
		AfxMessageBox(_T("Assemblies are currently not supported by gsf format"), MB_OK|MB_ICONSTOP);
		return TK_Error;
	}

	KModel_ptr model = component->GetModel();
	if( !model )
	{
		AfxMessageBox(_T("No model found to export. Assembly or Sub-assembly cannot be exported"), MB_OK|MB_ICONSTOP);
		return TK_Error;
	}
	...
	KSession_ptr session = KGetSession(); assert(session);
	{
		// NOTE: Limiting the scope of saveFile, this will prevent the temp_file
		// from getting locked and thus allow its deletion before we return - Rajesh B
		KCStream_ptr saveFile = session->CreateFileStream(H_ASCII_TEXT(m_psf_tempfilename), FALSE);
		if( !saveFile )
		{
			TCHAR msg[MVO_BUFFER_SIZE];
			_stprintf(msg, _T("Failed to create a temporary ProE file - %s"), m_psf_tempfilename);
			AfxMessageBox(msg);
			return TK_Error;
		}
		model->Save(saveFile);
