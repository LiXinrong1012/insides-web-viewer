...
// visibility off instead which also avoids the crash//Mustafa Bohari 15-Jun-09
	...
	//}}AFX_DATA_MAP
#ifdef PARASOLID
	DDX_Text(pDX, IDC_EDIT_SCHEMA_PATH, m_csSchemaPath);
	DDX_Check(pDX, IDC_FLATTEN, m_bFlatten);
	DDX_Check(pDX, IDC_READBINARYFILES, m_bUseBinaryFiles);
#endif // PARASOLID	
}
...
BEGIN_MESSAGE_MAP(FileOptionsPage, HOptionsPage)
	...
	//}}AFX_MSG_MAP
#ifdef PARASOLID
	ON_BN_CLICKED(IDC_BUTTON_BROWSE_SCHEMADIR, OnButtonSchemaDir)
#endif // PARASOLID
END_MESSAGE_MAP()
...
#ifdef PARASOLID
	m_csSchemaPath				= CAppSettings::SchemaPath;
	m_bFlatten					= bool2BOOL(CAppSettings::bFlattenAssemblies);
	m_bUseBinaryFiles			= bool2BOOL(CAppSettings::bBinaryX_TFiles);
#endif // PARASOLID
...
#ifdef PARASOLID
	CAppSettings::SchemaPath			= m_csSchemaPath;
	CAppSettings::bFlattenAssemblies	= BOOL2bool(m_bFlatten);
	CAppSettings::bBinaryX_TFiles		= BOOL2bool(m_bUseBinaryFiles);
#endif // PARASOLID
...
#ifdef PARASOLID
	// apply schema path if non-null
	if( _tcslen( CAppSettings::SchemaPath ) != 0 )
		HP_Set_Schema_Path(H_ASCII_TEXT(CAppSettings::SchemaPath));
 

	// apply flatten assembly
	HP_Set_Instances_To_Bodies(CAppSettings::bFlattenAssemblies);
#endif // PARASOLID
...
#ifdef PARASOLID
	// Do not reset the schema path - it is the user data
	//m_csSchemaPath				= CAppSettings::DefaultSchemaPath;
	m_bFlatten					= bool2BOOL(CAppSettings::DefaultbFlattenAssemblies);
	m_bUseBinaryFiles			= bool2BOOL(CAppSettings::DefaultbBinaryX_TFiles);
#endif // PARASOLID
...
#ifdef PARASOLID
void FileOptionsPage::OnButtonSchemaDir()
{
	CString csDlgCaption(_T("Choose the location of Schema Files..."));
	CString csDlgMsg(_T(""));
	CPathDialog dir_dlg(csDlgCaption, csDlgMsg, m_csSchemaPath);
	if (dir_dlg.DoModal()==IDOK)
		m_csSchemaPath = dir_dlg.GetPathName();
	UpdateData(FALSE);
}
#endif //PARASOLID
...
TessellationOptionsPage::TessellationOptionsPage(CWnd* pParent /*=NULL*/)
	: HOptionsPage(TessellationOptionsPage::IDD, pParent)
{
	//{{AFX_DATA_INIT(TessellationOptionsPage)
	m_bCurveChordAngTol = FALSE;
	m_bCurveChordTol = FALSE;
	m_bFacePlaneAngTol = FALSE;
	m_bFacetPlaneTol = FALSE;
	m_bSurfacePlaneAngTol = FALSE;
	m_bSurfacePlaneTol = FALSE;
	m_bLineCurveChordAngTol = FALSE;
	m_bLineCurveChordTol = FALSE;
	m_CurveChordAngTol = 0.0;
	m_CurveChordTol = 0.0;
	m_FacetPlaneAngTol = 0.0;
	m_FacetPlaneTol = 0.0;
	m_SurfacePlaneAngTol = 0.0;
	m_SurfacePlaneTol = 0.0;
	m_LineCurveChordAngTol = 0.0;
	m_LineCurveChordTol = 0.0;
	m_FacetMatch = _T("");
	//}}AFX_DATA_INIT
}


void TessellationOptionsPage::DoDataExchange(CDataExchange* pDX)
{
	HOptionsPage::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(TessellationOptionsPage)
	DDX_Check(pDX, IDC_CHECK_CURVE_CHORD_ANG_TOL, m_bCurveChordAngTol);
	DDX_Check(pDX, IDC_CHECK_CURVE_CHORD_TOL, m_bCurveChordTol);
	DDX_Check(pDX, IDC_CHECK_FACET_PLANE_ANG_TOL, m_bFacePlaneAngTol);
	DDX_Check(pDX, IDC_CHECK_FACET_PLANE_TOL, m_bFacetPlaneTol);
	DDX_Check(pDX, IDC_CHECK_SURFACE_PLANE_ANG_TOL, m_bSurfacePlaneAngTol);
	DDX_Check(pDX, IDC_CHECK_SURFACE_PLANE_TOL, m_bSurfacePlaneTol);
	DDX_Check(pDX, IDC_CHECK_LINE_CURVE_CHORD_ANG_TOL, m_bLineCurveChordAngTol);
	DDX_Check(pDX, IDC_CHECK_LINE_CURVE_CHORD_TOL, m_bLineCurveChordTol);
	DDX_Text(pDX, IDC_EDIT_CURVE_CHORD_ANG_TOL, m_CurveChordAngTol);
	DDX_Text(pDX, IDC_EDIT_CURVE_CHORD_TOL, m_CurveChordTol);
	DDX_Text(pDX, IDC_EDIT_FACET_PLANE_ANG_TOL, m_FacetPlaneAngTol);
	DDX_Text(pDX, IDC_EDIT_FACET_PLANE_TOL, m_FacetPlaneTol);
	DDX_Text(pDX, IDC_EDIT_SURFACE_PLANE_ANG_TOL, m_SurfacePlaneAngTol);
	DDX_Text(pDX, IDC_EDIT_SURFACE_PLANE_TOL, m_SurfacePlaneTol);
	DDX_Text(pDX, IDC_EDIT_LINE_CURVE_CHORD_ANG_TOL, m_LineCurveChordAngTol);
	DDX_Text(pDX, IDC_EDIT_LINE_CURVE_CHORD_TOL, m_LineCurveChordTol);
	DDX_CBString(pDX, IDC_COMBO_FACET_MATCH, m_FacetMatch);
	//}}AFX_DATA_MAP
...
BEGIN_MESSAGE_MAP(TessellationOptionsPage, HOptionsPage)
	//{{AFX_MSG_MAP(TessellationOptionsPage)
	ON_BN_CLICKED(IDC_CHECK_CURVE_CHORD_ANG_TOL, OnCheckCurveChordAngTol)
	ON_BN_CLICKED(IDC_CHECK_CURVE_CHORD_TOL, OnCheckCurveChordTol)
	ON_BN_CLICKED(IDC_CHECK_FACET_PLANE_ANG_TOL, OnCheckFacetPlaneAngTol)
	ON_BN_CLICKED(IDC_CHECK_FACET_PLANE_TOL, OnCheckFacetPlaneTol)
	ON_BN_CLICKED(IDC_CHECK_SURFACE_PLANE_ANG_TOL, OnCheckSurfacePlaneAngTol)
	ON_BN_CLICKED(IDC_CHECK_SURFACE_PLANE_TOL, OnCheckSurfacePlaneTol)
	ON_BN_CLICKED(IDC_CHECK_LINE_CURVE_CHORD_ANG_TOL, OnCheckLineCurveChordAngTol)
	ON_BN_CLICKED(IDC_CHECK_LINE_CURVE_CHORD_TOL, OnCheckLineCurveChordTol)
	ON_BN_CLICKED(IDC_BUTTON_RESET, OnButtonReset)
	ON_BN_CLICKED(IDC_BUTTON_APPLY, OnButtonApply)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
/////////////////////////////////////////////////////////////////////////////
// TessellationOptionsPage message handlers

static void PK_facet_match_t2CString( PK_facet_match_t match, CString& matchStr )		
{
	if(match == PK_facet_match_geom_c)
		matchStr = "Geom";				
	else if(match == PK_facet_match_trimmed_c)
		matchStr = "Trimmed";					
	else										
		matchStr = "Topol";	
}

static void CString2PK_facet_match_t( CString matchStr , PK_facet_match_t& match)		
{
	if( matchStr.CompareNoCase(_T("Geom")) == 0 )
		match = PK_facet_match_geom_c;
	else if( matchStr.CompareNoCase(_T("Trimmed")) == 0 )
		match = PK_facet_match_trimmed_c;
	else
		match = PK_facet_match_topol_c;
}


BOOL TessellationOptionsPage::OnInitDialog() 
{
	HOptionsPage::OnInitDialog();
	
	// get the exisiting settings
	m_bCurveChordTol		= (CAppSettings::IsCurveChordTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bCurveChordAngTol		= (CAppSettings::IsCurveChordAngTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bFacetPlaneTol		= (CAppSettings::IsFacetPlaneTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bFacePlaneAngTol		= (CAppSettings::IsFacetPlaneAngTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bSurfacePlaneTol		= (CAppSettings::IsSurfacePlaneTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bSurfacePlaneAngTol	= (CAppSettings::IsSurfacePlaneAngTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bLineCurveChordTol	= (CAppSettings::IsLineCurveChordTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bLineCurveChordAngTol	= (CAppSettings::IsLineCurveChordAngTol == PK_LOGICAL_true) ? TRUE : FALSE;

	m_CurveChordTol			= atof(H_ASCII_TEXT(CAppSettings::CurveChordTol));
	m_CurveChordAngTol		= atof(H_ASCII_TEXT(CAppSettings::CurveChordAngTol));
	m_FacetPlaneTol			= atof(H_ASCII_TEXT(CAppSettings::FacetPlaneTol));
	m_FacetPlaneAngTol		= atof(H_ASCII_TEXT(CAppSettings::FacetPlaneAngTol));
	m_SurfacePlaneTol		= atof(H_ASCII_TEXT(CAppSettings::SurfacePlaneTol));
	m_SurfacePlaneAngTol	= atof(H_ASCII_TEXT(CAppSettings::SurfacePlaneAngTol));
	m_LineCurveChordTol		= atof(H_ASCII_TEXT(CAppSettings::LineCurveChordTol));
	m_LineCurveChordAngTol	= atof(H_ASCII_TEXT(CAppSettings::LineCurveChordAngTol));

	PK_facet_match_t2CString(CAppSettings::FacetMatch, m_FacetMatch);
...
void TessellationOptionsPage::ApplySettings()
{
	// set the settings to the bridge
	PK_TOPOL_render_line_o_t		line_opts;	
	PK_TOPOL_render_facet_go_o_t	facet_opts;
	PK_TOPOL_facet_mesh_o_t			mesh_opts;

	HP_Show_Tessellation_Options(&line_opts, &facet_opts, &mesh_opts);

	bool is_changed = false;

	PK_LOGICAL_t new_is_curve_chord_tol 		=	(m_bCurveChordTol	== TRUE) ? PK_LOGICAL_true : PK_LOGICAL_false;
	PK_LOGICAL_t new_is_curve_chord_ang 		=	(m_bCurveChordAngTol== TRUE) ? PK_LOGICAL_true : PK_LOGICAL_false;
	PK_LOGICAL_t new_is_facet_plane_tol 		=	(m_bFacetPlaneTol	== TRUE) ? PK_LOGICAL_true : PK_LOGICAL_false;
	PK_LOGICAL_t new_is_facet_plane_ang 		=	(m_bFacePlaneAngTol	== TRUE) ? PK_LOGICAL_true : PK_LOGICAL_false;
	PK_LOGICAL_t new_is_surface_plane_tol		=   (m_bSurfacePlaneTol	== TRUE) ? PK_LOGICAL_true : PK_LOGICAL_false;
	PK_LOGICAL_t new_is_surface_plane_ang		=   (m_bSurfacePlaneAngTol== TRUE) ? PK_LOGICAL_true : PK_LOGICAL_false;
	PK_LOGICAL_t new_is_line_curve_chord_tol 	=	(m_bLineCurveChordTol	== TRUE) ? PK_LOGICAL_true : PK_LOGICAL_false;
	PK_LOGICAL_t new_is_line_curve_chord_ang 	=	(m_bLineCurveChordAngTol== TRUE) ? PK_LOGICAL_true : PK_LOGICAL_false;

	if( new_is_curve_chord_tol !=	mesh_opts.is_curve_chord_tol )
	{
		CAppSettings::IsCurveChordTol = mesh_opts.is_curve_chord_tol =	new_is_curve_chord_tol;
		is_changed = true;
	}
	if( new_is_curve_chord_ang !=	mesh_opts.is_curve_chord_ang )
	{
		CAppSettings::IsCurveChordAngTol = mesh_opts.is_curve_chord_ang =	new_is_curve_chord_ang;
		is_changed = true;
	}
	if( new_is_facet_plane_tol !=	mesh_opts.is_facet_plane_tol )
	{
		CAppSettings::IsFacetPlaneTol = mesh_opts.is_facet_plane_tol =	new_is_facet_plane_tol;
		is_changed = true;
	}
	if( new_is_facet_plane_ang !=	mesh_opts.is_facet_plane_ang )
	{
		CAppSettings::IsFacetPlaneAngTol = mesh_opts.is_facet_plane_ang =	new_is_facet_plane_ang;
		is_changed = true;
	}
	if( new_is_surface_plane_tol != mesh_opts.is_surface_plane_tol ) 
	{
		CAppSettings::IsSurfacePlaneTol = mesh_opts.is_surface_plane_tol = new_is_surface_plane_tol;
		is_changed = true;
	}
	if( new_is_surface_plane_ang != mesh_opts.is_surface_plane_ang ) 
	{
		CAppSettings::IsSurfacePlaneAngTol = mesh_opts.is_surface_plane_ang = new_is_surface_plane_ang;
		is_changed = true;
	}
	if( new_is_line_curve_chord_tol !=	line_opts.is_curve_chord_tol )
	{
		CAppSettings::IsLineCurveChordTol = line_opts.is_curve_chord_tol =	new_is_line_curve_chord_tol;
		is_changed = true;
	}
	if( new_is_line_curve_chord_ang !=	line_opts.is_curve_chord_ang )
	{
		CAppSettings::IsLineCurveChordAngTol = line_opts.is_curve_chord_ang =	new_is_line_curve_chord_ang;
		is_changed = true;
	}


	if( mesh_opts.curve_chord_tol	!= m_CurveChordTol )
	{
		mesh_opts.curve_chord_tol	= m_CurveChordTol;
		CAppSettings::CurveChordTol.Format(_T("%.5f"),  m_CurveChordTol );
		is_changed = true;
	}
	if( mesh_opts.curve_chord_ang	!= m_CurveChordAngTol )
	{
		mesh_opts.curve_chord_ang	= m_CurveChordAngTol;
		CAppSettings::CurveChordAngTol.Format(_T("%.5f"), m_CurveChordAngTol );
		is_changed = true;
	}
	if( mesh_opts.facet_plane_tol	!= m_FacetPlaneTol )	
	{
		mesh_opts.facet_plane_tol	= m_FacetPlaneTol;	
		CAppSettings::FacetPlaneTol.Format(_T("%.5f"), m_FacetPlaneTol );
		is_changed = true;
	}
	if( mesh_opts.facet_plane_ang	!= m_FacetPlaneAngTol )
	{
		mesh_opts.facet_plane_ang	= m_FacetPlaneAngTol;
		CAppSettings::FacetPlaneAngTol.Format(_T("%.5f"), m_FacetPlaneAngTol );
		is_changed = true;
	}
	if( mesh_opts.surface_plane_tol	!= m_SurfacePlaneTol )
	{
		mesh_opts.surface_plane_tol	= m_SurfacePlaneTol;
		CAppSettings::SurfacePlaneTol.Format(_T("%.5f"), m_SurfacePlaneTol );
		is_changed = true;
	}
	if( mesh_opts.surface_plane_ang != m_SurfacePlaneAngTol )
	{
		mesh_opts.surface_plane_ang = m_SurfacePlaneAngTol;
		CAppSettings::SurfacePlaneAngTol.Format(_T("%.5f"), m_SurfacePlaneAngTol );
		is_changed = true;
	}
	if( line_opts.curve_chord_tol	!= m_LineCurveChordTol )
	{
		line_opts.curve_chord_tol	= m_LineCurveChordTol;
		CAppSettings::LineCurveChordTol.Format(_T("%.5f"),  m_LineCurveChordTol );
		is_changed = true;
	}
	if( line_opts.curve_chord_ang	!= m_LineCurveChordAngTol )
	{
		line_opts.curve_chord_ang	= m_LineCurveChordAngTol;
		CAppSettings::LineCurveChordAngTol.Format(_T("%.5f"), m_LineCurveChordAngTol );
		is_changed = true;
	}

	PK_facet_match_t new_match;
	CString2PK_facet_match_t( m_FacetMatch, new_match);
	if( new_match !=  mesh_opts.match )
	{
		CAppSettings::FacetMatch = mesh_opts.match = new_match;
		is_changed = true;
	}

	HP_Set_Tessellation_Options(&line_opts, &facet_opts, &mesh_opts);

	// Get the active view to apply the new settings
	CMDIChildWnd *pChild = (CMDIChildWnd *) ((CMDIFrameWnd*)AfxGetMainWnd())->GetActiveFrame();
	assert(pChild);
	CSolidHoopsView* pActiveView = (CSolidHoopsView *)pChild->GetActiveView();
	if( is_changed && pActiveView )
		AfxMessageBox(_T("Tessellation Settings will only be effective for newly opened documents"));

 
}
void TessellationOptionsPage::OnOK()
{ 
	ApplySettings();
	HOptionsPage::OnOK();
}

#define UPDATE_THIS_EDIT_CONTROL( check_box_id, edit_box_id )	\
	{															\
		CButton* check_box = (CButton*) GetDlgItem(check_box_id);	\
		CEdit* edit_box = (CEdit*) GetDlgItem(edit_box_id);			\
		assert( check_box );										\
		assert( edit_box );											\
		if( check_box && edit_box )									\
		{															\
			if( check_box->GetCheck() == 1 )						\
...
void TessellationOptionsPage::UpdateEditControls()
{
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_CURVE_CHORD_TOL,	IDC_EDIT_CURVE_CHORD_TOL );
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_SURFACE_PLANE_TOL,	IDC_EDIT_SURFACE_PLANE_TOL );
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_FACET_PLANE_TOL,	IDC_EDIT_FACET_PLANE_TOL );
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_CURVE_CHORD_ANG_TOL,IDC_EDIT_CURVE_CHORD_ANG_TOL );
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_SURFACE_PLANE_ANG_TOL,	IDC_EDIT_SURFACE_PLANE_ANG_TOL );
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_FACET_PLANE_ANG_TOL,	IDC_EDIT_FACET_PLANE_ANG_TOL );
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_LINE_CURVE_CHORD_TOL,	IDC_EDIT_LINE_CURVE_CHORD_TOL );
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_LINE_CURVE_CHORD_ANG_TOL,IDC_EDIT_LINE_CURVE_CHORD_ANG_TOL );
}


void TessellationOptionsPage::OnCheckCurveChordAngTol() 
{
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_CURVE_CHORD_ANG_TOL,IDC_EDIT_CURVE_CHORD_ANG_TOL );
}

void TessellationOptionsPage::OnCheckCurveChordTol() 
{
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_CURVE_CHORD_TOL,	IDC_EDIT_CURVE_CHORD_TOL );
}

void TessellationOptionsPage::OnCheckFacetPlaneAngTol() 
{
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_FACET_PLANE_ANG_TOL,	IDC_EDIT_FACET_PLANE_ANG_TOL );
}

void TessellationOptionsPage::OnCheckFacetPlaneTol() 
{
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_FACET_PLANE_TOL,	IDC_EDIT_FACET_PLANE_TOL );
}

void TessellationOptionsPage::OnCheckSurfacePlaneAngTol() 
{
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_SURFACE_PLANE_ANG_TOL,	IDC_EDIT_SURFACE_PLANE_ANG_TOL );
}

void TessellationOptionsPage::OnCheckSurfacePlaneTol() 
{
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_SURFACE_PLANE_TOL,	IDC_EDIT_SURFACE_PLANE_TOL );
}

void TessellationOptionsPage::OnCheckLineCurveChordAngTol() 
{
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_LINE_CURVE_CHORD_ANG_TOL,IDC_EDIT_LINE_CURVE_CHORD_ANG_TOL );
}

void TessellationOptionsPage::OnCheckLineCurveChordTol() 
{
	UPDATE_THIS_EDIT_CONTROL( IDC_CHECK_LINE_CURVE_CHORD_TOL,	IDC_EDIT_LINE_CURVE_CHORD_TOL );
}


void TessellationOptionsPage::OnButtonApply() 
{
	UpdateData(true);
	ApplySettings();
}
...
void TessellationOptionsPage::OnButtonReset() 
{
	// reset to our default 
	m_bCurveChordTol		= (CAppSettings::DefaultIsCurveChordTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bCurveChordAngTol		= (CAppSettings::DefaultIsCurveChordAngTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bFacetPlaneTol		= (CAppSettings::DefaultIsFacetPlaneTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bFacePlaneAngTol		= (CAppSettings::DefaultIsFacetPlaneAngTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bSurfacePlaneTol		= (CAppSettings::DefaultIsSurfacePlaneTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bSurfacePlaneAngTol	= (CAppSettings::DefaultIsSurfacePlaneAngTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bLineCurveChordTol	= (CAppSettings::IsLineCurveChordTol == PK_LOGICAL_true) ? TRUE : FALSE;
	m_bLineCurveChordAngTol	= (CAppSettings::IsLineCurveChordAngTol == PK_LOGICAL_true) ? TRUE : FALSE;

	m_CurveChordTol			= atof(H_ASCII_TEXT(CAppSettings::DefaultCurveChordTol));
	m_CurveChordAngTol		= atof(H_ASCII_TEXT(CAppSettings::DefaultCurveChordAngTol));
	m_FacetPlaneTol			= atof(H_ASCII_TEXT(CAppSettings::DefaultFacetPlaneTol));
	m_FacetPlaneAngTol		= atof(H_ASCII_TEXT(CAppSettings::DefaultFacetPlaneAngTol));
	m_SurfacePlaneTol		= atof(H_ASCII_TEXT(CAppSettings::DefaultSurfacePlaneTol));
	m_SurfacePlaneAngTol	= atof(H_ASCII_TEXT(CAppSettings::DefaultSurfacePlaneAngTol));
	m_LineCurveChordTol		= atof(H_ASCII_TEXT(CAppSettings::LineCurveChordTol));
	m_LineCurveChordAngTol	= atof(H_ASCII_TEXT(CAppSettings::LineCurveChordAngTol));

	PK_facet_match_t2CString(CAppSettings::DefaultFacetMatch, m_FacetMatch);
...
void TessellationOptionsPage::DoDataExchange(CDataExchange* pDX)
{
	HOptionsPage::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(TessellationOptionsPage)
	DDX_Text(pDX, IDC_EDIT_ANGULAR_TOL, m_angularTol);
	DDV_MinMaxDouble(pDX, m_angularTol, 0., 1.);
	DDX_Text(pDX, IDC_EDIT_PLANAR_TOL, m_planarTol);
	DDX_Text(pDX, IDC_EDIT_TESSELLATION_TOL, m_tessellationTol);
	//}}AFX_DATA_MAP
}
...
BOOL TessellationOptionsPage::OnInitDialog() 
{
	...
	m_angularTol		= atof(H_ASCII_TEXT(CAppSettings::AngularTolerance));
	m_planarTol			= atof(H_ASCII_TEXT(CAppSettings::PlanarTolerance));
	m_tessellationTol	= atof(H_ASCII_TEXT(CAppSettings::TessellationTolerance));
...
void TessellationOptionsPage::ApplySettings()
{
	...
	if( atof(H_ASCII_TEXT(CAppSettings::AngularTolerance)) != m_angularTol )
	{
		CAppSettings::AngularTolerance.Format(_T("%.5f"),  m_angularTol );
		is_changed = true;
	}

	if( atof(H_ASCII_TEXT(CAppSettings::PlanarTolerance)) != m_planarTol )
	{
		CAppSettings::PlanarTolerance.Format(_T("%.5f"),  m_planarTol );
		is_changed = true;
	}

	if( atof(H_ASCII_TEXT(CAppSettings::TessellationTolerance)) != m_tessellationTol )
	{
		CAppSettings::TessellationTolerance.Format(_T("%.5f"),  m_tessellationTol );
		is_changed = true;
	}
	...
	// we want to apply changes even if they are not changed
	KTriangulator_ptr triangulator;
	HG_Show_Triangulator(triangulator);
	triangulator->SetAngularTolerance(m_angularTol);
	triangulator->SetPlanarTolerance(m_planarTol);
	triangulator->SetTessellationTolerance(m_tessellationTol);
...
void TessellationOptionsPage::OnButtonReset() 
{
	// reset to Granite default
	m_angularTol		= atof(H_ASCII_TEXT(CAppSettings::DefaultAngularTolerance));
	m_planarTol			= atof(H_ASCII_TEXT(CAppSettings::DefaultPlanarTolerance));
	m_tessellationTol	= atof(H_ASCII_TEXT(CAppSettings::DefaultTessellationTolerance));
