...
enum HViewerPerformance
{
	DefaultMode,
	Optimal,
	Custom,
	SegmentLevelDisplayLists,
	GeometryLevelDisplayLists,
	StaticModel,
	CullingThreshold,
	FixedFramerate,
	TargetFramerate
...
public:
	...
	PerformanceSettings();
	void Apply(CSolidHoopsView *view);
	void Get();
	void Reset();
	void SetMode(HViewerPerformance perf);
	void SetModeInternal(HViewerPerformance perf);
	void UnSet(HViewerPerformance perf);
	void UnSetInternal(HViewerPerformance perf);
	bool Show(HViewerPerformance perf);
	HViewerPerformance GetCurrentSetting();
	bool Compare(PerformanceSettings &c);

	UINT	m_Cutoff;
	UINT	m_MaxThreshold;
	UINT	m_DetailSteps;
	BOOL	m_bUseFramerate;
	BOOL	m_bDynamicAdjustment;
	BOOL	m_bUseLods;
	
	CString	m_csHLRMode;
	BOOL	m_bStaticModel;
	BOOL	m_bDisableEditing;
	BOOL	m_bLMV;
	BOOL	m_bOcclusionCulling;
	UINT	m_OcclusionThreshold;
	CString m_csDisplayList;
	BOOL	m_bCullingThreshold;
	UINT	m_CullingThreshold;
	CString	m_csFramerateMode;
	float	m_FramerateTime; 
	CString	m_csMultiThreading;
	CColorBtn	m_clrbtnFakeHLR;
private:
	void ApplyInternal();
};
...
public:
	...
	void Apply(CSolidHoopsView *view);
	void Get();
	void Set(HViewerEffects fx);
	void SetInternal(HViewerEffects fx);
	void UnSet(HViewerEffects fx);
	void UnSetInternal(HViewerEffects fx);
	bool Show(HViewerEffects fx);

	BOOL m_AmbientOcclusion;
	BOOL m_Bloom;
	BOOL m_FastSilhouetteEdges;
	BOOL m_ReflectionPlane;
	BOOL m_ShadowMaps;
	BOOL m_SimpleShadows;
private:
	void ApplyInternal();
};
...
public:
	...
	void	Init();
	void	BooleanOperation(unsigned long	operation);
	...
	bool FindBox(PK_ENTITY_t entity,PK_BOX_t *box);
#endif // PARASOLID
