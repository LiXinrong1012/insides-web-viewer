...
CProgressDlg::CProgressDlg(CWnd* pParent, LPCTSTR pszTitle, BOOL bSmooth /* = FALSE */)
{
    CommonConstruct();
    m_strTitle = pszTitle;
...
void CProgressDlg::CommonConstruct()
{
    m_nNumTextLines  = 4;
    m_nPrevPos       = 0;
    m_nPrevPercent   = 0;
...
BOOL CProgressDlg::Create(CWnd* pParent, LPCTSTR pszTitle, BOOL bSmooth /* = FALSE */)
{
    BOOL bSuccess;
...
BOOL CProgressDlg::GoModal(LPCTSTR pszTitle /*=_T("Progress")"*/, BOOL bSmooth /*=FALSE*/)
{
...
void CProgressDlg::SetWindowSize(int nNumTextLines, int nWindowWidth /*=390*/)
{
    int nMargin = 10;
...
void CProgressDlg::SetRange(int nLower, int nUpper, int nStep /* = 1 */)    
{
    if (!::IsWindow(GetSafeHwnd())) 
        return;
...
void CProgressDlg::SetText(LPCTSTR fmt, ...)
{
    if (!::IsWindow(GetSafeHwnd())) 
        return;

    va_list args;
    TCHAR buffer[MVO_BUFFER_SIZE];
...
void CProgressDlg::GetPreviousSettings()
{
...
void CProgressDlg::SaveCurrentSettings()
{   
    if (!IsWindow(m_hWnd))
        return;
...
bool StreamProgressCallback(unsigned HLONG so_far, unsigned HLONG expected, void * user_data)
{
	HStreamProgressDlg* pDlg = (HStreamProgressDlg*) user_data;
	// if we do not find the what we want, let the read/write take its natural course
	if( !pDlg )
		return true;

	// check if user wanted to cancel
	if( pDlg->Cancelled() ) 
	{
	...
	char dlg_text[MVO_BUFFER_SIZE];
	if( pDlg->GetProgressMode() == HStreamRead )
		sprintf(dlg_text, "%.1f K of %.1f K read ...\n\nPress 'ESC' key or 'Cancel' to abort", (so_far/1024.0f), (expected/1024.0f) );
	else
		sprintf(dlg_text, "%lu objects of %lu objects written...\n\nPress 'ESC' key or 'Cancel' to abort", so_far, expected );

	pDlg->SetText(H_TEXT(dlg_text));
	pDlg->SetRange(0, expected, 1);
	pDlg->SetPos( so_far );
	pDlg->PeekAndPump();
    pDlg->RedrawWindow(NULL,NULL,RDW_ERASE|RDW_FRAME|RDW_INVALIDATE|RDW_UPDATENOW);
	return true;
}


HStreamProgressDlg::HStreamProgressDlg(HStreamFileToolkit* tk, ProgressMode readOrWrite)
{
	assert( tk );
	m_tk = tk;
	m_ProgressMode = readOrWrite;
}


BOOL HStreamProgressDlg::Create(CWnd* pParent, LPCTSTR pszTitle, BOOL bSmooth)
{
	m_tk->SetProgressCallback( StreamProgressCallback );
	m_tk->SetProgressValue( (void*) this );
	BOOL b_ret = CProgressDlg::Create(pParent, pszTitle, bSmooth);
	if( b_ret == TRUE )
	{
		SetRedraw( TRUE );
		ShowWindow(SW_SHOWNA);
...
HStreamProgressDlg::~HStreamProgressDlg()
{
	// remove the stream callbacks and stuff
	m_tk->SetProgressValue(0);
	m_tk->SetProgressCallback(0);
}

bool HStreamProgressDlg::SetCurrentProgress(unsigned long so_far, unsigned long expected, void * user_data)
{
	return StreamProgressCallback(so_far, expected, user_data);
}
