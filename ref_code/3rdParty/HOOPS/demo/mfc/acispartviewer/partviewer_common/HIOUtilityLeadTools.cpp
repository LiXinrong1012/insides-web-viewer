...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/shared/partviewer_common/HIOUtilityLeadTools.cpp,v 1.14 2008-02-27 01:49:36 jason Exp $
//
#include "stdafx.h"
...
public:
	...
	*/
	static bool FileToImage(const char *filename, HC_KEY *image_key, HC_KEY segment_key, 
		const char *imagename, const char *format, bool local = false, HInputHandler * peer);
	...
	/*
		...
		\return A boolean value indicating success or failure.
	*/
	static bool ImageToFile( HC_KEY image_key, const char *filename, HOutputHandler * peer);
	...
	HC_KEY * image_key, HC_KEY segment_key, HInputHandlerOptions * options)
{
	...
	bool result = HLeadToolsInternal::FileToImage(filename, image_key, segment_key, image_name, image_format, local_image, this);
...
bool HLeadToolsInternal::FileToImage(const char *filename, HC_KEY *image_key, HC_KEY segment_key, 
									 const char *imagename, const char *format, bool local, HInputHandler * peer))
{
	...
	{
		peer->ReportInputInformation ("Error: No file extension. Could not determine input file format.\n");
		return false;
	}
	...
	if(l_err != SUCCESS)
	{
		peer->ReportInputInformation ("Error: LEAD TOOLS API L_LoadBitmap failed.\n");
		return false;
	}
	...
	for(int i = n_row_begin; i <= n_row_end; i++)
	{
	  ...
	  {
		peer->ReportInputInformation ("Error: LEAD TOOLS API L_GetBitmapRow failed.\n");
		 return false;
	  }
	}
...
HFileOutputResult HIOUtilityLeadTools::FileOutputFromImageKey(const char * filename, HC_KEY image_key, HOutputHandlerOptions * options)
{
	...
	bool result = HLeadToolsInternal::ImageToFile(image_key, filename, this);
...
bool HLeadToolsInternal::ImageToFile( HC_KEY image_key, const char *filename, HOutputHandler * peer)
{
	char file_ext[MVO_BUFFER_SIZE]; 
	HUtility::FindFileNameExtension(filename, file_ext);
	...
	if(strlen(file_ext) == 0) 
	{
		peer->ReportOutputInformation ("Error: No file extension. Could not determine file format for export.\n");
		return false;
	}
	...
	{
		peer->ReportOutputInformation ("Error: Unsupported file format for export.\n");
		return false;
	}
	...
	{
		peer->ReportOutputInformation ("Error: Export for non-rgb formats not implemented\n");
		return false;
	}
	...
	if( h_width < 1 )
	{
		peer->ReportOutputInformation ("Error: Image width too small for export\n");
		return false;
	}
	...
	if(l_err != SUCCESS)
	{
		peer->ReportOutputInformation ("Error: LEAD TOOLS API L_CreateBitmap failed.\n");
		return false;
	}
	...
	if(l_err != SUCCESS)
	{
		peer->ReportOutputInformation ("Error: LEAD TOOLS API L_SaveBitmap failed.\n");
		return false;
	}
