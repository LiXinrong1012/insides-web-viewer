  ...
  if (string_info->length != 0)
    {
      ...
      if (string_info->datum == (unsigned char *) NULL)
        ThrowMagickFatalException(ResourceLimitFatalError,
          "UnableToAcquireString",strerror(errno));
    }
  return(string_info);
}

...
MagickExport size_t CopyMagickString(char *destination,const char *source,
  const size_t length)
{
	size_t dif=destination>source?destination-source:source-destination;
	if(!length)
...
#define MAX_TEMP_BUFFER_SIZE	4096
		...
		if(length > MAX_TEMP_BUFFER_SIZE)
			buffer = (char *) malloc(length);
		strncpy(buffer, source, length);
		strncpy(destination, buffer, length);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  DestroyString() zeros memory associated with a string.
%
  ...
  const char *search,const char *replace)
{
    ...
    */
    ...
    if (copy_length != 0)
      ...
      */
      ...
      (void) strcat(result+destination_offset,replace);
  ...
  */
  ...
  (void) strcat(result+destination_offset,source);
