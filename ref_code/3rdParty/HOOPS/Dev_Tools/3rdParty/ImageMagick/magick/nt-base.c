...
BOOL WINAPI DllMain(HINSTANCE handle,DWORD reason,LPVOID lpvReserved)
{
  switch (reason)
  {
    case DLL_PROCESS_ATTACH:
    {
      ...
      new_path=(char *) AcquireMagickMemory(16*MaxTextExtent);
      path=(char *) AcquireMagickMemory(16*MaxTextExtent);
...
%
*/
MagickExport int NTFileTruncate(int file,off_t length)
{
  HANDLE
    handle;
  ...
  offset=SetFilePointer(handle,0,NULL,FILE_CURRENT);
  if ((offset == 0xFFFFFFFF) ||
      (SetFilePointer(handle,length,NULL,FILE_BEGIN) == 0xFFFFFFFF) ||
  ...
  const char *gs_productfamily)
{
  ...
  (void) sprintf(key, "Software\\%s",gs_productfamily);
  ...
  char *ptr,int len,const char *gs_productfamily)
{
  ...
  (void) sprintf(dotversion,"%d.%02d",(int)(gs_revision/100),(int)
    (gs_revision % 100));
  (void) sprintf(key, "Software\\%s\\%s",gs_productfamily, dotversion);
  length = len;
