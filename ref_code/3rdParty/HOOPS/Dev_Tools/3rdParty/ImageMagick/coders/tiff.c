  ...
  */
  ...
  do
  {
    ...
    if (TIFFGetField(tiff,TIFFTAG_PHOTOSHOP,&length,&profile) == 1)
      (void) ReadProfile(image,"iptc",profile,(long) length);
#endif
      ...
      case ReadSingleSampleMethod:
      {
        ...
        */
        ...
        if (image->matte != MagickFalse)
          packet_size*=2;
        length=Max((size_t) TIFFScanlineSize(tiff),packet_size*
          samples_per_pixel*width);
        scanline=(unsigned char *) AcquireMagickMemory(length*
          sizeof(*scanline));
        ...
        if (photometric == PHOTOMETRIC_PALETTE)
          {
            ...
            for (i=0; i < (long) image->colors; i++)
            {
              ...
              image->colormap[i].green=
                ScaleAnyToQuantum(green_colormap[i],range);
        ...
        */
        ...
        for (y=0; y < (long) image->rows; y+=rows)
        {
          ...
          for (x=0; x < (long) image->columns; x+=columns)
          {
            ...
            if ((x+columns) < (uint32) image->columns)
    ...
    switch (compress_tag)
    {
      ...
      case COMPRESSION_ADOBE_DEFLATE:
      {
        ...
        (void) TIFFSetField(tiff,TIFFTAG_ZIPQUALITY,image_info->quality ==
          UndefinedCompressionQuality ? 7 : Min(image_info->quality/10,9));
        break;
      }
