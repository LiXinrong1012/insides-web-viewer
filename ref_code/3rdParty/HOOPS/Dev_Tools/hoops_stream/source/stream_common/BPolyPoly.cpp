...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/shared/stream_common/BPolyPoly.cpp,v 1.32 2010-01-21 23:37:55 covey Exp $
//
...
TK_Status TK_PolyPolypoint::validate_primitive_count (BStreamFileToolkit & tk) alter {
    ...
    //else it's assumed to be valid already
    return TK_Normal;
}
    ...
    int *num_dimensions_out) 
{
    ...
    for (i = 0; i < primitive_count; i++) {
        ...
        if (!all_primitives_uniform)
            goto done;
        ptr = end; //get ready for the next primitive
    }
...
#endif
} //end static helper function analyze_floats
...
TK_Status TK_PolyPolypoint::analyze_dimensionality(BStreamFileToolkit & tk) alter {
    ...
    //just call straight through to the helper function
...
#ifndef BSTREAM_READONLY
//reorganize floats so that any redundant information is easy to leave out.
...
} //end static helper function unmangle_floats
...
static void predict(int i, int num_dimensions, int *minusone, int *minustwo, int *prediction) 
{
    ...
    //special handling for the first two points
    if (i < 2) {
        if (i < 1)
...
TK_Status TK_PolyPolypoint::compute_line_extend_points (BStreamFileToolkit & tk) alter 
{
    ...
    for (i = 0; i < m_point_count; i++) {
        //predict uses minusone for i>0 and minustwo for i>1.  computes a prediction for the next vertex location.
        predict(i, m_points_num_dimensions, minusone, minustwo, prediction);
        //we're done with minustwo now.  point val to it so that it will overwrite and then get swapped 
        //into minusone at the end of this iteration
        val = minustwo;  
        for (j = 0; j < m_points_num_dimensions; j++) {
            val[j] = (unsigned int)((fptr[j] - min[j]) * range[j] + 0.5f);
            CLAMP(prediction[j], 0, highest);
            diff[j] = prediction[j] - val[j];
            //add "highest" to diff[j] to make it always non-negative.  Requires extra bit per sample
            pack.Put(m_bits_per_sample + 1, diff[j] + highest);
        }
        fptr += m_points_num_dimensions;
        //swap minusone and minustwo
        temp = minusone;
        minusone = minustwo;
        minustwo = temp;
    }
    ...
    pack.SwapBytes(); //account for little/big endian
    ...
    return tk.Error (stream_readonly);
#endif
} // end function compute_line_extend_points
...
TK_Status TK_PolyPolypoint::compute_trivial_points (BStreamFileToolkit & tk) alter 
{
    ...
    if (m_subop & TKPP_GLOBAL_QUANTIZATION) {
        ...
        //should've already been checked
    ...
    if ((m_bits_per_sample != 8) || (m_subop & TKPP_ANY_2D_MASK)) {
        /* at this point, we don't know if m_bbox should be treated
    ...
    return tk.Error (stream_readonly);
#endif
} // end function TK_PolyPolypoint::compute_trivial_points
...
TK_Status TK_PolyPolypoint::write_compressed_points (BStreamFileToolkit & tk) alter 
{
    ...
    switch (m_substage) {
        ...
        //pointcount already handled externally

        //the bounding box (for the fully-specified dimensions only)
        case 0: {
            if (!(m_subop & TKPP_GLOBAL_QUANTIZATION)) {
        ...
        case 3: {
            ...
            if (!(m_subop & TKPP_ANY_2D_MASK)) {
                //early escape.  Nothing left to do.
                m_substage = 0;
                break;
            }
            m_substage = 0;
    ...
    return tk.Error (stream_readonly);
#endif
} // end function TK_PolyPolypoint::write_compressed_points
...
TK_Status TK_PolyPolypoint::write_trivial_leftovers (BStreamFileToolkit & tk) alter 
{
    ...
    switch (m_substage) {
        //figure out how many floats we have left over.  If it's just one float, send it and get out.
        case 0: {
            m_templen = m_points_num_floats - (m_points_num_dimensions * m_point_count);
            if (m_templen == 1) {
                //pointer to the leftovers after normal float arrays
        ...
        case 1: {
            //pointer to the leftovers after normal float arrays
            ptr = m_points + (m_points_num_dimensions * m_point_count);  
            // quantize the rest as a simple 1d array of floats
            status = quantize_and_pack_floats(
                            tk, 
                            m_templen, 
                            1, 
        ...
        //the compression scheme for the leftovers (always CS_TRIVIAL for now)
        case 2: {
        ...
        //no bits per sample here: assume same as before

        //1d leftover float data
        case 5: {
            if ((status = PutData(tk, m_workspace, m_workspace_used)) != TK_Normal)
                return status;
            m_substage = 0;
    ...
    return tk.Error (stream_readonly);
#endif
} // end function TK_PolyPolypoint::write_trivial_leftovers
...
} //end function read_compressed_points
...
TK_Status TK_PolyPolypoint::process_trivial_points (BStreamFileToolkit & tk) alter {
    ...
    if (status != TK_Normal)
        return status;
    //xxx unquantize_and_unpack_floats and trivial_decompress want to allocate their own 
    //memory for now, but that should change in the future.
    memcpy(m_points, temp, m_point_count * m_points_num_dimensions * sizeof(float));
    delete [] temp;
    return TK_Normal;

} //end function process_trivial_points
...
TK_Status TK_PolyPolypoint::process_line_extend_points (BStreamFileToolkit & tk) alter {
    ...
    for (i = 0; i < m_point_count; i++) {
        ...
        for (j = 0; j < m_points_num_dimensions; j++) {
            CLAMP(prediction[j], 0, highest);
            //"highest" was added to diff[j] on write side to make it always non-negative
            //bias added 1 to bits per sample required
            diff[j] = pack.Get(m_bits_per_sample + 1) - highest;
            val[j] = prediction[j] - diff[j];
            if (val[j] != highest)
                fptr[j] = val[j] * range[j] + m_bbox[j];
            else
                fptr[j] = m_bbox[j + m_points_num_dimensions]; // directly set to max -- avoid fp rounding error
...
} //end function process_line_extend_points
...
TK_Status TK_PolyPolypoint::read_trivial_leftovers (BStreamFileToolkit & tk) alter {
    ...
    switch (m_substage) {
        ...
        //figure out if there is any data left, and if we need to continue or not
        case 0: {
            if (m_subop & TKPP_ANY_2D_MASK)
                m_templen = m_points_num_floats - (m_points_num_dimensions * m_point_count);
            else
            ...
            if (m_templen <= 1) {
                if (m_templen == 1) {
                    ptr = m_points + (m_points_num_dimensions * m_point_count);  //pointer to the leftovers
        ...
        //get the compression scheme (but assume that it's CS_TRIVIAL for now)
        case 1: {
        ...
        //the rest is a simple 1d array of floats 
        //2 floats for the 1d "bounding box"
        case 2: {
            if (!(m_subop & TKPP_GLOBAL_QUANTIZATION)) {
        ...
        //process the data
        case 5: {
            ptr = m_points + (m_points_num_dimensions * m_point_count); //pointer to the lefovers
            status = unquantize_and_unpack_floats(
                            tk, 
                            m_templen, 
                            1, 
            ...
            if (status != TK_Normal)
                return status;
            //xxx unquantize_and_unpack_floats wants to allocate its own memory for now,
...
} //end function read_trivial_leftovers
...
TK_Status TK_PolyPolypoint::mangle (BStreamFileToolkit & tk) alter 
{
    ...
    //xxx it should be possible to do this work in place instead
...
TK_Status TK_PolyPolypoint::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        case 0: {
            if (tk.GetTargetVersion() < 702)    // first added in 7.02, so just skip it for lower versions
                return TK_Normal;
            if (m_subop & TKPP_GLOBAL_QUANTIZATION) {
        ...
        //compress point data (if applicable)
        case 7: {
            if ((status = mangle(tk)) != TK_Normal)
                return status;
            if (m_compression_scheme != CS_NONE) {
                //reorganize 2d data into (for example) xyxyxyxyz 
    ...
    return tk.Error (stream_readonly);
#endif
}  //end function TK_PolyPolypoint::Write
...
} //end function TK_PolyPolypoint::Read
...
TK_Status TK_PolyPolypoint::SetLengths (int primitive_count, int const *lengths) alter {
    ...
    if (m_primitive_count > m_lengths_allocated) {
        delete [] m_lengths;
        m_lengths = null;
        m_lengths_allocated = m_primitive_count + 16;  //add a small pad to reduce chances of reallocation
        m_lengths = new int [m_lengths_allocated];
        if (m_lengths == null)
            return TK_Error;
    }
...
void TK_PolyPolypoint::Reset () alter {
    m_point_count = 0;
    m_primitive_count = 0;
    m_subop = 0;
