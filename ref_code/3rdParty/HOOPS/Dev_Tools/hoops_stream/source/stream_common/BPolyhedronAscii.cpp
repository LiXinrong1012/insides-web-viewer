...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/shared/stream_common/BPolyhedronAscii.cpp,v 1.47 2010-01-21 23:37:55 covey Exp $
//
...
static char const * opcode_list[256] = {
    ...
    "PolyPolyline        ", "-- 0x011 --         ", "External_Reference  ", "External_Ref_Unicode", //^pqrs
...
TK_Status TK_Polyhedron::write_face_regions_ascii (BStreamFileToolkit & tk) alter {
	...
	case 0:
		...
		case 5: {
			if ((status = PutEndXMLTag(tk,"Face_Regions")) != TK_Normal)
				return status;
			mp_substage = 0;
				}   break;
	...
	default:{
		char buffer[1024] = {""};
		sprintf(buffer, "internal error in TK_Polyhedron::write_face_regions, %d", mp_substage);
...
TK_Status TK_Polyhedron::write_face_visibilities_ascii (BStreamFileToolkit & tk) alter {
    ...
    if (mp_fvisibilitycount == mp_facecount) {
        switch (mp_substage) {
			...
			case 3: {
				char * expanded_fvisibilities = new char[mp_fvisibilitycount * 4 +3];
				strcpy(expanded_fvisibilities, "");
				char * output = expanded_fvisibilities;
				char * input = mp_fvisibilities;
				int count = mp_fvisibilitycount;
				...
				int expanded_fvisibilities_count = (int)strlen(expanded_fvisibilities);

 				PutTab t(&tk);
				status = PutAsciiData(tk,"Visibilities", expanded_fvisibilities, expanded_fvisibilities_count);
				delete [] expanded_fvisibilities;
...
TK_Status TK_Polyhedron::write_edge_visibilities_ascii (BStreamFileToolkit & tk) alter {
    ...
    if (mp_evisibilitycount == mp_edgecount) {
        switch (mp_substage) {
            ...
            //all visibilities
            case 3: {
				char * expanded_evisibilities = new char[mp_evisibilitycount * 4 +3];
				strcpy(expanded_evisibilities, "");
				char * output = expanded_evisibilities;
				char * input = mp_evisibilities;
				int count = mp_evisibilitycount;
				...
				int expanded_evisibilities_count = (int)strlen(expanded_evisibilities);

 				PutTab t(&tk);
				status = PutAsciiData(tk,"Visibilities", expanded_evisibilities, expanded_evisibilities_count);
				delete [] expanded_evisibilities;
        ...
        switch (mp_substage) {
            ...
            case 4: {
                ...
                while (mp_progress < mp_edgecount) {
                    if (mp_edge_exists[mp_progress] & Edge_Visibility) {
						char progress[100] = {""};
						sprintf(progress, "%d", mp_progress);
						int progress_len = (int)strlen(progress);

                        if ((status = PutAsciiData(tk,"Progress", progress, progress_len)) != TK_Normal)
                            return status;
                    }
                    mp_progress++;
                }
            ...
            case 5: {
                ...
                while (mp_progress < mp_edgecount) {
                    if (mp_edge_exists[mp_progress] & Edge_Visibility) {
						if(mp_evisibilities[mp_progress]){
							status = PutAsciiData(tk,"Visibilities", "on", 2);
						} else {
							status = PutAsciiData(tk,"Visibilities", "off", 3);
						}
                        if (status != TK_Normal)
                            return status;
                    }
