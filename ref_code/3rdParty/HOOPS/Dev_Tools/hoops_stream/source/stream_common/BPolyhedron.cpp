...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/shared/stream_common/BPolyhedron.cpp,v 1.135 2010-11-19 21:58:34 covey Exp $
//
...
TK_Status TK_Polyhedron::SetVertexMarkerRGBAs(float const *colors) {
    int i;
    if (mp_vmrgbas == null) {
        mp_vmrgbas = new float [4*mp_pointcount];
    }
    if (mp_vmrgbas == null)
        return TK_Error;
    if (colors != null) {
        ...
        for (i = 0; i < mp_pointcount; i++)
            mp_exists[i] |= Vertex_Marker_Color;
        memcpy(mp_vmrgbas, colors, 4 * mp_pointcount * sizeof(float));
...
TK_Status TK_Polyhedron::write_vertex_colors_all (BStreamFileToolkit & tk, unsigned char op) alter {
    ...
    switch (op) {
        ...
        case OPT_ALL_VMCOLORS:
            ...
            break;
        case OPT_ALL_VMRGBAS:
            array = mp_vmrgbas;
            mask = Vertex_Marker_RGBA;
            break;
        default:
        ...
        switch (mp_substage) {
            ...
            case 3: {
                status = quantize_and_pack_floats(
                                tk, 
                                mp_pointcount, 
								(op == OPT_ALL_VMRGBAS) ? 4 : 3, 
                                array,      
								(op == OPT_ALL_VMRGBAS) ? rgba_cube : color_cube, 
                                mp_exists, 
                                mask, 
                                mp_bits_per_sample,                      
                                null, 
...
TK_Status TK_Polyhedron::write_vertex_colors (BStreamFileToolkit & tk, unsigned char op) alter {
    ...
    switch (op) {
        ...
        case OPT_VERTEX_MCOLORS:
            ...
            mask = Vertex_Marker_Color;
            break;
        case OPT_VERTEX_MRGBAS:
            count = mp_vmrgbacount;
            array = mp_vmrgbas;
            mask = Vertex_Marker_RGBA;
            break;
        default:
        ...
        switch (mp_substage) {
            ...
            case 5: {
                ...
                status = quantize_and_pack_floats(
                                tk, 
                                mp_pointcount, 
                                (op == OPT_VERTEX_MRGBAS) ? 4 : 3, 
                                array, 
                                (op == OPT_VERTEX_MRGBAS) ? rgba_cube : color_cube, 
                                mp_exists, 
                                mask, 
                                mp_bits_per_sample,                  
                                null, 
...
TK_Status TK_Polyhedron::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (mp_stage) {
        ...
        case 23: {
            if (tk.GetTargetVersion() >= 1755 &&
                mp_vmrgbacount > 0) {
                if ((status = write_vertex_colors_main(tk, OPT_ALL_VMRGBAS)) != TK_Normal)
                    return status;
            }
            mp_stage++;
        }   nobreak;
...
TK_Status TK_Polyhedron::read_vertex_normals_compressed_all (BStreamFileToolkit & tk) alter {
    ...
    switch (mp_substage) {
        ...
        case 2: {
			...
			if (mp_workspace_used > 12 * mp_pointcount)
				return tk.Error ("Bad compressed normal buffer size");
    ...
    if (mp_optopcode == OPT_PARAMETERS) {
        ...
        switch (mp_substage) {
            ...
            case 8: {
                ...
                while (i < mp_pointcount) {
					if (mp_exists[i] & Vertex_Parameter) {
...
TK_Status TK_Polyhedron::read_vertex_colors_all(BStreamFileToolkit & tk,
    ...
    if (tk.GetVersion() >= 650) {
        switch (mp_substage) {
            ...
            //process the workspace
            case 4: {
                status = unquantize_and_unpack_floats (tk, mp_pointcount,
													   (op == OPT_ALL_VMRGBAS) ? 4 : 3,
													   mp_bits_per_sample, 
													   (op == OPT_ALL_VMRGBAS) ? rgba_cube : color_cube,
													   mp_workspace, &array);
        ...
        switch (mp_substage) {
            ...
            case 3: {
                ...
                switch (op) {
                    ...
                    case OPT_VERTEX_MRGBAS:     count = mp_vmrgbacount;     break;
                }
            ...
            case 6: {
                ...
                status = unquantize_and_unpack_floats (tk, count,
													   (op == OPT_VERTEX_MRGBAS) ? 4 : 3,
													   mp_bits_per_sample, 
													   (op == OPT_VERTEX_MRGBAS) ? rgba_cube : color_cube,
													   mp_workspace, &temparray);
                if (status != TK_Normal)
                    return status;
                ...
                switch (op) {
                    ...
                    case OPT_VERTEX_MCOLORS:
                        ...
                        break;
                    case OPT_VERTEX_MRGBAS:
                        ENSURE_ALLOCATED(mp_vmrgbas, float, 4*mp_pointcount);
                        array = mp_vmrgbas;
                        break;
                    default:
				...
				int stride = (op == OPT_VERTEX_MRGBAS) ? 4 : 3;
                i = 0;
                j = 0;
                while (i < mp_pointcount) {
					if (mp_exists[i] & mask) {
						for (int k = 0; k < stride; k++)
							array[i*stride + k] = temparray[j*stride + k];
						j++;
					}
                    i++;
                }
...
TK_Status TK_Polyhedron::mangle_point_attributes(BStreamFileToolkit & tk, int pointmap_count, int *pointmap) {
    ...
    //RGBA Colors applied to markers
    if (HasVertexMarkerRGBAs()) {
        temparray = new float [pointmap_count * 4];
        if (!temparray) {
            return tk.Error("Out of Memory in TK_Polyhedron::mangle_point_attributes(4)");
        }
        for (i = 0; i < pointmap_count; i++) {
            ...
            temparray[i*3 + 1] = mp_vmcolors[index*3 + 1]; 
            temparray[i*3 + 2] = mp_vmcolors[index*3 + 2]; 
            temparray[i*3 + 3] = mp_vmcolors[index*3 + 3]; 
        }
        delete [] mp_vmrgbas;
        if (mp_vmrgbacount != pointmap_count)
            recount = true;
        mp_vmrgbas = temparray;
    }
                ...
                if (mp_exists[index] != Vertex_None) {
                    ...
                    if (mp_exists[index] & Vertex_Any_Color) {
                        ...
                        if (mp_exists[index] & Vertex_Marker_Color) 
                            mp_vmcolorcount++;
                        if (mp_exists[index] & Vertex_Marker_RGBA) 
                            mp_vmrgbacount++;
