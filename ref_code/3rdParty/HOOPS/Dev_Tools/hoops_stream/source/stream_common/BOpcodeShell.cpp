...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/shared/stream_common/BOpcodeShell.cpp,v 1.182 2010-01-21 23:37:55 covey Exp $
//
...
//public constructor.  pointcount and flistlen default to 0
TK_Shell::TK_Shell ()
...
TK_Status TK_Shell::EnumerateEdges () alter 
{
    ...
    enumcount = 0; 
    mp_edge_enumeration = new int[ m_flistlen * 4 ]; //upper bound;
    if (mp_subop & TKSH_TRISTRIPS) {
        while (fptr < end) {
            if (fptr[0] > 0) {
                len = fptr[0]; //triangle strip
                for (i = 3; i <= len; i++) {
        ...
        while (fptr < end) {
            if (fptr[0] > 0)
                len = fptr[0]; //polygonal face
            else
                len = -fptr[0]; //hole in previous face
            for (i = 1; i <= len; i++) {
                if (i == 1)
                    mp_edge_enumeration[ enumcount++ ] = fptr[len];
                else
    ...
    //flip each edge as necessary so that the lesser index comes first
    ...
    //remove duplicates and adjust the count downwards.
...
//local helper
//Here we compress a point list simply by using the appropriate type
//In the future, we can use the exact number of bits per sample that
//we need.  This compression is 100% lossless.
        ...
        unsigned char **workspace_in_out)
{
    ...
    *out++ = (unsigned char)bits_per_sample;
    ...
    return TK_Normal;
} // end function trivial_compress_faces
#endif
        ...
        int **flist_out)
{
    ...
    *flist_out = iptr;
    ...
    //HSF prior to 650 did not allow for negative numbers in the face list
...
} // end function trivial_decompress_faces
...
TK_Status TK_Shell::compute_advanced(BStreamFileToolkit & tk) alter {
    ...
    eb_compress_configs configs = {
        EB_DEFAULT, EB_DEFAULT, EB_DEFAULT, //xyz quantizations
        EB_DEFAULT, EB_DEFAULT, EB_DEFAULT, //normal quantizations
        EB_DEFAULT,                         //allowable ratio for vertices of "clean to dirty"
        null,                               //bounding 
        0,                                  //target version
        0                                   //hints
    };
    ...
    //early versions couldn't support full-resolution vertices, whereas later ones have full-rez verts
    //handled by the opcode (as opposed to having location data as part of EB data)
    if ((configs.target_version < 651) &&
        (!(mp_subop & TKSH_COMPRESSED_POINTS)))
        return TK_Abort;  
    //edgebreaker doesn't do well for very small shells.
    if (m_flistlen < 150)
        return TK_Abort;
    ASSERT(mp_compression_scheme == CS_EDGEBREAKER);
    //edgebreaker can't yet mangle to account for face attributes
    if (has_face_attributes())
        return TK_Abort;

    //set up non-default configurations
    if (mp_subop & TKSH_TRISTRIPS)
    ...
    if (configs.target_version >= 651) {
        ...
        if (flags & TK_Full_Resolution_Normals) {
            normals = null; //edgebreaker can't encode full resolution normals
        }
        else if (mp_normalcount != mp_pointcount) {
            normals = null; //edgebreaker requires that all normals be defined
        }
        #ifndef BSTREAM_DISABLE_REPULSE_COMPRESSION
        else if (num_normal_bits == 8 || num_normal_bits == 10) {
            normals = null; //we're better off using repulse compression for these values
        }
        #endif
        else if ((configs.target_version < 1000) && num_normal_bits > 48) {
            normals = null; //for the moment we are using short integers
        }
        else {
        ...
        //early versions couldn't support normals (they were handled instead by the TK_Shell opcode)
        //so reader wouldn't understand normals as part of edgebreaker data
        normals = null; 
    }
	...
	if (normals != null) {
        ...
        //XXX todo: implement facemap calculation in edgebreaker
    ...
    return tk.Error (stream_readonly);
#endif
} //end function TK_Shell::compute_advanced
...
TK_Status TK_Shell::read_advanced (BStreamFileToolkit & tk) alter {
    ...
    eb_decompress_configs configs = {null};
...
TK_Status TK_Shell::read_uncompressed_faces (BStreamFileToolkit & tk) alter {
    ...
    switch (m_substage) {
        ...
        case 3: {
            ...
            m_substage = 0; // done reading faces 
        }   break;  
