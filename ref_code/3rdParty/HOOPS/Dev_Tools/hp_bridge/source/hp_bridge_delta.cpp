...
extern "C" PK_ERROR_code_t FRU_delta_write(PK_DELTA_t, unsigned int , const char *);
extern "C" PK_ERROR_code_t FRU_delta_read(PK_DELTA_t, unsigned int, char *);
...
extern "C" int FRU__delta_init( int action )
{
    ...
    case 1:
        ...
        else
        {
            ...
            i = 0;
            for ( ; i < n_deltas_g; i++ ) deltas_g[i] = 0;
            active_g = 1;
            res = 1;
        }
        break;
...
extern "C" PK_ERROR_code_t FRU_delta_open_for_write( PK_PMARK_t pmark, PK_DELTA_t *key )
{
    delta_p_t delta;
    PK_DELTA_t i;

	// GB.  Not sure if this is the right place for this
	FRU__delta_init(1);
    ...
    *key = 0;
    ...
    if (*key == 0)
    {
        ...
        i = n_deltas_g;
        for ( ; i < 2 * n_deltas_g; i++ ) deltas_g[i] = 0;
        *key = n_deltas_g + 1;
        n_deltas_g *= 2;
    }
    deltas_g[*key - 1] =
    ...
    delta->read = 0;
    delta->first_block = 0;
    delta->current_block = 0;
    delta->offset = 0;
    return PK_ERROR_no_errors;
}

extern "C" PK_ERROR_code_t FRU_delta_open_for_read( PK_DELTA_t key )
{
    delta_p_t delta;
...
extern "C" PK_ERROR_code_t FRU_delta_write( PK_DELTA_t key, unsigned int n_bytes, const char *bytes)
{
    long n_copy;
    delta_p_t delta;
...
extern "C" PK_ERROR_code_t FRU_delta_read( PK_DELTA_t key, unsigned int n_bytes, char *bytes)
{
    int n_copy;
    delta_p_t delta;
    unsigned int offset;
    ...
    for ( ; n_bytes > offset; offset += n_copy )
    {
        ...
        n_copy = ((n_bytes - offset) > (unsigned int)(block_size - delta->offset)
...
extern "C" PK_ERROR_code_t FRU_delta_delete( PK_DELTA_t key )
{
    delta_p_t delta;
...
extern "C" PK_ERROR_code_t FRU_delta_close( PK_DELTA_t key )
{
    delta_p_t delta;
    ...
    if (((delta->read && delta->first_block) != 0) &&
        ((delta->current_block->next != 0) || 
         (delta->current_block->used != (unsigned int)delta->offset)))
    {
