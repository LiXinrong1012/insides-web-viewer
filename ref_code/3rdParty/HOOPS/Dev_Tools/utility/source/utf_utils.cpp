...
int
strcasecmp (char const *string1, char const *string2)
{
	char c1, c2;
...
int
wcscasecmp (__wchar_t const *string1, __wchar_t const *string2)
{
	__wchar_t c1, c2;
...
unicode_to_utf32 (utf32_char * utf32, T iter)
{
	int code_point = 0;
	do {
		code_point = *iter++;
...
H_UTF32::H_UTF32 (H_UTF32 const &rhs):
m_utf32 (0),
m_size (0)
{
	if (rhs.encodedText ()) {
...
H_UTF32 const &
H_UTF32::operator = (H_UTF32 const &rhs)
{
	delete[]m_utf32;
	m_utf32 = 0;
	m_size = 0;
...
H_UTF32::H_UTF32 (wchar_t const *text):m_utf32 (0), m_size (0)
{
	if (text) {
...
#ifdef _MSC_VER
H_UTF32::H_UTF32 (unsigned short const *text):
m_utf32 (0),
m_size (0)
{
	if (text) {
...
H_UTF32::H_UTF32 (char const *text):
m_utf32 (0),
m_size (0)
{
	if (text) {
...
H_UTF32::H_UTF32 (H_UTF16 const &unicode_text):
m_utf32 (0),
m_size (0)
{
...
H_UTF32::H_UTF32 (H_UTF8 const &unicode_text):
m_utf32 (0),
m_size (0)
{
	if (unicode_text.encodedText ()) {
...
H_UTF32::H_UTF32 (H_URI const &unicode_text):
m_utf32 (0),
m_size (0)
{
	if (unicode_text.encodedText ()) {
...
H_UTF32 & H_UTF32::format (unsigned short const *fmt,...)
{
	...
	va_start (va[1], fmt);
	wchar_t *
		wcs = _va_format_wcs ((wchar_t const *) fmt, va);
	va_end (va[0]);
	va_end (va[1]);
...
H_UTF32 & H_UTF32::insert (int loc, __wchar_t const * in_txt)
{
	H_UTF32 const tmp = *this;
	H_UTF32 const txt(in_txt);

	size_t const my_len = length();
	size_t const txt_len = txt.length();
	length<void*>(my_len + txt_len +1);

	if(loc > (int)my_len){
		loc = (int)my_len;
	}

	utf32_char * insert_here = m_utf32 + loc;
	utf32_char * trailing_ptr = insert_here + txt_len;
	utf32_char const * trailing_txt = tmp.m_utf32 + loc;

	memcpy(m_utf32, tmp.m_utf32, sizeof(utf32_char) * loc);
	memcpy(insert_here, txt.m_utf32, sizeof(utf32_char) * txt_len);
	memcpy(trailing_ptr, trailing_txt, sizeof(utf32_char) * (my_len - loc +1));

	return *this;
}

#ifdef _MSC_VER
H_UTF32 & H_UTF32::insert (int loc, unsigned short const *txt)
{
	return insert (loc, (wchar_t const *) txt);
}
#endif

H_UTF32 & H_UTF32::insert (int loc, char const *txt)
{
	return insert (loc, H_WCS(txt).encodedText());
}

H_UTF32 & H_UTF32::remove (int loc, int count)
{
	int const my_len = (int)length();
	...
	if(count > 0){
		H_UTF32 const tmp = *this;
		utf32_char * remove_here = m_utf32 + loc;
		utf32_char const * trailing_ptr = tmp.m_utf32 + loc + count;

		memcpy(remove_here, trailing_ptr, sizeof(utf32_char) * (my_len - loc - count +1));
	}

	return *this;
}

H_UTF32 & H_UTF32::flush()
{
	trunc(0);
	return *this;
}

H_UTF32 & H_UTF32::trunc (int loc)
{
	remove(loc, INT_MAX);
	return *this;
}

H_UTF32 & H_UTF32::replace (int loc, __wchar_t const *txt)
{
	remove(loc, (int)wcslen(txt));
	return insert(loc, txt);
}

#ifdef _MSC_VER
H_UTF32 & H_UTF32::replace (int loc, unsigned short const *txt)
{
	return replace (loc, (wchar_t const *) txt);
}
#endif

H_UTF32 & H_UTF32::replace (int loc, char const *txt)
{
	return replace (loc, H_WCS(txt).encodedText());
}

int H_UTF32::find (__wchar_t const *in_txt) const
{
	H_UTF32 txt(in_txt);
	int const txt_len = (int)txt.length();
	int const possible_beginnings = (int)length() - txt_len;
	for(int i = 0; i <= possible_beginnings; ++i){
		if(memcmp(&m_utf32[i], txt.encodedText(), txt_len * sizeof(utf32_char)) == 0)
			return i;
	}
	return -1;
}
...
#ifdef _MSC_VER
int H_UTF32::find (unsigned short const *txt) const
{
	return find ((wchar_t const *) txt);
}
#endif

int H_UTF32::find (char const *txt) const
{
	return find (H_WCS(txt).encodedText());
}

int
H_UTF32::iterator::get_and_advance (utf32_char const *&text) const
{
	int
		code_point = *text++;
	return code_point;
...
int
H_UTF32::iterator::operator * () const
{
	utf32_char const *
		tmp = m_text;
	int
...
H_UTF32::iterator H_UTF32::begin () const
{
	return iterator (m_utf32);
}

H_UTF32::iterator H_UTF32::end () const
{
	return iterator (m_utf32 + length () + 1);
}

bool
H_UTF32::operator == (H_UTF32 const &rhs) const
{
...
template < typename T > void
unicode_to_utf16 (utf16_char * utf16, T iter)
{
	int code_point = 0;
	do {
		code_point = *iter++;
...
H_UTF16 const &
H_UTF16::operator = (H_UTF16 const &rhs)
{
	delete[]m_utf16;
	m_utf16 = 0;
	m_size = 0;
...
H_UTF16::H_UTF16 (wchar_t const *text):m_utf16 (0), m_size (0)
{
	if (text) {
		if (sizeof (wchar_t) == sizeof (utf16_char)) {
			iterator
			iter1 ((utf16_char const *) text);
			m_size = utf16_char_count (iter1);
			m_utf16 = new utf16_char[m_size];
			iterator
			iter2 ((utf16_char const *) text);
			unicode_to_utf16 (m_utf16, iter2);
		}
		else {
			H_WCS
				u;
...
#ifdef _MSC_VER
H_UTF16::H_UTF16 (unsigned short const *text):
m_utf16 (0),
m_size (0)
{
	if (text) {
		if (sizeof (wchar_t) == sizeof (utf16_char)) {
			iterator iter1 ((wchar_t const *) text);
			m_size = utf16_char_count (iter1);
			m_utf16 = new utf16_char[m_size];
			iterator iter2 ((wchar_t const *) text);
			unicode_to_utf16 (m_utf16, iter2);
		}
		else {
			H_WCS u;
...
H_UTF16::H_UTF16 (char const *text):
m_utf16 (0),
m_size (0)
{
	if (text) {
...
H_UTF16::H_UTF16 (H_UTF32 const &unicode_text):
m_utf16 (0),
m_size (0)
{
	if (unicode_text.encodedText ()) {
...
H_UTF16::H_UTF16 (H_UTF8 const &unicode_text):
m_utf16 (0),
m_size (0)
{
	if (unicode_text.encodedText ()) {
...
H_UTF16::H_UTF16 (H_URI const &unicode_text):
m_utf16 (0),
m_size (0)
{
	if (unicode_text.encodedText ()) {
...
H_UTF16 & H_UTF16::insert (int loc, __wchar_t const * in_txt)
{
	H_UTF32 tmp = *this;
	*this = tmp.insert(loc, in_txt);
	return *this;
}

#ifdef _MSC_VER
H_UTF16 & H_UTF16::insert (int loc, unsigned short const *txt)
{
	return insert (loc, (wchar_t const *) txt);
}
#endif

H_UTF16 & H_UTF16::insert (int loc, char const *txt)
{
	return insert (loc, H_WCS(txt).encodedText());
}

H_UTF16 & H_UTF16::remove (int loc, int count)
{
	H_UTF32 tmp = *this;
	*this = tmp.remove(loc, count);
	return *this;

}

H_UTF16 & H_UTF16::flush()
{
	trunc(0);
	return *this;
}

H_UTF16 & H_UTF16::trunc (int loc)
{
	remove(loc, INT_MAX);
	return *this;
}

H_UTF16 & H_UTF16::replace (int loc, __wchar_t const *txt)
{
	remove(loc, (int)wcslen(txt));
	return insert(loc, txt);
}
...
#ifdef _MSC_VER
H_UTF16 & H_UTF16::replace (int loc, unsigned short const *txt)
{
	return replace (loc, (wchar_t const *) txt);
}
#endif

H_UTF16 & H_UTF16::replace (int loc, char const *txt)
{
	return replace (loc, H_WCS(txt).encodedText());
}

int H_UTF16::find (__wchar_t const *in_txt) const
{
	H_UTF32 const txt(*this);
	return txt.find(in_txt);
}

#ifdef _MSC_VER
int H_UTF16::find (unsigned short const *txt) const
{
	return find ((wchar_t const *) txt);
}
#endif

int H_UTF16::find (char const *txt) const
{
	return find (H_WCS(txt).encodedText());
}

int
H_UTF16::iterator::get_and_advance (utf16_char const *&text) const
{
	int
		w1 = *text++;
...
int
H_UTF16::iterator::operator * () const
{
	utf16_char const *
		tmp = m_text;
...
H_UTF16::iterator H_UTF16::begin () const
{
	return iterator (m_utf16);
}

H_UTF16::iterator H_UTF16::end () const
{
	return iterator (m_utf16 + length () + 1);
}

bool
H_UTF16::operator == (H_UTF16 const &rhs) const
{
	if (length () != rhs.length ())
		return false;
...
template < typename T > void
unicode_to_utf8 (utf8_char * utf8, T iter)
{
	int code_point = 0;
	do {
		code_point = *iter++;
...
H_UTF8 const &
H_UTF8::operator = (H_UTF8 const &rhs)
{
	delete[]m_utf8;
	m_utf8 = 0;
	m_size = 0;
...
#ifdef _MSC_VER
H_UTF8::H_UTF8 (unsigned short const *text):
m_utf8 (0),
m_size (0)
{
	if (text) {
...
H_UTF8::H_UTF8 (H_UTF32 const &unicode_text):
m_utf8 (0),
m_size (0)
{
	if (unicode_text.encodedText ()) {
...
H_UTF8::H_UTF8 (H_UTF16 const &unicode_text):
m_utf8 (0),
m_size (0)
{
	if (unicode_text.encodedText ()) {
...
H_UTF8::H_UTF8 (H_URI const &unicode_text):
m_utf8 (0),
m_size (0)
{
	if (unicode_text.encodedText ()) {
...
H_UTF8 & H_UTF8::insert (int loc, __wchar_t const * in_txt)
{
	H_UTF32 tmp = *this;
	*this = tmp.insert(loc, in_txt);
	return *this;
...
#ifdef _MSC_VER
H_UTF8 & H_UTF8::insert (int loc, unsigned short const *txt)
{
	return insert (loc, (wchar_t const *) txt);
}
#endif

H_UTF8 & H_UTF8::insert (int loc, char const *txt)
{
	return insert (loc, H_WCS(txt).encodedText());
}

H_UTF8 & H_UTF8::remove (int loc, int count)
{
	H_UTF32 tmp = *this;
	*this = tmp.remove(loc, count);
	return *this;
...
H_UTF8 & H_UTF8::trunc (int loc)
{
	remove(loc, INT_MAX);
	return *this;
}

H_UTF8 & H_UTF8::replace (int loc, __wchar_t const *txt)
{
	remove(loc, (int)wcslen(txt));
	return insert(loc, txt);
}
...
#ifdef _MSC_VER
H_UTF8 & H_UTF8::replace (int loc, unsigned short const *txt)
{
	return replace (loc, (wchar_t const *) txt);
}
#endif

H_UTF8 & H_UTF8::replace (int loc, char const *txt)
{
	return replace (loc, H_WCS(txt).encodedText());
}

int H_UTF8::find (__wchar_t const *in_txt) const
{
	H_UTF32 const txt(*this);
	return txt.find(in_txt);
}
...
#ifdef _MSC_VER
int H_UTF8::find (unsigned short const *txt) const
{
	return find ((wchar_t const *) txt);
}
#endif

int H_UTF8::find (char const *txt) const
{
	return find (H_WCS(txt).encodedText());
}

int
H_UTF8::iterator::get_and_advance (utf8_char const *&text) const
{
	int
		w1 = *text++;
	if (w1 < 0x7f) {
...
int
H_UTF8::iterator::operator * () const
{
	utf8_char const *
		tmp = m_text;
	return get_and_advance (tmp);
...
H_UTF8::iterator H_UTF8::begin () const
{
	return iterator (m_utf8);
}

H_UTF8::iterator H_UTF8::end () const
{
	return iterator (m_utf8 + length () + 1);
}

bool
H_UTF8::operator == (H_UTF8 const &rhs) const
{
	if (length () != rhs.length ())
		return false;
...
bool
H_URI::unreserved_char (int const c)
{
	/* We don't percent encode any of these. */
	return ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9') || (c >= '0' && c <= '9') || (c == '-') || (c == '_') || (c == '.') ||
		/* (c == '~' ) || The ~ shouldn't be percent encoded, but HOOPS doesn't allow ~ in segment names... */
		(c == 0));
}

template < typename T > int
uri_char_count (T iter)
{
	int code_point = 0;
	int uri_chars_needed = 0, count, b[4];
	do {
		code_point = *iter++;
		if (code_point >= 0 && code_point <= 0x7f) {
			count = 1;
			b[0] = (utf8_char) code_point;
		}
		else if (code_point >= 0x80 && code_point <= 0x7ff) {
			count = 2;
...
template < typename T > void
unicode_to_uri (utf8_char * uri, T iter)
{
...
H_URI const &
H_URI::operator = (H_URI const &rhs)
{
	delete[]m_uri;
	m_uri = 0;
	m_size = 0;
...
#ifdef _MSC_VER
H_URI::H_URI (unsigned short const *text):
m_uri (0),
m_size (0)
{
	if (text) {
...
H_URI::H_URI (H_UTF32 const &unicode_text):
m_uri (0),
m_size (0)
{
	if (unicode_text.encodedText ()) {
...
H_URI::H_URI (H_UTF16 const &unicode_text):
m_uri (0),
m_size (0)
{
	if (unicode_text.encodedText ()) {
...
H_URI::H_URI (H_UTF8 const &unicode_text):
m_uri (0),
m_size (0)
{
	if (unicode_text.encodedText ()) {
...
H_URI & H_URI::insert (int loc, __wchar_t const * in_txt)
{
	H_UTF32 tmp = *this;
	*this = tmp.insert(loc, in_txt);
	return *this;
...
#ifdef _MSC_VER
H_URI & H_URI::insert (int loc, unsigned short const *txt)
{
	return insert (loc, (wchar_t const *) txt);
}
#endif

H_URI & H_URI::insert (int loc, char const *txt)
{
	return insert (loc, H_WCS(txt).encodedText());
}

H_URI & H_URI::remove (int loc, int count)
{
	H_UTF32 tmp = *this;
	*this = tmp.remove(loc, count);
	return *this;
...
H_URI & H_URI::trunc (int loc)
{
	remove(loc, INT_MAX);
	return *this;
}

H_URI & H_URI::replace (int loc, __wchar_t const *txt)
{
	remove(loc, (int)wcslen(txt));
	return insert(loc, txt);
}
...
#ifdef _MSC_VER
H_URI & H_URI::replace (int loc, unsigned short const *txt)
{
	return replace (loc, (wchar_t const *) txt);
}
#endif

H_URI & H_URI::replace (int loc, char const *txt)
{
	return replace (loc, H_WCS(txt).encodedText());
}

int H_URI::find (__wchar_t const *in_txt) const
{
	H_UTF32 const txt(*this);
	return txt.find(in_txt);
}
...
#ifdef _MSC_VER
int H_URI::find (unsigned short const *txt) const
{
	return find ((wchar_t const *) txt);
}
#endif

int H_URI::find (char const *txt) const
{
	return find (H_WCS(txt).encodedText());
}

int
H_URI::iterator::get_and_advance (utf8_char const *&text) const
{
	int
		w1 = *text++;
	if (w1 != '%') {
...
H_URI::iterator H_URI::begin () const
{
	return iterator (m_uri);
}

H_URI::iterator H_URI::end () const
{
	return iterator (m_uri + length () + 1);
}

bool
H_URI::operator == (H_URI const &rhs) const
{
	if (length () != rhs.length ())
		return false;
...
static DATA_TYPE
get_conversion_type (wchar_t const *code)
{
	if (code[1] == L'%') {
		return TYPE_PERCENT;
	}
...
static void
get_conversion_code (wchar_t * code, wchar_t const *buffer)
{
	while (*buffer == L'%')
		*code++ = *buffer++;
	...
	while ((*buffer >= L'A' && *buffer <= L'Z') || (*buffer >= L'a' && *buffer <= L'z'))
		*code++ = *buffer++;
...
static wchar_t *
_va_format_wcs (wchar_t const *const wformat, va_list va[])
{
	wchar_t *retval = 0;
	for (int i = 0; i < 2; ++i) {
		size_t count = 0;
		...
		while (*tmp != L'\0') {
			if (*tmp == L'%') {
				...
				case TYPE_DOUBLE:{
						...
						H_DEFAULT_LOCALE (swprintf (buffer, 50, conversion_code, arg_float));
						if (retval) {
							wcscpy (&retval[count], buffer);
						}
						count += wcslen (buffer);
...
static char *
_va_format_mbs (char const *const format, va_list va[])
{
...
HUtilityUnicodeStr::HUtilityUnicodeStr (const wchar_t * p16):
m_wcs (p16)
{
}

HUtilityUnicodeStr::operator  const wchar_t *(void)
     const
     {
	     return
		     m_wcs.
...
HUtilityUnicodeStr::HUtilityUnicodeStr (const char *p8):
m_wcs (p8)
{
}
...
HUtilityAsciiStr::HUtilityAsciiStr (const char *p8):
m_sz (0)
{
	if (p8) {
...
HUtilityAsciiStr::operator  const char *(void)
     const
     {
	     return
		     m_sz;
...
#ifdef _MSC_VER
HUtilityAsciiStr::HUtilityAsciiStr (const unsigned short *p16):
m_sz (0)
{
	if (p16) {
		size_t size = 0;
		H_DEFAULT_LOCALE (size = wcstombs (0, (wchar_t const *) p16, 0) + 1);
		m_sz = new char[size];
		H_DEFAULT_LOCALE (wcstombs (m_sz, (wchar_t const *) p16, size));
	}
}
#endif

HUtilityAsciiStr::HUtilityAsciiStr (const wchar_t * p16):
m_sz (0)
{
	if (p16) {
		...
		size_t size = 0;
		H_DEFAULT_LOCALE (size = wcstombs (0, p16, 0) + 1);
		m_sz = new char[size];
		H_DEFAULT_LOCALE (wcstombs (m_sz, p16, size));
	}
}
...
int const
H_FORMAT_TEXT::h_vsnprintf (char *const buf, size_t const n, char const *const format, va_list valist)
{
...
H_FORMAT_TEXT::operator  char *()
     ...
     H_FORMAT_TEXT const &
     H_FORMAT_TEXT::operator= (H_FORMAT_TEXT const &rhs)
{
	free (m_str);
...
char * KeyToCharPtr::format_key() const
{
	if(!p)
	{
		sprintf(str, "0x%p", key);
		...
		if(0 == strncmp(p, "0x0x", 4))
		{
			p += 2;
		}

		if(trim && 0 == strncmp(p, "0x", 2))
		{
			p += 2;
		}
...
KeyToCharPtr::KeyToCharPtr(void * k, bool trim_0x) : key(k), p(0), trim(trim_0x)
{
	str[0] = '\0';
}

KeyToCharPtr::operator char const * () const
{
	return format_key();
}

KeyToCharPtr::operator char * () const
{
	return format_key();
}
...
{  
 return fopen((char*)H_UTF8(wcfilename).encodedText(),(char*)H_UTF8(wcmode).encodedText());
}

int wremove(wchar_t const * wcfilename) { 

 return remove((char*)H_UTF8(wcfilename).encodedText());
}

int wmkdir(wchar_t const * wcfilename, int mode) {
 return mkdir((char*)H_UTF8(wcfilename).encodedText(),mode);
}

DIR * wopendir(wchar_t const * wcfilename) {
 return opendir((char*)H_UTF8(wcfilename).encodedText());
}

#else
FILE *
wfopen (wchar_t const *wcfilename, wchar_t const *wcmode)
{
	FILE *
		retval = 0;
	if (wcfilename && wcmode) {
		...
		if (size > 0) {
			++size;	/* for the null */
			char *const
				mbfilename = (char *const) malloc (size * sizeof (char));
			if (mbfilename) {
				int
					result = 0;
...
int
wremove (wchar_t const *wcfilename)
{
	int
		retval = -1;
	if (wcfilename) {
...
int
wmkdir (wchar_t const *wcfilename, int mode)
{
	int
		retval = -1;
	if (wcfilename) {
...
DIR*
wopendir (wchar_t const *wcfilename)
{
	DIR*
		retval = 0;
	if (wcfilename) {
