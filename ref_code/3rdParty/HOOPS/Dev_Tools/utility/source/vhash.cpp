...
* manner of such use.
*
* $Id: vhash.cpp,v 1.71 2010-09-28 22:30:13 jason Exp $
*/
...
#define VMALLOC(v, p, t) do{ \
	if (v->memory_pool){ \
		Memory_Pool * mp=v->memory_pool; POOL_ALLOC_CACHED(p, t, mp); \
	} else { \
...
#define VMALLOC_ARRAY(v, p, c, t) do{ \
	if (v->memory_pool) { \
		Memory_Pool * mp=v->memory_pool;POOL_ALLOC_ARRAY_CACHED(p, c, t, mp); \
	} else { \
...
VHASH_API vhash_t * VHASH_CDECL clone_vhash(vhash_t * v)
{
	vhash_t * nv;
	
#ifdef HOOPS_DEFINED
	nv = new_vhash(v->table_size, v->memory_pool);
#else
	nv = new_vhash(v->table_size, v->vmalloc, v->vfree);
#endif

	nv->key_count = v->key_count;
	nv->unique_count = v->unique_count;
	nv->key_string = v->key_string;

	if (nv->key_count == nv->unique_count &&
		nv->key_string == 0)
		memcpy(nv->table, v->table, sizeof(vhash_node_t)*v->table_size);
	else
		ASSERT(0);
	...
	unsigned long		table_size)
{
    unsigned long		i;
	int					j;
	unsigned long		old_unique_count = v->unique_count;
	...
	new_table_size = get_power_of_2_size(table_size);

	if (v->table_size == new_table_size)
		return VHASH_STATUS_SUCCESS;

	v->table_size = new_table_size;
	...
	for (i=0; old_unique_count>0 && i<old_size; i++) {
...
{
    unsigned long			i;
	unsigned long			unique_count = v->unique_count;
