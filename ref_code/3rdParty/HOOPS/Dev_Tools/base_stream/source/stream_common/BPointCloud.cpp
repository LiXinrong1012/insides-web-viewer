...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/shared/stream_common/BPointCloud.cpp,v 1.6 2006-08-07 20:38:46 stage Exp $
//
#include "BStream.h"
...
class BKDTreeLeaf : public BKDTree {
  protected:
    int m_id;           // the id of this node in the BPointCloud
    int *m_indices;     // the indices of points in original array who map to this kd tree leaf
    int m_indices_used;
    int m_indices_allocated;

    BKDTreeLeaf() : 
        BKDTree( 'l' ) //'l' for "leaf"
    {
        m_id = -1;                  //not known yet
        m_indices_used = 0;
...
/* returns true if "pt" is inserted into a previously untouched bucket */
...
TK_Status BPointCloud::InitWrite( int resolution, const float *bbox, int original_point_count, const float *points  ) {
    ...
    m_resolution -= (resolution % 3); //force resolution to be divisible by 3
...
/* recursive private helper for writing */
...
/* recursive private helper for reading */
TK_Status BPointCloud::get( BKDTree *b, BPack *pack, int num_bits, int i, int j, int k, int span ) 
{
    ...
    if( span == 0 ) {
        ...
        if( i == m_max_val )
            pt[0] = m_bbox[3]; // avoid fp rounding error
        else
            pt[0] = m_bbox[0] + i * m_cell_size[0];
        if( j == m_max_val )
            pt[1] = m_bbox[4]; // avoid fp rounding error
        else
            pt[1] = m_bbox[1] + j * m_cell_size[1];
        if( k == m_max_val )
            pt[2] = m_bbox[5]; // avoid fp rounding error
        else
    ...
    switch( b->m_partition_type ) {
        ...
        case 'z': {
            ...
            if( count != 0 ) {
                b->m_kids[0] = new BKDTree( next_partition_type );
                b->m_kids[0]->m_point_count = count;
                get( b->m_kids[0], pack, num_bits, i, j, k, span>>1 ); //cut span in half
            }
            if( count != m_point_count ) {
                b->m_kids[1] = new BKDTree( next_partition_type );
                b->m_kids[1]->m_point_count = m_point_count - count;
                get( b->m_kids[1], pack, num_bits, i, j, k+span, span>>1 ); //cut span in half
            }
        } break;
        default:
            return TK_Error;
...
{ 
    ...
    return 
        1 +                             //1-byte compression scheme
        4 +                             //32-bit point count
        log2n * m_point_count +         //point locations (very loose upper bound)
        log2n * m_original_point_count; //point map
}
...
void BPointCloud::Generate( unsigned char *buffer, int bytes_allocated, int *bytes_used )
{
    ...
    pack.SwapBytes(); //account for big/little endian
    if( bytes_used != null )
