...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/shared/stream_common/BCompress.cpp,v 1.56 2009-02-28 03:02:42 covey Exp $
//
...
const float rgba_cube[8]   = { 0,0,0,0, 1,1,1,1 };
        ...
        case 8: {
            ...
            for (i = 0; i < end; i++) {
                if (mp_exists[i] & Vertex_Normal) {
                    ...
                    phi = (float) acos(normal[2]); //acos(z)
                    if (fabs(normal[1]) < EPS && 
                        fabs(normal[0]) < EPS)
                        theta = 0;
                    else
                        theta = (float) atan2(normal[1], normal[0] ); //atan(y/x)
