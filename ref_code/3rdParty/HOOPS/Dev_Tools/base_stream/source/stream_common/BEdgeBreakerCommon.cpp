 ...
 * manner of such use.
 *
 * $Header: /files/homes/master/cvs/hoops_master/shared/stream_common/BEdgeBreakerCommon.cpp,v 1.43 2010-08-04 23:30:05 evan Exp $
 */
...
#define alter
     ...
     * call-by-name side effects (i.e. no foo=SWAP32(i++))*/
    #define SWAP32(i) (\
            (((i) >> 24) & 0x000000ff) |\
            (((i) & 0x00ff0000) >> 8) |\
            (((i) & 0x0000ff00) << 8) |\
...
int int_stack_init( int_stack alter *s ) 
{
    ...
    s->allocated = 6;
    s->data = (int *) malloc (s->allocated * sizeof( int ));
    if( s->data == NULL ) {
...
int int_stack_expand( int_stack alter *s ) 
{
    ...
    if( s->allocated < 2 )
        ...
        s->allocated *= 2;
    temp = (int *) malloc (s->allocated * sizeof( int ));
    if( temp == NULL ) {
...
/* returns and removes some value */
...
void mtable_info_free( mtable_info alter *m )
{
    ...
    if( m->m2stackoffsets != NULL ) {
        free( m->m2stackoffsets );
        m->m2stackoffsets = NULL;
    }
    if( m->m2gateoffsets != NULL ) {
        free( m->m2gateoffsets );
        m->m2gateoffsets = NULL;
    }
    if( m->dummies != NULL ) {
        free( m->dummies );
...
/* validate_edge
 * returns nonzero if the edge looks like it maintains some basic assumptions.
 * meant for debugging
 */
...
} /* end function validate_edge */


/* validate_associations
 * Debug mode only.  associations should be either exactly equal to the #defined 
 * value reserved for GARBAGE_VERTEX, or within the range of [0,pointcount) */
...
} /* end function validate_associations */
...
int half_edge_array_init( half_edge_array alter *ea, int initial_size ) {
    ...
    ea->edges = (half_edge *) malloc (ea->allocated * sizeof( half_edge ));
    ea->used = 0;
    if( ea->edges == NULL ) {
...
int half_edge_array_expand( half_edge_array alter *ea ) {
    ...
    if( ea->allocated > 5 )
        ...
        ea->allocated += 5;
    temp = (half_edge *) malloc (ea->allocated * sizeof( half_edge ));
    if( temp == NULL ) {
    ...
    if( ea->visitations != NULL ) {
        int *itemp;
        itemp = (int *) malloc (ea->allocated * sizeof( int ) );
...
/* old_predict
 * used by both compression and decompression sides, this function makes a 
 * guess regarding the location of an unknown vertex based on the connections
 * to vertices with known locations */
int old_predict( 
    int const *associations, 
    int const *points, 
    ...
    int alter *out )
{
    ...
    /* If all three points are valid, we use the formula d = a + b - c
     * (a.k.a. complete the parallelogram).  If any of the three is not 
     * valid, however, we simply use the first valid known point that the 
     * unknown vertex is connected to as our predicted location.  If that
     * fails, we predict the origin, and fall back to using an absolute 
     * coordinate. */
    if( associations[0] >= 0 && 
        associations[1] >= 0 &&
        associations[2] >= 0 ) {
...
int lookup_vertex(const vhash_t *proxy_hash, int v) 
{
    ...
    /* we made the proxy hash, so (if we're not sanity checking ourselves), we can 
     * assume that there's something there for us */
    #ifdef DEBUG
 ...
 * used by both compression and decompression sides, this function makes a 
 * guess regarding the location of an unknown vertex based on the connections
 * to vertices with known locations.  Formula for parallelogram completion is 
 * "d=a+(b-c)".  This function should not be called for open edges, so we will
 * always have valid triangles behind us from which to do our predictions. */
void predict( 
        half_edge_array const *ea, 
        int ei,                     /* index of edge across which we are predicting */
        int third_vertex_unknown,   /* the id of the vertex whose location we predict */
        char const *touched,
        ...
        int alter *prediction_out )
{
    ...
    // keep the compiler quiet about things we're only using during sanity checking 
  LABEL(done:);
    UNREFERENCED(status);
 ...
 * matching this call with a vsfree is unnecessary (but would do no harm). 
 * pointer must be 4-byte aligned. 
 * The buffer behind pointer will be treated as const except for the need 
 * to swap bytes on BIGENDIAN (which is what keeps it from being declared const).
 */
...
void vsfree( varstream *vs )
{
    ...
    /* the varstream never owns rdata, so no need to free */
}
...
static void vsreallocate( varstream *vs )
{
    ...
    if( vs->can_reallocate ) {
        ...
        if( vs->allocated < 2 )
            ...
            vs->allocated *= 2;
        temp = (unsigned int *)malloc( vs->allocated * sizeof( unsigned int ) );
        if( temp == NULL ) {
        ...
        /* We don't own the pointer. Declare error, jettison all data to avoid a seg fault */
        vs->status = STATUS_ERROR;
        vs->used = 0;
    }
}
...
/* old_pack_points
 * 
 * used only for support of legacy formats
 * 
 * Arguments:
 *   original_pointcount: self-explanatory
 *   pointmap: the order in which the points should be sampled from "points" 
 *   points: self-explanatory
 *   pointslen_in_out: initial value indicates what was allocated.  Return 0 if the points can't
 *      fit in that amount of space.  At end of function, should be set to the actual amount used.
 *   buffer_out: where to write out the points.  It is guaranteed to be 4-byte aligned. 
        ...
        eb_compress_configs const *configs )
{
    ...
    if( configs->bounding != NULL ) {
        ...
        /* by this point we should have a valid bounding either in 
         * the configs or in the mtable */
    ...
    quantized_points = (int *) malloc ( original_pointcount * 3 * sizeof( int ) );
    ...
    if( hptr->points_scheme == 0 ) {
        ...
        /* 3 16-bit short integers for each xyz coordinate. */
        ...
        int x_quantization, int y_quantization, int z_quantization )
{
    ...
    quantized_points = (int *) malloc ( pointcount * 3 * sizeof( int ) );
    #ifdef DEBUG
    /* fill the array with garbage, so we can tell if we would have been
     * using any uninitialized data */
    ...
    if( scheme == 0 ) {
        /* typecast away the const of diffs_in, but we promise not to change it */
        ...
        /* We don't play by the rules here.  We change the diffs_in pointer even though
         * we promised not to.  We'll fix it later if someone complains. */
...
} /* end function old_unpack_points */
        ...
        int x_quantization_normals, int y_quantization_normals, int z_quantization_normals )
{
    ...
    quantized_normals = (int *) malloc ( pointcount * 3 * sizeof( int ) );
    ...
    /* We don't play by the rules here.  We change the diffs_in pointer (on big endian, at least),
     * even though we promised not to.  We'll fix it later if someone complains. */
    vsinit_read( &vs, len, (void *)diffs_in );
    vsswap( &vs );
    for( i = 0 ; i < pointcount*3 ; i += 3 )    {
        ASSERT( associations[ i + 0 ] < i );
...
} /* end function old_unpack_normals */
