 ...
 * manner of such use.
 *
 * $Header: /files/homes/master/cvs/hoops_master/shared/stream_common/BEdgeBreakerDecompress.cpp,v 1.60 2008-11-20 21:29:42 jason Exp $
 */
...
/* unpack_mtable
 * note: unpack_mtable allocates the arrays and has to be matched 
 * with an mtable_info_free 
...
int unpack_mtable( void const *src, mtable_info alter *m, vhash_t alter **proxy_hash_out )
{
    ...
    if( m->flags & MTABLE_HAS_LENGTHS ) {
        ...
        m->mlengths = (int *) malloc (m->mlengths_allocated * sizeof( int ) );
    }
    else {
    ...
    if( m->flags & MTABLE_HAS_M2STACKOFFSETS ) {
        ...
        m->m2stackoffsets = (int *) malloc (m->m2stackoffsets_allocated * sizeof( int ) );
        m->m2gateoffsets_used = m->m2stackoffsets_used;
        m->m2gateoffsets_allocated = m->m2gateoffsets_used;
        m->m2gateoffsets = (int *) malloc (m->m2gateoffsets_allocated * sizeof( int ) );
    }
    else {
    ...
    if( m->flags & MTABLE_HAS_DUMMIES ) {
        ...
        m->dummies = (int *) malloc (m->dummies_allocated * sizeof( int ) );
    }
    else {
    ...
    if( m->flags & MTABLE_HAS_PATCHES ) {
        ...
        m->patches = (int *) malloc (m->patches_allocated * sizeof( int ) );
    ...
    if( m->flags & MTABLE_HAS_BOUNDING ) {
        int temp[6];
        m->bounding = (ET_Bounding *) malloc (sizeof( ET_Bounding ) );
        temp[0] = swap32(*in++);
        temp[1] = swap32(*in++);
        temp[2] = swap32(*in++);
        temp[3] = swap32(*in++);
...
} /* end function unpack_mtable */




/* decompress_preprocess
 * In this function we a) figure out the size of the initial bounding loop, and
 * b) find the offsets (number of vertices to be skipped) for all CASE S operations */
            ...
            int alter *ecount_out, VArray<int> alter &offsets )
{
    ...
    /* estack records the length of the bounding loop at the time of CASE S operations */
    int_stack estack;
    /* sstack records id's (scount at creation time) of CASE S ops for match by CASE E */
    int_stack sstack;

    int_stack_init( &estack );
    int_stack_init( &sstack );
    /* note: shouldn't be necessary to check (i < opslen), since valid data will 
     * always end with the "E" operation that makes the stack less than 0.  Do so 
     * anyways, though, for safety */
        ...
        switch( ops[i] ) {
            ...
            case CASE_E:
                ...
                if( estack.used > 0 ) {
                    ...
                    /* early escape.  We've discovered the end of this topologically
                     * distinct piece (or, possibly, the end of the whole thing) */
                    goto done;
                break;
            case CASE_R:
                ecount += 1;
...
} /* end function decompress_preprocess */
 ...
 * We do two things here.  First is that we apply the patches to merge back 
 * any vertices that were split to resolve nonmanifolds.  Second is that we
 * remove any faces that reference one of the vertices in the dummies array */
                ...
                half_edge_array alter *ea ) 
{
    ...
    pointmap = (int *) malloc ( opcode_pointcount * sizeof( int ) );
    ...
    for( i = 0 ; i < mtable->dummies_used ; i++ ) {
        ...
        /* no double entries in the patch table */
    ...
    for( i = 0 ; i < mtable->patches_used ; i += 2 ) {
        ...
        /* dummies and patches should be mutually exclusive */
    ...
    /* remove faces that reference dummies */
    flen_temp = flen = *flen_in_out; /* copy flen for loop control variable */
    shift = 0;
    for( i = 0 ; i < flen_temp ; i += 4 ) {
        ASSERT( faces[i] == 3 );  /* assuming triangles only */
        ...
        if( pointmap[ faces[ i+1 ] ] == DUMMY ||
            ...
            /* kill the face. It references a dummy vertex. */
            shift += 4;
            flen -= 4;
        }
        else {  
    ...
    /* offset all of the vertex id's to account for the things we've removed */
    for( i = 0 ; i < flen ; i++ ) {
        /* every fourth entry is a face length */
        if( (i & 0x3) == 0 ) { 
            ASSERT( faces[ i ] == 3 );
            continue;
        }
        /* if the face references a dummy vertex, it shouldn't be here */
    ...
    /* we don't have any real vertex locations for the dummy vertices, 
     * so we can't use them to make predictions regarding locations of 
     * unknown vertices. In this loop, we replace the pointmap themselves
     * with "GARBAGE_VERTEX", and shift everything else down to account for
     * the pointmap that have been removed. If we don't have an associations array,
     * that generally means either that we are not encoding points or normals
     * within edgebreaker, or that we have a half-edge array to do the predictions */
    if( associations != NULL ) {
        /* remove entries in the associations array that predict the location of a dummy */
        shift = 0;
        for( i = 0 ; i < opcode_pointcount ; i++ ) {
            if( pointmap[ i ] == DUMMY ) {
                shift += 3;
        ...
        for( i = 0 ; i < opcode_pointcount * 3 ; i++ ) {
            /* validate associations for every 3rd entry in the associations array */
    ...
    if( ea != NULL ) {
        /* this should be the first place where we mark vertices as special.  Make
         * sure that it hasn't already happened somewhere else */
        ...
        for( i = 0 ; i < ea->used ; i++ ) {
            if( pointmap[ ea->edges[i].start ] == ALIAS ) {
                ...
                /* if the edge starts with a dummy, kill the whole triangle */
                int next, prev;
                next = HNEXT(i);
                prev = HPREV(i);
                ...
                //skip past the current triangle
                while( next == i+1 ) {
                    i++; 
                    next = HNEXT(i);
                }
...
int old_decompress_loop( half_edge_array alter *ea, int ecount, int vbase, int *N, int *P )
{
    ...
    ebase = ea->used;
    /* create the edges (vertex indices are relative to vbase)*/
    for( i = 0 ; i < ecount ; i++ ) {
    ...
    /* create the last edge, and connect it to the first */
    hi = ecount - 1;
    h = ea->edges + hi;
    h->start = hi + vbase;
    P[hi] = ebase + ecount - 2;
...
} /* end function old_decompress_loop*/
...
/* old_process_opcodes
 * "ea" is a local variable here that is used only to track the boundary,
 * not the overall triangle mesh.
 * "opcode_pointcount" is the number of vertices that process_opcodes thinks
 *    there are in the surface -- in other words, the point count prior to patching */
...
{
    int i, j, k;
    half_edge_array ea; /* tracks the bounding loop */
    int v = 0;          /* tracks the next vertex that we will introduce */
    half_edge *b, *h, *gate;
    ...
    int third;          /* the vertex that should be reflected for parallelogram completion */
    int_stack third_stack;
    VArray<int> offsets;
    int hash_allocated = 3 * opslen;
    int *N = NULL, *P = NULL;

    N = (int *) malloc ( hash_allocated * sizeof( int ) );
    P = (int *) malloc ( hash_allocated * sizeof( int ) );
    ...
    while( i < opslen ) {
        ...
        /* create a bounding loop of edges */
        ...
        /* now loop through the opcodes to build the face list */
        while( gate_stack.used >= 0 ) {
            /* we always introduce a triangle, and the gate is always
             * the first two indices we use */
            gi = (int)(gate - ea.edges);
            ...
            switch( ops[i++] ) {
                case CASE_C:
                    faces[3] = v; /* introduce a new vertex */
                ...
                case CASE_E:
                    ...
                    if( gate_stack.used > 0 ) {
                        ...
                        gate_stack.used = -1; /* end of the current run */
                    break;
                case CASE_R:
                    faces[3] = ea.edges[ N[N[gi]] ].start;
                    third = ea.edges[ N[gi] ].start;
                    /* short-circuit one link of the doubly-linked list */
                    N[gi] = N[N[gi]];
                    P[N[gi]] = gi;
                    break;
                case CASE_S:
                    ...
                    h->start = gate->start;
                    /* h will later need to become the gate, so push it */
                    hi = (int)(h - ea.edges);
                    ...
                    scount++;
                    break;
                case CASE_M: /* this case is obsolete and should thus never happen except with legacy data */
                    faces[3] = v;
                    ...
                    /* now merge that loop with the currently active one */
                    half_edge_array_append( &ea, &h ); 
                    gate = ea.edges + gi;
                    h->start = gate->start;
                    gate->start = v;
                    ...
                    v += loop_length;
                    break;
                case CASE_M2: /* this case is obsolete and should thus never happen except with legacy data */
                    ...
                    half_edge_array_append( &ea, &h ); 
                    gate = ea.edges + gi;
                    /* insert a new edge (next 5 lines) */
                    hi = (int)(h - ea.edges);
                    h->start = gate->start;
                    gate->start = ea.edges[N[bi]].start;   
                    P[hi] = P[gi];
                    N[P[gi]] = hi;
                    /* connct (i.e. absorb) the loop that we have encountered */
                    N[loop_end] = gi;
                    P[loop_start] = hi;
                    N[hi] = loop_start;
                    P[gi] = loop_end;
...
} /* end function old_process_opcodes */
...
int validate_loop( int gi, int const *N, int const *P )
{
    int bi = gi;
    do {
        /* next and prev should point to each other */
        if( P[N[bi]] != bi )
            return 0;
        bi = N[bi];
    } while( bi != gi );
...
{
    int i, j, k;
    int ecount;         /* tracks the next "edge" to be introduced on the bounding loop */
    int v = 0;          /* tracks the next vertex that we will introduce */
    int status = STATUS_NORMAL; /* assume success until we hear otherwise */
    VArray<int> offsets;
    int *N = NULL, *P = NULL, *start = NULL, *twin = NULL; /* together, these track the bounding loop */
    int_stack gate_stack;
    int_stack available_edges;
    int hash_allocated = 100, hash_used;
    int ei = 0;     //points to a location in the real half edge array
    int gi, bi, hi; //point to locations on the bounding loop
    int scount = 0;

    half_edge_array_init( ea, 3*opslen );
    N = (int *) malloc ( hash_allocated * sizeof( int ) );
    P = (int *) malloc ( hash_allocated * sizeof( int ) );
    start = (int *) malloc ( hash_allocated * sizeof( int ) );
    twin = (int *) malloc ( hash_allocated * sizeof( int ) );

    /* "i" is incremented by the inner loop.  
     * Inner loop iterates once per opcode.  
     * Outer loop iterates once per topologically distinct piece */
    i = 0;
    int_stack_init( &gate_stack );
    ...
    while( i < opslen ) {
        ...
        /* now loop through the opcodes to build the face list */
        while( gate_stack.used >= 0 ) {
            /* with every op, we introduce a triangle, and the gate is always
             * the first two indices we use */
            faces[0] = 3;
            faces[1] = start[gi];
            faces[2] = start[ N[gi] ];
            /* make sure that there is always one "edge" available so that we can 
             * stretch the loop if need be */
            ...
            /* gate edges, almost by definition, automatically add these things, no matter 
             * what case of triangle is coming up */
            ...
            switch( ops[i++] ) {
                ...
                case CASE_L:
                    ...
                    /* fix the doubly-linked list (short-circuit one link) */
                    start[ gi ] = start[ P[gi] ];
                ...
                case CASE_E:
                    ...
                    if( twin[ P[gi] ] != GARBAGE_EDGE )
                        ea->edges[ twin[P[gi]] ].twin = ei+2;
                    ea->edges[ ei+2 ].start = start[ P[gi]];
                    /* list shrinks to zero length.  Fetch a new one. */
                    ASSERT( N[N[N[gi]]] == gi );
                ...
                case CASE_S:
                    ...
                    scount++;
                    /* fix the doubly-linked list. (list splits in two) */
                    hi = int_stack_pop( &available_edges );
                    int_stack_push( &gate_stack, hi );
                    N[P[gi]] = hi;
                    P[hi] = P[gi];
                    ...
                    N[bi] = gi;
                    break;
                case CASE_M: /* obsolete */
                case CASE_M2: /* obsolete */
                default:
                    ASSERT(0);
            } /* end switch */

			/* XXX hack alert!  this avoids a crash but drops data.  Something went wrong on the write side. */
			if ( faces[1] == faces[2] ||
				 faces[2] == faces[3] ||
				 faces[3] == faces[1] ) {
            ...
            //unless we just finished off a piece, loop should be valid 
            if( gate_stack.used >= 0 ) {
                ASSERT( validate_loop( gi, N, P ) );
                //the following ASSERT can get quite expensive, so only do it every so often.
                ASSERT( i % 20 ||
                        loop_contains_no_freed_edges( gi, N, &available_edges ) );
            }
            ea->used += 3;
            ei += 3;
        } /* end while(gate_stack.used) -- i.e. end contiguous piece*/
        scount = 0;
    }
...
{   
    ...
    ASSERT( strip_len > 0 ); //not ready to handle trifans yet
        ...
        vhash_t const *proxy_hash )
{
    ...
    for( i = start_tristrip_id ; i < num_tristrips ; i++ ) {
        ...
        if( decode_tristrip( ei, tristrip_lengths[i], ea, proxy_hash, tsptr ) <= 0 ) {
            //the writing side is supposed to be disabling tristrips (for now) in situations
            //where mesh processing is required. 
            status = STATUS_ERROR;
            COMPLAIN( "decode_all_tristrips failed.  Falling back to triangles\n" );
            goto done;
        }
        ASSERT( tristrip_lengths[i] > 0 ); //xxx not ready for trifans yet
        tsptr += tristrip_lengths[i] + 1;
    }
  done:
    return status;
        ...
        int alter **tristrips_out )
{
    ...
    int *tristrips = NULL;
    int *tsptr = NULL; //iterates over tristrips
    int p[3];
    ...
    touched = (char *) malloc( pcount * sizeof( char ) );
    ...
    if( hptr->normals_scheme > 0 ) {
        ...
        normals = (int *) malloc( 3 * coords_count * sizeof( int ) );
        for( i = 0 ; i < coords_count ; i++ ) {
            normals[i*3+0] = vsget( &vs, bits2 );
            normals[i*3+1] = vsget( &vs, bits2 );
            normals[i*3+2] = vsget( &vs, bits2 );
        }
        ndiffs = (int *) malloc( 3 * diffs_count * sizeof( int ) );
        for( i = 0 ; i < diffs_count ; i++ ) {
    ...
    if( by_tristrips ) {
        if( tristrips_out != NULL ) {
            ...
            tristrip_starts = (int *) malloc( num_tristrips * sizeof( int ) );
            tristrip_lengths = (int *) malloc( num_tristrips * sizeof( int ) );
            ...
            tristrips = (int *) malloc( tristrip_total_length * sizeof( int ) );
            tsptr = tristrips;
    ...
    if( quantized_points_out != NULL || quantized_normals_out != NULL ) {
        c = 0;
        for( i = 0 ; i < ea->used ; i++ ) {
            /* no need for ea->visitations[i] because here on decompression side, edges are 
             * already in visitation order  */
            ei = i;
            if( ei == next_tristrip_ei ) {
                // if !by_tristrips, next_tristrip_ei will be negative, so we should never get here
                ASSERT( by_tristrips );
                if( decode_tristrip( ei, tristrip_lengths[next_tristrip_id], ea, proxy_hash, tsptr ) <= 0 ){
                    by_tristrips = false;
                    next_tristrip_ei = -1; //prevent any more matches from happening
                    //this situation was supposed to have been resolved at write time
                    COMPLAIN( "tristrip decoding failed.  Falling back to triangles\n" );
                } 
                else {
                    ASSERT( tristrip_lengths[next_tristrip_id] > 0 ); //xxx not ready for trifans yet
            ...
            for( k = 0 ; k < 3 ; k++ ) { 
                ...
                if( !touched[v] ) {
                    touched[v] = 1; 
                    /* get the current edge's start xyz and normal */
            ...
            while( regular_edges.used || discontinuous_edges.used ) {
                ...
                if( VERTEX_SPECIAL(v) ) {
                     ...
                     * (or DUMMY_VERTEX, if it's just there to plug a hole) */
                    proxyv = PROXY_VERTEX_INDEX(proxy_hash,v);
                }
                else
                    eav = proxyv = v;
        ...
        //traversal over edges is greatly simplified if all we are interested in is the tristrips
    ...
    // if we ran past the end, inform the caller that an error occurred
    ...
    if( touched )
        free( touched );
    if( tristrip_starts )
        free( tristrip_starts );
    if( tristrip_lengths )
        free( tristrip_lengths );
    if( by_tristrips ) {
...
/* show_decompress_size
 * like "HC_Show_Shell_Size", shows the user what he needs to allocate before 
 * making the call to the decompress routine.  Returned values are upper bounds */
...
} /* end function show_decompress_size */
            ...
            eb_decompress_configs const *configs )
{
    ...
    int opcode_pointcount; /* the number of points before subtracting dummies and aliases */
    ...
    /* copy the values from the start of the stream into a local struct, 
     * and swap (if BIG ENDIAN) as you go */
    ...
    if( hptr_in->scheme >= 1 )
        ...
        hptr.normalslen = 0; //prior to scheme 1, there were no normals

    if( hptr.opslen == 0 )
        goto done;

    ASSERT( hptr_in->scheme <= 2 ); /* the most recent encoding scheme this reader knows about */
    /* make sure that the user is allocating enough memory */
    ...
    /* no unpacking necessary for ops... they're already in native format */
    offset += hptr.opslen;
    ...
    if( hptr.scheme >= 2 ) {
        ea = (half_edge_array *) malloc ( sizeof( half_edge_array ) );
        ...
        associations = (int *) malloc ( (hptr.opslen*3) * 3 * sizeof( int ) );
        if( associations == NULL ) {
    ...
    flen = *flen_in_out; /* upper bound.  patch_faces() returns the real value */ 
    ...
    if( hptr.scheme >= 2 &&
        ...
        if( hptr.points_scheme > 0 ) {
            quantized_points = (int *) malloc( 3 * pcount * sizeof( int ) );
        }
        if( hptr.normals_scheme > 0 ) {
            quantized_normals = (int *) malloc( 3 * pcount * sizeof( int ) );
        }
        /* cast pdata to non-const, because BIGENDIAN systems swap32 it */
...
} /* end function edgebreaker_decompress */
