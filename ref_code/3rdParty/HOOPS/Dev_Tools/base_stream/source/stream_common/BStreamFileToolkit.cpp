...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/shared/stream_common/BStreamFileToolkit.cpp,v 1.301 2010-10-26 20:24:38 heppe Exp $
//
...
void Internal_Data_Accumulator::restart () alter {
    if (m_compressed) {
        if (m_writing)
    ...
    // not necessary to clean out data, just make sure we think we have none
    m_pending_size = 0;
    m_buffer_size = 0;
    m_generated = 0;
}
...
void Internal_Data_Accumulator::clean () alter {
    // throuw away our internal buffer
    delete [] m_pending_buffer;
...
TK_Status Internal_Data_Accumulator::start_compression () alter {
...
TK_Status Internal_Data_Accumulator::stop_compression (bool flush) alter {
    ...
    if (flush) {
        if (m_buffer_size == 0)     // relatively rare case, writing the stop caused the buffer to fill
            return TK_Pending;
        ...
        if (success != Z_OK && success != Z_DATA_ERROR) {   // may be shutting down after error
            m_compressed = false;   // assume we are done anyway
            return error ("compression termination failed");
        }
    }
...
#ifndef BSTREAM_READONLY
    if (size == 0)              // so a lot of callers don't have to check
        return TK_Normal;

    if (m_buffer_size == 0)
        return TK_Pending;      // already have an overflow, no more writes

    // Note: this write will always succeed, but may save away stuff to be consumed later
    if (m_compressed) {
        int     success;
...
TK_Status Internal_Data_Accumulator::consume () alter {
#ifndef BSTREAM_READONLY
    if (m_pending_size == 0)
        return TK_Normal;
...
TK_Status Internal_Data_Accumulator::start_decompression () alter {
    if (m_compressed)
...
TK_Status Internal_Data_Accumulator::stop_decompression (bool force) alter {
    // normally don't actually do anything here, the read sequence will detect
    // the end of the compressed stream and do the final shutoff

    //if (!m_compressed)
    //    return error ("stop_decompression called while not decompressing");
...
TK_Status Internal_Data_Accumulator::lookat (char alter & buffer) alter {
    if (m_pending_size > 0) {
        ...
        m_z_stream->avail_out = 1;      // we only want a little
        ...
        if (success == Z_STREAM_END) {
            // compressed data done, finalize shutdown
            if (inflateEnd (m_z_stream) != Z_OK)
                return error ("decompression termination failed");

            m_compressed = false;
            // no return, should have data to process
        }

        if (m_z_stream->avail_out == 0) {       // done!
            buffer = *m_pending_position;
            m_pending_size = 1;
            return TK_Normal;
        }
        else if (m_z_stream->avail_in == 0) {   // still need more
            return TK_Pending;
        }
        else if (success != Z_STREAM_END)   // stream_end cleanup might not generate data
...
TK_Status Internal_Data_Accumulator::read (char alter * buffer, int size) alter {
    ...
    if (size <= m_pending_size) {
        ...
        if (m_buffer_size == 0)     // zlib "might" generate output if no new input, but not safe
            return TK_Pending;
        ...
        if (m_pending_size != 0) {
            // if m_pending_size, we failed before and are building the result in the pending buffer
            m_z_stream->next_out = (Bytef *)m_pending_position + m_pending_size;
            m_z_stream->avail_out = size - m_pending_size;
        }
        else {
            // else assume we can read directly into the request buffer
        ...
        if (m_z_stream->avail_out == 0) {       // done!
            if (m_pending_size != 0) {
                // wasn't put in request buffer, so copy from pending buffer now
            ...
            if (m_pending_size != 0) {
                // just update the amount we have so far
                m_pending_size = (int)((char *)m_z_stream->next_out - m_pending_position);
            }
            else {
                // move anything we did get to pending buffer to try again later
                if (size > m_pending_buffer_allocated) {
                    delete [] m_pending_buffer;
                    m_pending_buffer_allocated = size + 1024;  // plus a little extra
                    m_pending_buffer = new char [m_pending_buffer_allocated];
                }
...
void Internal_Data_Accumulator::save () alter {
    int             needed = m_failed_size;
    char alter *    old_buffer = null;

    if (needed == 0)    // if we didn't fail a read, we must be done, so no data needs to be saved
        return;

    if (needed > m_pending_buffer_allocated) {
        old_buffer = m_pending_buffer;
        m_pending_buffer_allocated = needed + 1024;  // plus a little extra
        m_pending_buffer = new char [m_pending_buffer_allocated];
    }
...
Internal_ExRef_List::Internal_ExRef_List (char const * ref, ID_Key context, Internal_ExRef_List * caller)
	: m_next (null), m_caller (caller), m_actual (null), m_context (context) {
    m_ref = new wchar_t [strlen (ref) + 1];
	wcsstrcpy (m_ref, ref);
}

#ifdef _MSC_VER
Internal_ExRef_List::Internal_ExRef_List (unsigned short const * ref, ID_Key context, Internal_ExRef_List * caller)
	: m_next (null), m_caller (caller), m_actual (null), m_context (context) {
    m_ref = new wchar_t [wcslen((wchar_t const *)ref) + 1];
    wcscpy (m_ref, (wchar_t const *)ref);
}
#endif

Internal_ExRef_List::Internal_ExRef_List (wchar_t const * ref, ID_Key context, Internal_ExRef_List * caller)
	: m_next (null), m_caller (caller), m_actual (null), m_context (context) {
    m_ref = new wchar_t [wcslen(ref) + 1];
    wcscpy (m_ref, ref);
}
...
#ifdef _MSC_VER
void	Internal_ExRef_List::SetActual (unsigned short const * actual) alter {
	SetActual((wchar_t const *) actual);
}
#endif

void	Internal_ExRef_List::SetActual (wchar_t const * actual) alter {
	delete [] m_actual;
    m_actual = new wchar_t [wcslen(actual) + 1];
    wcscpy (m_actual, actual);
}
...
static float find_basis_points (int count, float const * points, int * basis) {
    ...
    /* maybe add code to adjust basis in case points happen to be coplanar */
    ...
    for (i = 0; i < limit; i++) {
        ...
        if (score < best)
            basis[swap] = save;  /* revert back to what we had */
        else
            best = score;
        swap = (swap+1) % 4;
...
bool Recorded_Instance::generate_basis (int count, float const * points) alter {
    ...
    if (count > 0) {
        ...
        // loose epsion for basis determinant
        epsilon = 0.01f * (max - min);
        epsilon *= epsilon * epsilon;   // determinant calculation basically cubes the deltas44

        // can we find a basis amongst the points
        // NOTE: this requires "volume" -- will need to adjust to deal with flat things like polygons
        if (find_basis_points (count, points, m_basis_indices) >= epsilon) {
            ...
            // construct the matrix which is the transform from the basis to identity
            // so first the simple matrix which transforms the identity to the basis...
            ...
            // we should always be able to invert since we determined a real basis set, but check anyway
            if (invert_matrix (matrix, m_local_basis)) {
                // keep one extra point around for a quick transform & test before we do all points
                i = count - 1;
    ...
    // if we couldn't generate a basis, modify this instance so we will never try it again...
    if (!m_basis_valid)
        m_opcode = 0;
...
BStreamFileToolkit::BStreamFileToolkit() {
	...
	m_log_fp = null;
	m_logging = false;
	m_logging_options = ~0U; // all options by default
	m_log_line_length = 0;
	m_opcode_sequence = 0;
	m_objects_written = 0;
	...
	SetOpcodeHandler (TKE_Named_Style_Def,		new TK_Named_Style_Def);
	...
	// Note: Stop_User_Data code should never be encountered by itself
	SetOpcodeHandler (TKE_XML,					new TK_XML);
	SetOpcodeHandler (TKE_External_Reference,	new TK_External_Reference);
	SetOpcodeHandler (TKE_External_Reference_Unicode,	new TK_External_Reference_Unicode);
	SetOpcodeHandler (TKE_URL,					new TK_URL);
	SetOpcodeHandler (TKE_Material,				new TK_Material);
	...
	SetOpcodeHandler (TKE_Named_Style,			new TK_Referenced_Segment (TKE_Named_Style));
	...
	SetOpcodeHandler (TKE_Conditional_Action,	new TK_Conditional_Action);
	...
	SetOpcodeHandler (TKE_User_Index_Data,		new TK_User_Index_Data);
	...
	SetOpcodeHandler (TKE_Line,					new TK_Line (TKE_Line));
	SetOpcodeHandler (TKE_Infinite_Line,		new TK_Line (TKE_Infinite_Line));
	SetOpcodeHandler (TKE_Infinite_Ray,			new TK_Line (TKE_Infinite_Ray));
...
TK_Status BStreamFileToolkit::LocateDictionary () alter {
    FILE *          fp = (FILE *)m_file;
...
TK_Status BStreamFileToolkit::LocateEntity (ID_Key key, int variant) alter {
    int					offset, length;
    wchar_t const *		oldfile = GetCurrentFile();
    wchar_t const *		newfile;
    TK_Status			status = TK_Normal;
    ...
    if (offset == 0)    // no specified variant value
        return TK_NotFound;

    if (newfile != null && newfile != oldfile) {    // string pointer compares ok, always internal strings
...
void BStreamFileToolkit::SetFilename (char const * name) alter {
	delete [] m_wfilename;
	m_wfilename = new wchar_t [strlen (name) + 1];
	wcsstrcpy (m_wfilename, name);
	m_current_filename = m_wfilename;
}

#ifdef _MSC_VER
void BStreamFileToolkit::SetFilename (unsigned short const * name) alter {
	SetFilename((wchar_t const *) name);
}
#endif

void BStreamFileToolkit::SetFilename (wchar_t const * name) alter {
	wchar_t *	old = m_wfilename;		// just in case old happens to be m_wfilename

	m_wfilename = new wchar_t [wcslen(name) + 1];
	wcscpy (m_wfilename, name);
	m_current_filename = m_wfilename;
...
void BStreamFileToolkit::SetNewFile (char const * name) alter {
	wchar_t *	buffer = new wchar_t [strlen (name) + 1];

	wcsstrcpy (buffer, name);
	SetNewFile (buffer);
...
#ifdef _MSC_VER
void BStreamFileToolkit::SetNewFile (unsigned short const * name) alter {
	SetNewFile((wchar_t const *) name);
}
#endif

void BStreamFileToolkit::SetNewFile (wchar_t const * name) alter {
    if (m_file_count == m_files_allocated) {
        wchar_t **	old_names = m_file_names;
        int *		old_indices = m_file_indices;

        m_files_allocated += 16;

        m_file_names = new wchar_t * [m_files_allocated];
        memset (m_file_names, 0, m_files_allocated * sizeof (wchar_t *));
        memcpy (m_file_names, old_names, m_file_count * sizeof (wchar_t *));
		delete [] old_names;
    ...
    m_file_names[m_file_count] = new wchar_t [wcslen(name) + 1];
    wcscpy (m_file_names[m_file_count], name);
...
TK_Status BStreamFileToolkit::SelectFile (char const * name) alter {
	...
	TK_Status	result = SelectFile (buffer);
...
#ifdef _MSC_VER
TK_Status BStreamFileToolkit::SelectFile (unsigned short const * name) alter {
	return SelectFile((wchar_t const *) name);
}
#endif

TK_Status BStreamFileToolkit::SelectFile (wchar_t const * name) alter {
    if (m_file_names != null) {
        int             i;

        for (i = 0; i < m_file_count; ++i) {
            if (!wcscmp (name, m_file_names[i])) {
...
TK_Status BStreamFileToolkit::OpenFile (unsigned short const * name, bool write) alter {
	return OpenFile((wchar_t const *) name, write);
}
#endif

TK_Status BStreamFileToolkit::OpenFile (wchar_t const * name, bool write) alter {
		if ((m_file = wfopen ((const wchar_t*)name, write ? L"wb" : L"rb")) == null)
			return Error ("file open failure");
		if (name != m_wfilename)
			SetFilename(name);	
		return TK_Normal;
...
TK_Status BStreamFileToolkit::CloseFile () alter {
    FILE *          fp = (FILE *)m_file;
...
TK_Status BStreamFileToolkit::GetFileSize (unsigned HLONG & size) alter {
	bool			query = false;
    FILE *          fp = (FILE *)m_file;
    if (fp == null)
...
void BStreamFileToolkit::empty_lists () alter {
    int         i;

    for (i=0; i<256; i++) {
        while (m_instance_hash[i] != null) {
            Recorded_Instance alter *       victim = m_instance_hash[i];
    ...
    while (m_active_segments != null) {
        Internal_Segment_List alter *       victim = m_active_segments;
        m_active_segments = victim->m_next;
        delete victim;
    }

    while (m_revisit != null) {
        Internal_Revisit_Item alter *       victim = m_revisit;
        m_revisit = victim->m_next;
        delete victim;
    }
    while (m_revisit_working != null) {
        Internal_Revisit_Item alter *       victim = m_revisit_working;
        m_revisit_working = victim->m_next;
        delete victim;
    }

    while (m_external_references != null) {
        Internal_ExRef_List alter *         victim = m_external_references;
        m_external_references = victim->m_next;
        delete victim;
    }
    m_external_ref_tail = null;
    while (m_previous_exrefs != null) {
        Internal_ExRef_List alter *         victim = m_previous_exrefs;
        m_previous_exrefs = victim->m_next;
        delete victim;
    }
}
...
void BStreamFileToolkit::Restart () alter {
    int         i;
...
void BStreamFileToolkit::read_completed () alter {
    // no special actions here
}

int BStreamFileToolkit::ParseVersion (char const * block)
{
	if (!strncmp (block, ";; HSF V", 8)) {
		...
		while (cp) {
			if ('0' <= *cp && *cp <= '9')
				...
				generate_error ("BStreamFileToolkit::ParseVersion", "error reading version number");
...
TK_Status BStreamFileToolkit::ParseBuffer (char const * b, int s, TK_Status mode) alter {
    ...
    forever {
        ...
        if (status == TK_Pause) {
            // disable "read simplified" if no dictionary (we would never be able to sweeten!) 
        ...
        if (current_active != m_active_segments) {      // make sure we have correct context
        ...
        if (executed == TK_Revisit)
            continue;                                   // we don't count this object in case of 'single'
...
#ifdef _MSC_VER
void BStreamFileToolkit::LogEntry (unsigned short const * string) const {
	LogEntry((wchar_t const *) string);
}
#endif

void BStreamFileToolkit::LogEntry (wchar_t const * string) const {
    wchar_t const *		cp = string;
    wchar_t				cc;
	bool				all_ascii = true;
...
void BStreamFileToolkit::CloseLogFile () alter {
    fclose (m_log_fp);
    m_log_fp = null;
}
...
ID_Key BStreamFileToolkit::RemoveSegment () alter {
...
TK_Status BStreamFileToolkit::AppendLastKey (ID_Key key) alter {
	...
	if (m_last_keys_used >= m_last_keys_allocated) {
		//we've run out of space, so expand the array
		ID_Key *temp;

		if ( m_last_keys_allocated < 4194304 )	// double until 4MB
			m_last_keys_allocated *= 2;
		else
			m_last_keys_allocated += 1048576;		// then add 1MB at a time		
...
// Note: IndexToKey operates in the scope of the current file,
// while KeyToIndex operates across all files (returning effective index)
...
// record the current object for later reprocessing
...
bool BStreamFileToolkit::add_exref (Internal_ExRef_List * exref) alter {
	Internal_ExRef_List * caller = exref->Caller();

	while (caller != null) {
		if (!wcscmp (exref->Reference(), caller->Reference())) {	// would form an infinite loop, so ignore it
			delete exref;
			return false;
		}
...
bool BStreamFileToolkit::AddExternalReference (char const * ref, ID_Key context) alter {
    return add_exref (new Internal_ExRef_List (ref, context, m_external_references));
}

#ifdef _MSC_VER
bool BStreamFileToolkit::AddExternalReference (unsigned short const * ref, ID_Key context) alter {
	return AddExternalReference((wchar_t const *) ref, context);
}
#endif

bool BStreamFileToolkit::AddExternalReference (wchar_t const * ref, ID_Key context) alter {
    return add_exref (new Internal_ExRef_List (ref, context, m_external_references));
}

bool BStreamFileToolkit::NextExternalReference () alter {
    ...
    if ((exref = m_external_references) != null) {
        ...
        exref->m_next = m_previous_exrefs;
		m_previous_exrefs = exref;
    }
...
// record the current object for possible instancing
...
// try to locate an instance matching the specified object
...
void Internal_Translator::clean () alter {
    Hash_Block *        index_block;
    int                 i;
...
TK_Status Internal_Translator::add_pair (int index, ID_Key key) alter {
    ...
    Hash_Block *            index_block = m_blocks[hash];
...
TK_Status Internal_Translator::index_to_key (int index, ID_Key alter & key) const {
    ...
    if (index >= m_size)
        return TK_NotFound;
    if (!m_pairs || 
		m_pairs[index].m_index != index)    // was this entry really filled in?
        return TK_NotFound;
...
TK_Status Internal_Translator::key_to_index (ID_Key key, int alter & index) const {
    Hash_Block *            block = m_blocks[hash_key (key)];

    if (block != null) do {
        int             i;

        for (i=0; i<block->m_used; i++) {
            int             test = block->m_indices[i];

            if (m_pairs[test].m_key == key) {
                if (m_pairs[test].m_index == test) {    // was this entry really filled in?
                    index = test;
                    return TK_Normal;
                }
            }
...
TK_Status BStreamFileToolkit::GetOffset (
    ...
    int alter &             	offset,
    int alter &             	length,
    unsigned short const * alter &		filename) const {
		return GetOffset(key, variant, offset, length, (wchar_t const * &)filename);
}
#endif
...
TK_Status BStreamFileToolkit::GetOffset (
    ...
    int alter &             	offset,
    int alter &             	length,
    wchar_t const * alter &		filename) const {
    TK_Status               	status = TK_Normal;
    int                     	index;
    int                     	i;
...
void Internal_Key_Record::clean () alter {
    Hash_Block *            block;
    int         i;
...
TK_Status Internal_Key_Record::add_key (ID_Key key) alter {
    int                     hash = hash_key (key);
    Hash_Block *            block = m_blocks[hash];
...
TK_Status BStreamFileToolkit::Read_Stream_File () {
    ...
    int                     flags;
    unsigned HLONG           total_file_size = 0;
    unsigned HLONG           processed = 0;
    TK_Progress_Callback    callback = 0;
    void *                  value = 0;
    bool                    first_file = true;
    ID_Key                  context = -1;
	wchar_t const *			wfilename = m_wfilename;
	int						version;
	...
	// set starting file to look like an initial external reference
    AddExternalReference (wfilename, context);
	wfilename = GetExternalReference ();

    do {
        SetNewFile (wfilename);

        if (GetLogging()) {
			LogEntry (wfilename);
			LogEntry ("\n");
			int length = 0;
			while (wfilename[length] != 0)
				++length;
			log_bar (this, '-', length);
			SetOpcodeSequence();
        }

		if (MatchPreviousExRef())
			status = TK_Complete;
		else {
			...
			if ((status = OpenFile (wfilename)) == TK_Normal)
				m_external_references->SetActual (wfilename);
			else {
				if (first_file)
					return status;

				// must be an exref, retry in current directory (strip path)
				wchar_t const *				bare = wcsrchr2 (wfilename, '/', '\\');

				if (bare != null)
					status = OpenFile (++bare);		// note: bare now points to the bare filename
				else
					bare = wfilename;				// was a bare filename anyway

				if (status == TK_Normal)
					m_external_references->SetActual (bare);
				else {
					// bare file not found, try it against all the paths in this exref chain
					size_t						barelen = wcslen (bare);
					Internal_ExRef_List *	caller = m_external_references->Caller();

					if (caller != null) do if (caller->Actual() != null) {
						wchar_t *			trial = new wchar_t [wcslen(caller->Actual()) + barelen];	// more than we need

						wcscpy (trial, caller->Actual());

						wchar_t *			slash = wcsrchr2 (trial, '/', '\\');

						if (slash != null) {
							wcscpy (++slash, bare);
							status = OpenFile (trial);
							if (status == TK_Normal)
								m_external_references->SetActual (trial);
						}
						// else no usable path on that one

						delete [] trial;
					} while (status != TK_Normal && (caller = caller->Caller()) != null);
				}

				if (status != TK_Normal) {
					// no general mechanism to flag an error and continue, but
					// if we have a callback registered, we can at least signal by giving a zero file size...
					if (callback) {
						if (!(*callback) (0, 0, value)) {
							status = TK_Abort;
							break;
						}
					}
			...
			if (status == TK_Normal) {
				...
				do {
					...
					if (callback) {
						...
						if (!(*callback) (processed, total_file_size, value)) {
							status = TK_Abort;
							break;
						}
					}
				...
				if (first_file)
					...
					status = TK_Complete;	// ignored ExRefs we let pass
			}
		}

        // see if we have more external references to process
		...
		first_file = false;
        if ((wfilename = GetExternalReference()) != null) {
            if (GetLogging())
                log_bar (this, '=', 80);
        }
    ...
    SetReadFlags (flags);   // restore in case we tried Ignore_Version
...
TK_Status TK_Read_Stream_File (unsigned short const * filename, BStreamFileToolkit * tk) {
	return TK_Read_Stream_File((wchar_t const *) filename, tk);
}
#endif
...
TK_Status TK_Read_Stream_File (unsigned short const * filename, int flags) {
	return TK_Read_Stream_File((wchar_t const *) filename, flags);
}
#endif

TK_Status TK_Read_Stream_File (wchar_t const * filename, int flags) {
	BStreamFileToolkit *    tk;
	TK_Status               status;
