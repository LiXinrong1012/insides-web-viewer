...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/shared/stream_common/BPolyhedron.h,v 1.81 2009-02-28 03:02:42 covey Exp $
//
...
class BBINFILETK_API TK_Polyhedron : public BBaseOpcodeHandler {
	public:
		...
		*/
		enum Exists {                   
			...
			Vertex_Marker_RGBA		= 0x0800,  //!< vertex color in rgba format, to be applied to marker
			Vertex_Any_Color    	= 0x08FC  //!< multi-bit alias to quickly test for any color or color-by-index
		};
		...
		*/
		...
		enum Opt_Opcode {
			...
			OPT_ALL_VMRGBAS					= 29, //!< set rgba colors on all vertices, to apply to drawing of vertex_markers 
			OPT_VERTEX_MRGBAS				= 30, //!< set rgba colors on some vertices, to apply to drawing of vertex_markers 
			/**/
		...
		/*! \return a pointer to the array of vertex rgba colors (as applied to the markers) */
		float const *GetVertexMarkerRGBAs() const { return mp_vmcolors; };
		/*! \return a pointer to the array of vertex rgba colors (as applied to the markers) */
		float *GetVertexMarkerRGBAs() { return mp_vmcolors; };
		...
		/*!Allocates the vertex rgba colors (as applied to markers), and fills with values and sets the flags in mp_exists.
			Allocated to a length of mp_pointcount, regardless of the number of values that are intended to be specified.
		   \param colors can be null, in which case colors are left untouched (assumed to be specified at a later
			point.  Normally, however, "colors" should refer to the rgba quads for the 
			values of the vertex colors. These values will be copied, and the appropriate bits in mp_exists will 
			be updated. 
		   \return TK_Normal or TK_Error to indicate success or failure. */
		...
		/*! \return true if and only if at least one vertex rgba color has been set (to apply to a marker) */
		bool HasVertexMarkerRGBAs() const { return mp_vmrgbas != 0; };
	...
	protected:
		...
		float *			mp_vmrgbas;    /*!< Vertex marker colors array */
		...
		int				mp_vmrgbacount;/*!< Number of explicitly set vertex marker rgba colors */
		...
		/*! Quantizes the normal vectors to one of 2^bits vectors precalculated
