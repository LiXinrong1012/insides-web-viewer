...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HMaterialLibrary.cpp,v 1.78 2011-01-19 18:34:11 david Exp $
//
		...
		void *datlist)
{
	...
	for (i=0; i<vconf_chunk_count(v); i++) {
		...
		if ((option = vconf_get_option(v, "CHANNEL"))) {
		...
		{
			if ((option = vconf_get_option(v, "SOURCE"))) {
		...
		{
			if ((option = vconf_get_option(v, "HEIGHTFIELD"))) {
		...
		if ((option = vconf_get_option(v, "MULTITEXTURE"))) {
			if (streq (option, "yes") ||
				streq (option, "on") ||
				streq (option, "true"))
		...
		if ((option = vconf_get_option(v, "PARAMETERIZATION_SOURCE"))) {
			char * tmp = new char [strlen(option)+1];
			strcpy(tmp, option);
			dat->parameterization_source = tmp;
		}

		if ((option = vconf_get_option(v, "TWEAKABLES"))) {
			char * tmp = new char [strlen(option)+1];
			strcpy(tmp, option);
			dat->tweakables = tmp;
		}

		if ((option = vconf_get_option(v, "COLOR"))) {
			char *tmp = new char [strlen(option)+1];
			strcpy(tmp, option);
			dat->color = tmp;
		}

		if ((option = vconf_get_option(v, "OPTIONS"))) {
			char *tmp = new char [strlen(option)+1];
			strcpy(tmp, option);
			dat->options = tmp;
		}
		...
		const wchar_t *filename)
{
	...
	status = tk.ParseBuffer (buffer, HSF_CHUNK);
		...
		const wchar_t *path)
{
	...
	if (find_handle != INVALID_HANDLE_VALUE) {
		...
		} while ((find_result = FindNextFileW(find_handle, &find_data)));
	...
	bool environment)
{
	...
	HC_Open_Segment_By_Key (m_MaterialSegmentKey); {
		HC_Open_Segment (name); {
			...
			if ( ptr = strrchr(diffuse_color, ',') )
				*ptr = '\0';
			if ( ptr = strrchr(other_color, ',') )
				*ptr = '\0';
		...
		const float *vals)
{
	...
	HC_Open_Segment_By_Key (m_MaterialSegmentKey); {
		HC_Open_Segment (material_name); {
			...
			remainder++;
			offset = declaration - shader_source;
...
const char *HMaterialLibrary::FindValidMaterial (
		char const *needle) const
{
...
bool HMaterialLibrary::IsValidMaterial 
...
bool HMaterialLibrary::ApplyMaterial (
		const char *name)
{
	bool status = true;
...
TK_Status HTK_One_Material::Write (BStreamFileToolkit & tk) alter {
	TK_Status       status = TK_Normal;
...
TK_Status HTK_One_Material::Read (BStreamFileToolkit & tk) alter {
	TK_Status       status = TK_Normal;
...
TK_Status HTK_One_Material::Execute (BStreamFileToolkit & tk) alter {
	if (m_Instancee != NULL) {
		HC_Open_Segment_By_Key (m_HMat->m_MaterialSegmentKey); {
...
void HTK_One_Material::Reset () alter 
{
	delete [] m_Name;
	m_Name = NULL;
	m_NameLength = 0;
