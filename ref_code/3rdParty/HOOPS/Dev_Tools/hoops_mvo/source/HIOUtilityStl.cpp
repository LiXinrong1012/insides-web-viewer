...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HIOUtilityStl.cpp,v 1.33 2010-06-14 19:21:22 jason Exp $
//
	...
	HOutputHandlerOptions	*options)
{
	...
	HC_Open_Segment_By_Key(the_key);
		...
		if (streq(vis_setting, "on"))
		{
			...
			HC_Begin_Contents_Search(".", "shell, sphere, polycylinder, nurbs surface, cylinder, polygon, circle, circular chord, circular wedge, ellipse, mesh, grid");
	...
	/* This may be needed to ensure consistent handedness within shells, but can cause long write times
	 *
	 *					HPoint * normals = new HPoint[numvertices];
	 *					HC_MShow_Net_Vertex_Normals(key, 0, numvertices, normals);
	 *					HC_Compute_Optimized_Shell(numvertices, points, normals, tristripslen, tristrips, "fix handedness",
	 ...
	 */
						...
						HC_Delete_By_Key(key);	/* we're now done with that temporary shell */
						...
						while (iptr < iend)  /* over entire list of tristrips */
						{
							...
							{
								float normal[3] = {0.f, 0.f, 0.f};  /* null normal */
								float * vertices;
								iptr++;
...
HFileInputResult HIOUtilityStl::ReadStlFileBinary(const wchar_t *FileName) 
{
	...
	fp = wfopen(FileName, L"rb"); // open the file as BINARY
	if (fp == NULL) 
		return InputFail;
	...
	/* 40000 triangles, 3 points each, 3 components each */
	vertexes = new HPoint[3 * NUM_TRIS_PER_SHELL];
	...
	while (facets_left > 0){
		...
		if (triangle_count) {
			int npout;
			HPoint *pout = new HPoint [point_index];
			int foutlen;
			int flistlen = triangle_count * 4;   //assumes triangles only
			int *flist = new int [flistlen];
			int *fout = new int [flistlen];
			int j;
