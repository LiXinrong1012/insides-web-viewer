...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HMarkupManager.cpp,v 1.44 2010-06-08 21:58:05 reuben Exp $
//
...
static void deselect_markup(HBaseView *view, HC_KEY start_seg)
{
	...
	HC_Begin_Contents_Search("...", "segments, geometry");
		while(HC_Find_Contents(type, &found_key))
		{
			if(sel_set->IsSelected(found_key))
				sel_set->DeSelect(found_key);
		}
...
void HMarkupManager::UpdatePreviewImage()
{
	if(changes()){
		HCamera saved, cam;
...
char const *
HMarkupManager::GetLayerNames(void) const
{
	if(m_allNames){
		free(m_allNames);
		m_allNames = 0;
