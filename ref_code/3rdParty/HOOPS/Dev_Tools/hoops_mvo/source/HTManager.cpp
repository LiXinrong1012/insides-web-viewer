...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HTManager.cpp,v 1.23 2008-08-07 00:14:46 jason Exp $
//
...
void HTManager::Tick( float time )
{
    ...
    while( m_request_time + m_interval < m_actual_time ) {
        ...
        if( m_current_bucket == m_output_hz ) {
            ...
            for(;;) {
				expiration = MVO_V2I(vlist_peek_first( m_recently_deleted_expirations ));
				if( expiration != 0 &&
