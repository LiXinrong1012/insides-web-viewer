...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HUtilityQuadTree.cpp,v 1.6 2006-08-07 20:38:45 stage Exp $
//
...
HQuadTreeNode::~HQuadTreeNode() 
{
	...
	if(m_pContents) {
		...
		if(m_CopiedItems) {
			...
			HQuadTreeItem * item;
			while((item = (HQuadTreeItem *) vlist_remove_first(m_pContents)))
				delete item;
		}
...
HQuadtreeInsertStatus HQuadTreeNode::Insert(HQuadTreeItem *item, int MaxDepth, bool AllowOverlap, bool CopyItem)
{
	...
	if(!AllowOverlap && m_pContents) {
		...
		while((test_item = (HQuadTreeItem *) vlist_peek_cursor(m_pContents))) {
