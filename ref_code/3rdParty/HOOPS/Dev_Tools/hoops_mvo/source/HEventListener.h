...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HEventListener.h,v 1.45 2008-12-09 22:00:11 david Exp $
//
...
#define HLISTENER_EVENT(listenertype, manager, function)	 { listenertype *el = (listenertype *)manager->GetEventListenerManager(listenertype::GetType()); \
												if (el) \
													el->function; }
#define HLISTENER_EVENT_STRING(listenertype, manager, function)	 { listenertype *el = (listenertype *)manager->GetEventListenerManager(listenertype::GetStringType()); \
												if (el) \
													el->function; }

#define HLISTENER_FUNCTION_0(classname, method) \
	virtual int method() { \
  				if (!m_pEventListenerManager) 	 \
 					return 0;  \
 				m_pEventListenerManager->StartQuery(); \
				classname* listener; \
				while ((listener = (classname*)m_pEventListenerManager->GetEventListener())) \
					if (listener->method() == HLISTENER_CONSUME_EVENT) \
						break; \
				return 1; }		


#define HLISTENER_FUNCTION_1(classname, method, param_type_1, param_1) \
	virtual int method( param_type_1 param_1 ) { \
  				if (!m_pEventListenerManager) 	 \
 					return 0;  \
 				m_pEventListenerManager->StartQuery(); \
				classname* listener; \
...
#define HLISTENER_FUNCTION_2(classname, method, param_type_1, param_1, param_type_2, param_2) \
	virtual int method( param_type_1 param_1 , param_type_2 param_2) { \
  				if (!m_pEventListenerManager) 	 \
 					return 0;  \
				m_pEventListenerManager->StartQuery(); \
				classname* listener; \
...
#define HLISTENER_FUNCTION_3(classname, method, param_type_1, param_1, param_type_2, param_2, param_type_3, param_3) \
	virtual int method( param_type_1 param_1 , param_type_2 param_2, param_type_3 param_3) { \
  				if (!m_pEventListenerManager) 	 \
 					return 0;  \
				m_pEventListenerManager->StartQuery(); \
				classname* listener; \
...
#define HLISTENER_SETUP_FUNCTIONS(listenertype)								\
  	static	HEventListenerType GetType() { return listenertype##Type; }			\
	virtual HEventListener *CreateListenerManager()							\
			{ HEventListener *temp = (HEventListener *)new listenertype();	\
			  temp->ConvertToManager();										\
			  return temp;													\
			}
													
#define HLISTENER_SETUP_FUNCTIONS_STRING(listenertype, stringtype)						\
static	HEventListenerType GetType() { return HUserdefinerListenerType; }   \
	static	const char * GetStringType() { return stringtype; } \
	virtual HEventListener *CreateListenerManager()							\
			{ HEventListener *temp = (HEventListener *)new listenertype();	\
			  temp->ConvertToManager();										\
			  return temp;													\
