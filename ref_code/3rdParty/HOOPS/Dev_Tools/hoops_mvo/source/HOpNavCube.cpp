...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HOpNavCube.cpp,v 1.26 2008-03-27 21:16:42 jason Exp $
//
...
{
	return (HBaseOperator *)new HOpNavCube(GetView());
}
...
int HOpNavCube::OnLButtonDown(HEventInfo &event)
{
	...
	HC_Open_Segment_By_Key(m_NavCubeSegKey);
		HC_Set_Heuristics("related selection limit = 4");
		HViewSelectionLevel lvl = GetView()->GetViewSelectionLevel();
		GetView()->SetViewSelectionLevel(HSelectionLevelEntity);
		res = HC_Compute_Selection(GetView()->GetDriverPath(),".","v", new_pos.x, new_pos.y);
