...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HOpCreateNurbs3D.cpp,v 1.36 2007-10-18 23:10:03 chad Exp $
//
#include <math.h>
#include <string.h>
#include <stdio.h>
...
{
	return (HBaseOperator *)new HOpCreateNurbs3D(GetView());
}
...
int HOpCreateNurbs3D::OnLButtonDown(HEventInfo &event)
{
	...
	HC_Open_Segment_By_Key(((HBaseView *)GetView())->GetViewKey());
		...
		if (HC_Compute_Selection( ".", "./scene/overwrite","v",new_point.x,new_point.y)>0)
		{
			do {
...
void HOpCreateNurbs3D::EmitModifyControlPointMessage(HBaseView *pView, HC_KEY key, int control_point, float x, float y, float z)
{
	...
	{
		long ident;
		if (pView->GetSharedKey() && (ident = pView->GetSharedKey()->GetIdentFromKey(key)))
		{
...
void HOpCreateNurbs3D::ModifyControlPointFromMessage(const char *in_data, unsigned int data_length, HBaseView *pView)
{
	...
	if (pView->GetSharedKey() && (key = pView->GetSharedKey()->GetKeyFromIdent(ident)))
	{
		HNurbsSurface nurbs;
		nurbs.Init(key);
