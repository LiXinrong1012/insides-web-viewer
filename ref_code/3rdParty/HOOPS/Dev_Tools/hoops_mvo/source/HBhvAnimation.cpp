...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HBhvAnimation.cpp,v 1.41 2009-02-23 20:12:29 thomas Exp $
//
...
{
	return ((HBhvInterpolator *)vlist_peek_first(m_InterpolatorList));
}
	...
	{	
		...
		{		
			while (1)
			{
				...
				if (!(temp = (HBhvAnimation *)vlist_peek_cursor(alist)))
				break;
...
void HBhvAnimation::DeleteKeyframe(int keyframe)
{
	...
	if (pos!=-1)
	{
  		...
  		while (1)
		{
			...
			if (!(temp = (HBhvInterpolator *)vlist_peek_cursor(m_InterpolatorList)))
				break;
			temp->Remove(pos);
