...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HOpSelectPolyline.cpp,v 1.31 2008-07-03 01:16:47 nathan Exp $
//
...
{
	return (HBaseOperator *)new HOpSelectPolyline(GetView());
}
...
int HOpSelectPolyline::OnLButtonDblClk(HEventInfo &event)
{
    ...
    HC_Open_Segment_By_Key(GetView()->GetViewKey());
		...
		if (m_PolylineCount > 1) 
		{
			...
			count = HC_Compute_Selection_By_Polylin(".", "./scene/overwrite", "v", m_PolylineCount, m_pPolyline);
