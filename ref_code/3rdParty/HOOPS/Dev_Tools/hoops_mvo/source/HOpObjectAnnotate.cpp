...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HOpObjectAnnotate.cpp,v 1.62 2007-11-01 00:08:09 jason Exp $
//
...
{
	return (HBaseOperator *)new HOpObjectAnnotate(GetView());
}
...
int HOpObjectAnnotate::OnLButtonDown(HEventInfo &event)
{
	...
	if (!OperatorStarted()) {
		...
		HC_Open_Segment_By_Key(GetView()->GetViewKey());
			...
			int count = HC_Compute_Selection (".", "./scene/overwrite", "v", WindowSpace.x, WindowSpace.y);
