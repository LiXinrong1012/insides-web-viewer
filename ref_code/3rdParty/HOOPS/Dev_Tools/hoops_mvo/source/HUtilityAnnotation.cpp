...
//
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HUtilityAnnotation.cpp,v 1.144 2010-10-12 00:28:25 nathan Exp $
//
...
{   
	...
	inv[1] = old_points[0];
	int cnum;
	if (cnum = Clip_Line_Inverse((HIC_DC_Point *)old_points,lo2, top, left, bottom, right))
	{
		inv[0] = lo2[1];
		inv[1] = lo2[0];
 			...
 			switch(type){
				...
				case BG_CLOUD:{
					...
					for(i=0; i<POINT_COUNT; ++i){
						theta = i/(float)POINT_COUNT * 2 * 3.1415926535;
						r = fabs(cos((float)(5*theta))/5.0f)  +1.25;
