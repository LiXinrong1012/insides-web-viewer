...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HOpSelectPolygon.cpp,v 1.29 2008-07-03 01:16:47 nathan Exp $
//
...
{
	return (HBaseOperator *)new HOpSelectPolygon(GetView());
}
...
int HOpSelectPolygon::OnLButtonDblClk(HEventInfo &event)
{
    ...
    HC_Open_Segment_By_Key(GetView()->GetViewKey());
		...
		HC_QSet_Driver_Options("./scene/overwrite", "selection proximity = 0.0");
		
		HViewSelectionLevel lvl = GetView()->GetViewSelectionLevel();
		GetView()->SetViewSelectionLevel(HSelectionLevelEntity);
		int count = HC_Compute_Selection_By_Polygon(".", "./scene/overwrite", "v", m_PolylineCount, m_pPolyline);
		GetView()->SetViewSelectionLevel(lvl);

		HC_QUnSet_Driver_Options("./scene/overwrite" );
