...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HIOUtilityHsf.cpp,v 1.54 2010-10-22 19:10:17 stage Exp $
//
...
public:
	...
	TK_Status Interpret(BStreamFileToolkit &tk, ID_Key key, int variant) alter{
		...
		if(strstr(proj, "oblique"))
		{
			...
			m_settings[11]=oblique_y;
			m_settings[12]=oblique_x;
		}
...
TK_Status HW3D_Image::Write (BStreamFileToolkit & tk) alter {
...
TK_Status HW3D_Image::Read (BStreamFileToolkit & tk) alter {
    TK_Status       status = TK_Normal;
...
TK_Status   HW3D_Image::Execute (BStreamFileToolkit & tk) alter
{
	return TK_Normal;
}

void HW3D_Image::Reset (void) alter {
...
void HW3D_Image::set_name (int length) alter {
...
void HW3D_Image::set_name (char const * name) alter {
    if( name != 0 ) {
