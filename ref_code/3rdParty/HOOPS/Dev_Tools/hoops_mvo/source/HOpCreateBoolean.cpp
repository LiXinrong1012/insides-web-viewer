...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HOpCreateBoolean.cpp,v 1.15 2007-11-20 01:49:23 conor Exp $
//
...
{
	return (HBaseOperator *)new HOpCreateBoolean(GetView(), m_BooleanType);
}
...
int HOpCreateBoolean::OnLButtonDown(HEventInfo &event)
{
	...
	HC_Open_Segment_By_Key(GetView()->GetViewKey());
		...
		if ((res = HC_Compute_Selection(".", "./scene/overwrite", "v", new_pos.x, new_pos.y)))
...
void HOpCreateBoolean::HandleSelection(HEventInfo &event)
{
	...
	if(strcmp (type, "shell") && strcmp (type, "cylinder") 
		&& strcmp (type, "polycylinder") && strcmp (type, "sphere"))
		return;
