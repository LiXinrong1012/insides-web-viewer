...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HOpObjectClash.h,v 1.41 2009-10-08 17:38:56 nathan Exp $
//
...
protected:
	...
	*/
	enum HOOC_SelectMode {
		HOOC_World =	 0x1,//!< This selection mode computes the selection in world coordinates prior to the camera transformation and then looks for bounding box intersections. Use this mode for speed.
		HOOC_Screen =	 0x2,//!< This selection mode computes the selection in screen coordinates and looks for bounding box intersections.
		HOOC_Shell =	 0x3,//!< This selection mode looks for shells that intersect the faces of the current selection set in HBaseView.  This is the default mode and is most accurate.
		HOOC_Enclosure = 0x4//!< This selection mode detects if the selection is completely enclosed by the translated object.
	};
	...
	*/
	enum HOOC_Result {
		...
		HOOC_In =	 0x1,//!< The translated objects are completely enclosed by the highlighted object.
		HOOC_Out =	 0x2,//!< The translated objects is not completely enclosed by the highlighted object.
		HOOC_Clash =	 0x3//!< The selected objects intersect wit the translated objects.
	};
...
protected:
	...
	*/
	void ProcessSelectionResults( HC_KEY current_shell = INVALID_KEY, float *transformed_points = 0 );
	...
	*/
	...
	\return An #HOperatorReturn indicating the status of the event.
	*/
	int SelectByScreenVolume( HSmartSelItem * sel_item );
	...
	\return An #HOperatorReturn indicating the status of the event.
	*/
	int SelectByWorldVolume( HSmartSelItem * sel_item );
	...
	\return An #HOperatorReturn indicating the status of the event.
	*/
	int SelectByEnclosure( HSmartSelItem * sel_item );
	...
	\return An #HOperatorReturn indicating the status of the event.
		*/
	int SelectByShell( HSmartSelItem * sel_item );
