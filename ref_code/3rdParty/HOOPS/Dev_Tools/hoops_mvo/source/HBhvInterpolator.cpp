...
void HBhvInterpolator::AddScaleToMatrix(HPoint &scale)
{
	...
	HPoint pivot;
 	HC_Open_Segment_By_Key(m_pTarget);
		...
		HC_Compute_Translation_Matrix(-pivot.x,-pivot.y,-pivot.z, pivot_translation_matrix);
		HC_Compute_Matrix_Inverse(pivot_translation_matrix, pivot_translation_matrix_inverse);	
  		HC_Compute_Scale_Matrix(scale.x, scale.y, scale.z, scalematrix);
  		segment_modelling_matrix[12] = segment_modelling_matrix[13] = segment_modelling_matrix[14] = 0;
		...
		GetTranslationFromMatrix(op);
  		HC_Compute_Translation_Matrix(op.x, op.y, op.z, translation_matrix);  		
		HC_Compute_Matrix_Product(pivot_translation_matrix_inverse, segment_modelling_matrix, result);		    	
		HC_Compute_Matrix_Product(result, scalematrix, result2);
   		HC_Compute_Matrix_Product(result2, pivot_translation_matrix, result);		    	
  		HC_Compute_Matrix_Product(result, translation_matrix, result2);		    	
		HC_Set_Modelling_Matrix(result2);
	HC_Close_Segment();			
}
...
void HBhvInterpolator::GetRotationFromMatrix(HQuat &t)
{
	...
	HC_Open_Segment_By_Key(m_pTarget);
		...
		GetAnimation()->GetTarget()->GetPivot(pivot);
 		if (HC_Show_Existence("modelling matrix"))
				...
				HC_Compute_Identity_Matrix(segment_modelling_matrix); 
		t = HQuat::MatrixToQuaternion(segment_modelling_matrix);
  	HC_Close_Segment();
}
...
void HBhvInterpolatorPosition::CalculateAllTangents()
{
	...
	{
		if (((HKeyframeChannel *)splinearray[i])->m_channeltype == HANIFollowPath || ((HKeyframeChannel *)splinearray[i])->m_channeltype == HANIFollowPath || ((HKeyframeChannel *)splinearray[i])->m_channeltype == HANIHermiteSpline)
		{
