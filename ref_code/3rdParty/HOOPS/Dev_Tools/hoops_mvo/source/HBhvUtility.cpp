...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HBhvUtility.cpp,v 1.46 2010-08-16 17:01:02 jason Exp $
//
...
HQuat HQuat::QslerpNoInvertExtraSpins(const HQuat &rkP,const HQuat &rkQ,float fT, int iExtraSpins)
{
	...
	float fAngle = float(H_ACOS(fCos));
...
void HKeyframeChannelCurve::DrawTangents(HPoint p2)
{
	...
	HC_Insert_Line(p2.x, p2.y, p2.z, p2.x + m_tangent2.x, p2.y + m_tangent2.y, p2.z + m_tangent2.z);
...
void HKeyframeChannelCurve::CalculateCurveLength(HPoint p2)
{
	...
	for (int i=0;i<100;i++,fraction+=adder )
	{
		...
		m_curvelength += (float)HC_Compute_Vector_Length(&delta);
		oldpoint = res;
	}
}
...
void HKeyframeMatrix::Serialize(HUtilityXMLTag *xmlgen)
{
	char text[MVO_BUFFER_SIZE];
 	...
 	HCLOCALE(sprintf(text, "[%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f]",m_matrix[0], m_matrix[1],
		m_matrix[2],m_matrix[3],m_matrix[4],m_matrix[5],m_matrix[6],m_matrix[7],m_matrix[8],m_matrix[9],m_matrix[10],
		m_matrix[11],m_matrix[12],m_matrix[13],m_matrix[14],m_matrix[15]));
 	xmlgen->AddListItem(text);
}
...
void HBhvUtility::AddShellToMorphList(HC_KEY key, struct vlist_s *morph_data_list)
{
	...
	ShellMorphInfo *temp;
	if (!(temp = (ShellMorphInfo *)vlist_peek_cursor(morph_data_list)))
	{
 		temp = new ShellMorphInfo();
		temp->original_key = key;
		vlist_add_last(morph_data_list, (void *)temp);
	}
	int plength, flength;
	HPoint *points;
	...
	vlist_add_last(temp->arrays_list, (void *)points);
	temp->temp_key = key;
}
...
void HBhvUtility::MakeCameraKeyframe(HBaseView *m_pView, float timediff, bool targetmode) 
{
	HPoint p,t,u;
	float fx, fy;
	char proj[4096];
	...
	HC_Show_Segment(m_pView->GetSceneKey(), targetname);
