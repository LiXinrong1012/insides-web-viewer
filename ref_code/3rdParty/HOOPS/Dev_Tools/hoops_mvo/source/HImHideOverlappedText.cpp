...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HImHideOverlappedText.cpp,v 1.9 2008-02-05 20:45:31 jason Exp $
//
...
void HImHideOverlappedText::draw_text( HIC_Rendition const *nr )
{
	...
	struct text_node_s ** nodes = (struct text_node_s **) malloc(n_nodes*sizeof(struct text_node_s *));

	for(i=0;i<n_nodes;i++) 
		nodes[i] = (struct text_node_s *)vlist_nth_item(text_list,i);

	zsort(nodes, nodes + n_nodes - 1);
...
void HImHideOverlappedText::update( HIC_Rendition const *nr, HIC_Segment_Info const *seg )
{
	...
	while((node = (struct text_node_s *) vlist_remove_first(text_list))) {
		free(node->karacter);
	...
	while((annotation = (struct segment_node_s*) vlist_remove_first(annotation_list))) {
