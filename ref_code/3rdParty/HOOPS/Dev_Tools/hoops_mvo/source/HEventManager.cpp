...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HEventManager.cpp,v 1.35 2008-02-19 21:26:05 jason Exp $
//
...
HEventListener *HEventManager::GetEventListenerManager(HEventListenerType eventtype)
{
	HEventListener *eventlistenerlist = 0;
  	if (!(vhash_lookup_item(m_pEventListenerListHash, (void *)eventtype , (void **)&eventlistenerlist) == VHASH_STATUS_SUCCESS))
		return 0;
 	return ((HEventListener *)eventlistenerlist);
}
...
{
/*
	struct vlist_s *temp_vlist = new_vlist(malloc, free);
 	vhash_string_keys_to_vlist(m_pEventListenerListHash, temp_vlist, malloc);
	HEventListener *eventlistenerlist = 0;
 	vlist_reset_cursor(temp_vlist);
 	while (1)
	{
		HEventListener *temp;
		if (!(temp = (HEventListener *)vlist_peek_cursor(temp_vlist)))
				break;
		 temp->RemoveEventListener(listener);

   		vlist_advance_cursor(temp_vlist);	
	}
	delete_vlist(temp_vlist);*/
}
