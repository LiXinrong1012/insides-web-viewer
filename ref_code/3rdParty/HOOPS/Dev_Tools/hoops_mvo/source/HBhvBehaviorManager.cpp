...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HBhvBehaviorManager.cpp,v 1.68 2009-09-04 01:38:18 conor Exp $
//
...
HBhvTargetObject * HBhvBehaviorManager::FindTargetObjectByName(const char *name)
{
 	...
 	while (1)
	{
		...
		if (!(temp = (HBhvTargetObject *)vlist_peek_cursor(m_TargetObjectList)))
				break;
...
void HBhvBehaviorManager::ScheduleAllAnimations(bool reset)
{
	...
	{
		HBhvAnimation *temp;
		if (!(temp = (HBhvAnimation *)vlist_peek_cursor(m_AnimationList)))
				break;
		if (reset)
		...
		HC_Begin_Contents_Search("...", "shells");
			while(HUtility::FindContents(hShellKey) && !bCollisionFound)
			{
				...
				HC_Open_Segment_By_Key(GetActiveView()->GetOverwriteKey());
					...
					HViewSelectionLevel lvl = GetActiveView()->GetViewSelectionLevel();
					GetActiveView()->SetViewSelectionLevel(HSelectionLevelEntity);
					if(HC_Compute_Selection_By_Shell( "v",
														".", 
														nPointCount, 
														pTransformedPoints, 
														nFaceListLength, 
					...
					GetActiveView()->SetViewSelectionLevel(lvl);
