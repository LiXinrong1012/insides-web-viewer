...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HUtility.cpp,v 1.517 2011-01-06 22:39:36 reuben Exp $
//
...
void HUtility::Find_Relative_Path(HC_KEY from_key, HC_KEY to_key, char * path)
{
	...
	sprintf(relative_path + 3*i, to_path);
...
{
	return (bool) ((c1.position.Equal(c2.position)) &&
...
void HUtility::FindFileNameExtension(const char * filename_in, char * ext_out, int size)
{
	...
	HC_Parse_String(filename_in, ".", -1, ext_out);

	// make it lower case
	unsigned int i;
	for (i = 0; i < strlen(ext_out); i++)
...
void HUtility::FindFileNameExtension(wchar_t const * const filename_in, __wchar_t * ext_out, int size)
{
	...
	wchar_t const * tmp = wcsrchr(filename_in, L'.');
	if(!tmp){
...
{
	ext_out[0] = '\0';
	unsigned int len16 = H_UNICODE_STRLEN(filename_in);
	unsigned int i = len16;
	unsigned int j;
	...
	// copy extention & null terminator to ext_out
	for (j=0, i++ ; i <= len16+1; i++, j++)
		ext_out[j] = filename_in[i];
...
void HUtility::FindFileNameAndDirectory(const char * filename_in, char * dir_out, char * filename_out)
{
	...
	unsigned int i = (unsigned int)strlen(filename_in);
	...
	if (i!=0) {
		...
		if (dir_out[i] == '\0') {
			...
			if (filename_out) {
				strcpy(filename_out, (const char *)(&dir_out[i]));
			}
			dir_out[i]='\0';
		}
...
int HUtility::CalculateKeyPath(HC_KEY startkey, HBaseView *view, HC_KEY *keyarray, int maxkeynum)
{
	...
	while (1)
	{
		...
		{
  			newkey  = HC_KShow_Owner_Original_Key(newkey);
			if (newkey == -1)
				return knum;
			keyarray[knum++] = newkey;
		}
...
{
	float matrix[16];
 	HC_KEY sikey = HC_KOpen_Segment(segname);
...
//	HImSetCallback("draw segment tree","HUtility_scale_segment");
 	HC_Close_Segment();
 	HC_Close_Segment();
 	return  (sikey);
}
...
HC_KEY HUtility::GetScaleIndependentGeomSegmentKey(HC_KEY segkey)
{
	...
	HC_Open_Segment("contents");
 	HC_KEY geomkey = HC_KOpen_Segment("geom");
	HC_Close_Segment();
	HC_Close_Segment();
	HC_Close_Segment();
	return geomkey;
...
bool HUtility::IsModelKey(HC_KEY modelkey, HC_KEY objectkey)
{
	...
	if (strncmp(modelsegment, owner, strlen(modelsegment)) == 0)
		return true;
	else
		return false;
}
...
HC_KEY HUtility::SelectUnderCursor(HBaseView *view, float window_x,float window_y, bool doSelection, bool DeSelect)
{
    ...
    HC_Open_Segment_By_Key(view->GetViewKey());
		...
		count = HC_Compute_Selection(".","./scene/overwrite","v", window_x, window_y);
...
{
    int plen1, flen1, plen2, flen2, plen3, flen3, i;
    HPoint *points1;
    int *faces1;
    HPoint *points2;
    int *faces2;
...
int HUtility::CalculateMajorAxis(HBaseView *view)
{
    HPoint t;
    HC_Open_Segment_By_Key(view->GetSceneKey());
    HC_Show_Net_Camera_Up_Vector (&t.x, &t.y, &t.z);
    HC_Close_Segment();
    
    float x = (float)fabs(t.x);
    float y = (float)fabs(t.y);
    float z = (float)fabs(t.z);
...
void HUtility::ReadjustTarget(HBaseView *view)
{
    ...
    HC_Open_Segment_By_Key(view->GetViewKey());
		lvl = view->GetViewSelectionLevel();
		view->SetViewSelectionLevel(HSelectionLevelEntity);
		int selection_count = HC_Compute_Selection(".", "./scene/overwrite","v", 0,0);
		view->SetViewSelectionLevel(lvl);
		if (!selection_count)
		{
			lvl = view->GetViewSelectionLevel();
			view->SetViewSelectionLevel(HSelectionLevelEntity);
			selection_count = HC_Compute_Selection_By_Area(".", "./scene/overwrite","v", -0.3, 0.3, -0.3, 0.3);
			if (!selection_count)
				selection_count = HC_Compute_Selection_By_Area(".", "./scene/overwrite","v", -1, 1, -1, 1);
			view->SetViewSelectionLevel(lvl);
		}
		if (selection_count)
		{
...
void HUtility::MakeViewSnapshot(HBaseView * view, int width, int height, char ** data)
{
	...
	view->GetWindowColor(WindowColorTop, WindowColorBottom);
	HCLOCALE(sprintf(color, "windows=(r=%f g=%f b=%f), window contrast = (r=%f g=%f b=%f)", WindowColorTop.x, WindowColorTop.y, WindowColorTop.z, WindowColorBottom.x, WindowColorBottom.y, WindowColorBottom.z));
	HC_Set_Color(color);
...
//	pos = (HT_Row_Vector alter *)camera->positioning_matrix.elements;
	HPoint eye, crosseye;
	eye.Set(camera->target.x - camera->position.x, camera->target.y - camera->position.y, 
		camera->target.z - camera->position.z);
 	float eyedistance = HC_Compute_Vector_Length(&eye);
	...
	HC_Open_Segment("/");{
		...
		HC_Open_Segment_By_Key(m_hiddenSegKey);{
			...
			image_key = HC_KInsert_Image_By_Ref(0, 0, 0,
				H_FORMAT_TEXT("name = %s, %s", (char*)m_imageName, format),
...
void BandedImageDriver::DefineTexture()
{
	/* With only one block, we can bypass the tiled code.  */
	if(m_blockCount == 1){
		HC_Define_Texture(m_textureName, H_FORMAT_TEXT("source = %s", (char*)m_imageName));
	}
	else
	{
								 ...
								 HPoint &totaldeltadiv, HShellWrapper *shellmatrix, HShellWrapper &leftover, float boxdiagonallength, bool DoCut, int regionnum, ShellToRegionMapper *mapper, HC_KEY newkey)
{
	...
	for (i=0;i<10000;i++)
		lastkey[i] = -1;
	int storedkeys = 0;
	while (flen = myshell.NextFace(&face))
	{
...
{ 
	return (bool)((u - v < epsilon) && (u - v > -epsilon));
}
...
HUtility::AdjustPositionToPlane(HBaseView * pView, HPoint &position, HPoint point_in_plane)
{
	...
	HC_Open_Segment_By_Key(pView->GetSceneKey());{
		...
		HC_Compute_Coordinates (".", "world", &position,
			"projection", &position);
...
{
	HShellRegion oRetval(this, index);
	return (HShellRegion const)oRetval;
}
...
void HShellVertexData::SetFIndexData(int const size, float const * const data){
	if (m_pFIndexArray)
...
void HVerticalUtilityInternal::CompareCloudToShells::operator () (HC_KEY const cloud) const {
	...
	int count = m_shells->Count();
	while(count--){
		compare_func(m_shells->GetData(count));
	}

	HC_MSet_Vertex_Colors_By_FIndex(cloud, "markers", 0, results.Count(), &results[0]);
...
void HVerticalUtility::ColorizePointCloud(HC_KEY const segment){
	HC_Open_Segment_By_Key(segment);{
		...
		if(shell_keys.Count() > 0 && point_cloud_keys.Count() > 0){
			...
			int count = point_cloud_keys.Count();
			while(count--){
...
const unsigned short * HUnicodeOptions::Show_One_Net_Unicode_Option( const unsigned short * which){
	return (unsigned short const*)Show_One_Net_Unicode_Option((wchar_t const *)which);
}
#endif
...
const unsigned short * HUnicodeOptions::Show_One_Unicode_Option(const unsigned short* which){
	return (unsigned short const*)Show_One_Unicode_Option((wchar_t const *) which);
}
#endif
...
const wchar_t* HUnicodeOptions::Show_One_Unicode_Option(const wchar_t* which)
{
	...
	m_length = (int)wcslen(L"user options = ")+(int)wcslen(which);
...
HBestImageDriver::operator char* () const
{
	return *m_dvr;
}
