...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HUtilityAntialiasing.cpp,v 1.33 2010-02-23 20:13:48 reuben Exp $
//
...
bool HUtilityAntialiasing::CreateImage()
{
	...
	if (m_bFirst)
	{
 	HC_Flush_Contents(".", "includes");
		...
		HC_Set_Color_By_Value("windows", "RGB", window_color.x, window_color.y, window_color.z);
		sprintf (buffer, "use window id = (image key = %s%p)", H_EXTRA_POINTER_FORMAT, image_key);
		if (m_bFirst)
		    HC_Set_Driver_Options (buffer);
		HC_Set_Driver_Options ("isolated");
...
//	   		(m_pHView->GetEventCheckerCallback())();
		...
		for (int j=0;j<y;j++)
		{
			...
			m_pAAImageData[i+j*x] = HPixelRGB(r/div, g/div, b/div);
		}
	}
 	delete [] image_data;
