...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HOpMoveHandle.cpp,v 1.39 2008-11-06 19:18:16 nathan Exp $
//
...
{
	return (HBaseOperator *)new HOpMoveHandle(GetView());
}
...
/*			HC_Open_Segment_By_Key(owner);
			HC_Delete_Segment("box");
			HC_Close_Segment();
			ghandle.TurnOff(m_pHandleKey);*/
			m_pHandleKey = INVALID_KEY;
			m_pObjectKey = INVALID_KEY;
			SetOperatorStarted(false);
...
int HOpMoveHandle::OnLButtonDownAndMove(HEventInfo &event)
{
	...
	if (ghandle.GetConstraintType(m_pHandleKey) == HConstrainToPlane || ghandle.GetConstraintType(m_pHandleKey) == HConstrainToRay || ghandle.GetConstraintType(m_pHandleKey) == HConstrainToViewplane)
	{
		if (ghandle.GetConstraintType(m_pHandleKey) == HConstrainToRay){
		    ...
		    ghandle.AdjustPointToRay(m_pHandleKey, new_point, GetView()->GetSceneKey(), GetView());
			SetNewPoint(new_point);
		}
		ghandle.Move(m_pHandleKey,GetNewPoint(), GetView()->GetSceneKey(), GetView());
		HC_KEY owner = HC_KShow_Owner_By_Key(m_pHandleKey);
