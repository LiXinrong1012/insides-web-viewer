...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HIOUtilityVRML.cpp,v 1.63 2010-09-27 05:36:34 jason Exp $
//
...
HFileInputResult HIOUtilityVRML::FileInputByKey(const wchar_t * FileName, HC_KEY key, HInputHandlerOptions * options)
{
	...
	bool result = v2hread.vrmltoHOOPS(FileName, key);
...
bool V2HUtils::BuildTexPointList(int& tc_num, float*& tc_list)
{
	...
	for (;;)
	{
		...
		HCLOCALE(tc_list[tc_num*2]= (float)atof(token));
 		...
 		if(!V2HUtils::NextToken(token))
		{
  		...
  		HCLOCALE(tc_list[tc_num*2+1]=(float)atof(token));
...
bool V2HUtils::Token2RGBA(char * token, const int n_components, unsigned char * RGBA)
{
	...
	char hex_str = V2HUtils::HexStr2Int(hex_pix);
	...
	hex_pix[1] = end[1];
	hex_pix[2] = '\0';
	hex_str = V2HUtils::HexStr2Int(hex_pix);
	if(n_components == 2)
	{
		RGBA[0] = RGBA[1] = RGBA[2] = hex_str;
		return true;
...
bool V2HUtils::vrmltoHOOPS(const wchar_t *filename, HC_KEY key)
{
	...
	now = ftell(fhandle);
	status = fseek(fhandle, 0, SEEK_END);
	file_size = ftell(fhandle);
	m_file_size = file_size - now;
	status = fseek(fhandle, now, SEEK_SET); 
...
//			HCLOCALE(sprintf(amb_def, "ambient = (R = %f G = %f B = %f)", a, a, a));
//			HC_Open_Segment_By_Key(m_segkey);
...
//			HC_Close_Segment();
		...
		else if (streq(m_token,"shininess") )
		{
			...
			{
				HCLOCALE(sprintf(shin_def, "faces = (gloss = %f)", a*128)); //see lighting equation for VRML2.0 
				HC_Open_Segment_By_Key(m_segkey);
...
bool VText::ReadString(V2HUtils *v2hread)
{
	...
	assert_NEXT_TOKEN( m_token, "[", v2hread );
