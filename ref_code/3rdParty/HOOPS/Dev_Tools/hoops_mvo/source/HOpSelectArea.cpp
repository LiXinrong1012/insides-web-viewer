...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HOpSelectArea.cpp,v 1.121 2010-10-04 22:07:27 boni Exp $
//
...
{
	return (HBaseOperator *)new HOpSelectArea(GetView());
}
...
int HOpSelectArea::OnLButtonUp(HEventInfo &event)
{
	...
	HC_Open_Segment_By_Key(GetView()->GetViewKey());
		...
		{
			HC_QSet_Heuristics ("./scene/overwrite", "no related selection limit, no selection sorting, no internal selection limit");
			...
			selection_count = HC_Compute_Selection_By_Area(".",
				...
				"v", m_ptRectangle[0].x, m_ptRectangle[1].x,	m_ptRectangle[0].y, m_ptRectangle[1].y);
		}
		else
		{
			HC_QSet_Heuristics ("./scene/overwrite", "related selection limit = 0, selection sorting, no internal selection limit");
			HPoint new_pos = event.GetMouseWindowPos();
			selection_count = HC_Compute_Selection(".",
				(selection->GetSubwindowPenetration() ? "" : "./scene/overwrite"), 
				"v", new_pos.x, new_pos.y);
		}
