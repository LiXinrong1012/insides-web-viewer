...
// manner of such use.
//
// $Id: HBaseView.cpp,v 1.993 2010-11-10 22:32:08 reuben Exp $
//
	...
	const char * driver_path)
{
	...
	HC_Open_Segment(MVO_FAKE_HLR_STYLE_SEGMENT);
		m_FakeHLRSource=HC_KOpen_Segment("");
			...
			if (!strieq(m_sDriverType,"opengl2") && !strieq(m_sDriverType,"dx9") )
    ...
    int debug_flags )
{
	...
	HC_Open_Segment_By_Key(m_ViewKey);
		HC_UnSet_Driver ();  //shuts down the old driver
	HC_Close_Segment();

	old_driver_path = m_sDriverPath;
	if(!driver_type) 
		driver_type = HDB::GetDriverType();  //just grab the mvo default
    if(!instance_name) 
	...
	m_sDriverPath = new char[strlen(pathname)+1];
	strcpy(m_sDriverPath, pathname);
	//new driver will start on the next update
	...
	HC_Open_Segment_By_Key(m_ViewKey);
		...
		sprintf (dopt, "use window id = (default = %s%p)", H_EXTRA_POINTER_FORMAT, m_pWindowHandle);
		...
		HC_Set_Driver_Options (dopt);
	    sprintf (dopt, "debug = %u", debug_flags);
		HC_Set_Driver_Options (dopt);
	HC_Close_Segment();
...
void HBaseView::FitSelection(HC_KEY selectionkey, bool adjustToShellNormal)
{
	...
	for (int i= 0; i < list_size; i++)
	{
		HC_KEY current_key;
		if (selectionkey != INVALID_KEY)
		    current_key = selectionkey;
		else
		   current_key = GetSelection()->GetAt(i);
		...
		if (strcmp(type,"segment")==0)
		{
 			int succ;
 			if (i==0)
				succ = HC_Filter_Circumcuboid_By_Key(current_key,"",&min,&max);
			succ = HC_Filter_Circumcuboid_By_Key(current_key,"", &newmin,&newmax);
		...
		else
		{
			if (strcmp(type,"shell")==0 && adjustToShellNormal)
				HUtility::ComputeShellNormal(current_key,normal);

			HC_KEY ownerkey = HC_KShow_Owner_By_Key(current_key);
			...
			HC_KEY temp = HC_KOpen_Segment("");
				HC_Move_By_Key(current_key,".");
				int succ;
 				if (i==0)
					succ = HC_Compute_Circumcuboid_By_Key(temp,&min,&max);
				succ = HC_Filter_Circumcuboid_By_Key(temp,"",&newmin,&newmax);
			HC_Close_Segment();
			HC_Move_By_Key(current_key,".");
			// now must convert the bounding box into world space
		...
		int kcount = HUtility::CalculateKeyPath(segKey, this, keys, MVO_BUFFER_SIZE);
		...
		newmin.x < min.x ? min.x = newmin.x : 0;
		newmin.y < min.y ? min.y = newmin.y : 0;
		newmin.z < min.z ? min.z = newmin.z : 0;
		newmax.x > max.x ? max.x = newmax.x : 0;
		newmax.y > max.y ? max.y = newmax.y : 0;
		newmax.z > max.z ? max.z = newmax.z : 0;
		 ...
		 ) 
	{
		...
		if (GetViewUpdateMode() == OnDemand || 
			(GetViewUpdateMode() == Continuous && GetUpdateRequested()) ||
				(GetViewUpdateMode() == Deferred && (GetUpdateRequested() || GetUpdateNotFinished())))
		{
			...
			if (GeometryChanged() &&
				...
				if ((strlen(plane_buf) != 0) && (strlen(light_buf) != 0))
				{
					int count;
...
bool HBaseView::GetDisplayListMode()
{
    ...
    HC_Open_Segment_By_Key(m_ViewKey);
	 	HC_Show_One_Net_Rendering_Optio("display lists", cval);
    HC_Close_Segment();
...
bool HBaseView::GetLodMode()
{
    ...
    HC_Open_Segment_By_Key(m_SceneKey);
	 	HC_Show_One_Net_Rendering_Optio("lod", cval);
    HC_Close_Segment();
...
int	HBaseView::GetLodThreshold()
{
    ...
    HC_Open_Segment_By_Key(m_SceneKey);
	 	HC_Show_One_Net_Rendering_Optio("threshold", cval);
    HC_Close_Segment();
...
ProjMode HBaseView::GetProjMode()
{
	...
	HC_Show_Net_Camera_Projection(val);
...
void HBaseView::GetWindowColor(HPoint &color_out)
{
	...
	HC_Open_Segment_By_Key(GetViewKey());
		...
		if ( HC_Show_Existence("color=windows") )
			HC_Show_One_Color_By_Value("windows",colorSpace,&color_out.x,&color_out.y,&color_out.z);			
		else
			HC_Show_One_Net_Color_By_Value("windows",colorSpace,&color_out.x,&color_out.y,&color_out.z);			
...
{	
	...
	HC_Open_Segment_By_Key(GetViewKey());
		...
		if ( HC_Show_Existence("color=windows") )
			HC_Show_One_Color_By_Value("windows",colorSpace,&color_top.x,&color_top.y,&color_top.z);		
		else
			HC_Show_One_Net_Color_By_Value("windows",colorSpace,&color_top.x,&color_top.y,&color_top.z);		

		assert( streq(colorSpace,"rgb") );

		if ( HC_Show_Existence("color=window contrast") )		
			HC_Show_One_Color_By_Value("window contrast",colorSpace,&color_bottom.x,&color_bottom.y,&color_bottom.z);			
		
		else
			HC_Show_One_Net_Color_By_Value("window contrast",colorSpace,&color_bottom.x,&color_bottom.y,&color_bottom.z);			
...
void HBaseView::SetInitialCamera(HPoint pos, HPoint tar, HPoint up, float fieldx, float fieldy, char *proj)
{
...
void HBaseView::PrepareForCameraChange()
{
	...
	while(temp = hmanager->GetNextUndoItem())
	{
	...
	if(!old_item || 
		memcmp(old_item->GetCamera(),item->GetCamera(),sizeof(HCamera)))
    ...
    HC_Open_Segment_By_Key(GetModel()->GetModelKey());
		if (strcmp(segment, "nosegment") != 0)
			HC_Open_Segment(segment);
 				TK_Status status = GetModel()->GetStreamFileTK()->ParseBuffer(in_data, data_length);
				(void)status;
		if (strcmp(segment, "nosegment") != 0)
			HC_Close_Segment();
    HC_Close_Segment();
...
void HBaseView::EmitSelectMessage(HC_KEY key, bool isSelect)
{
	...
	if(emit_message_function) {
		...
		if (m_pSharedKey && (ident  = m_pSharedKey->GetIdentFromKey(key)))
		{
			int temp = isSelect;
	...
	if(GetRenderMode() == HRenderFakeHiddenLine)
	{
		...
		HC_Open_Segment_By_Key(GetViewKey());
			...
			if(HC_Show_Existence("user option=saved wcolor"))
			{
				...
				HC_Show_One_User_Option("saved contrast wcolor", buf);
				sprintf(wcolor, "%s, window contrast=%s", wcolor, buf);
				HC_Set_Color(wcolor);
    ...
    switch (NewRenderMode)
    {
		...
		case HRenderFakeHiddenLine: {
			...
			HC_Open_Segment_By_Key(GetViewKey());
				...
				HC_Show_One_Net_Color("windows", wcolor);
				sprintf(tmp, "saved wcolor=%s", wcolor);
				HC_Show_One_Net_Color("window contrast", wcolor);
				sprintf(tmp, "%s, saved contrast wcolor=%s", tmp, wcolor);
...
HFileOutputResult HBaseView::FileOutput(const wchar_t * FileName, HOutputHandler * handler, HOutputHandlerOptions * options)
{
	...
	if(handler->GetOutputStyle() == HOutputHandlerStyleModel) {
		...
		if(options->m_Window_Width == 0 || options->m_Window_Height == 0 || options->m_Image_Dpi == 0) {
			...
			HBestImageDriver driver_segment;
...
void HBaseView::AdjustShadowPlane(HPlane& shadow_plane, HPoint const & light_pos)
{
	...
	HC_Open_Segment_By_Key(GetModel()->GetModelKey());
		...
		float temp;
		temp = (max_world_corner.x - min_world_corner.x)/100.f;
		min_world_corner.x -= temp;
		max_world_corner.x += temp;
		temp = (max_world_corner.y - min_world_corner.y)/100.f;
		min_world_corner.y -= temp;
		max_world_corner.y += temp;
		temp = (max_world_corner.z - min_world_corner.z)/100.f;
	...
	{
		for( i = 0; i < 8; i++) {
			float d = HC_Compute_Dot_Product(&corners[i], &light_pos);
...
void HBaseView::SetShadowColor(HPoint color)
{
    ...
    HC_Close_Segment();
    HCLOCALE(sprintf( shadow_opts, "simple shadow = (color = '( r = %f g = %f b = %f)')",
...
void HBaseView::SetShadowParams(const HPlane& shadow_plane, const HPoint& light_pos, 
								const HPoint* color, const char * filter)
{
	char shadow_opts[MVO_BUFFER_SIZE];
	...
	HC_Open_Segment_By_Key(GetSceneKey());
		...
		HCLOCALE(sprintf( shadow_opts, "simple reflection = plane = (%f, %f, %f, %f)", 
				 -shadow_plane.a(), -shadow_plane.b(), -shadow_plane.c(), -shadow_plane.d()));
		HC_Set_Rendering_Options(shadow_opts);
...
void HBaseView::SetShadowParams(HShadow* shadowObj)
{
	...
	HC_Open_Segment_By_Key(GetSceneKey());
		...
		HC_Compute_Coordinates(".", "viewpoint", &cuboid_corner[lowest_pt_indx], "world", &cuboid_corner[lowest_pt_indx]);
...
bool HBaseView::GetFrameBufferEffectsOptions(char * options, int *border_out)
{
	...
	*ptr = '\0';
	...
	HC_Show_One_Net_Driver_Option ("fast silhouette edges", temp);
	if (strncmp (temp, "off", 3)) {
	...
	HC_Show_One_Net_Driver_Option ("ambient occlusion", temp);
	if (strncmp (temp, "off", 3)) {
	...
	HC_Show_One_Net_Driver_Option ("depth of field", temp);
	if (strncmp (temp, "off", 3)) {
	...
	HC_Show_One_Net_Driver_Option ("bloom", temp);
	if (strncmp (temp, "off", 3)) {
...
{ 
	...
	else if (mode == Continuous || mode == Deferred)
	{
		...
		HC_Show_Time(&StartTime);
 		...
 		m_pUpdateTimerClient = new 	HTClient(rate, 
				 HTCS_PeriodicSkip, 
				UpdateTick, this); 
...
{	
	...
	if(!view->GetSuppressUpdateTick() &&
		view->GetModel()->GetFileLoadComplete())
...
int HBaseView::OnLButtonDown(HEventInfo &hevent) 
{
	if (GetAxisMode() != AxisOff)
	{
		...
		HC_Close_Segment();
		if (new_pos.x>=0 && new_pos.x <=m_AxisWindowSize && new_pos.y>=m_YSize - m_AxisWindowSize && new_pos.y <=m_YSize)
		{
			m_bLButtonDown = true;	 
	...
	if (m_bLButtonDown && !m_bMouseMove)
	{
		...
		HC_Open_Segment_By_Key(GetAxisTriadKey());
			...
			{ 		
				...
				bool selectionmode = GetVisibilitySelectionMode();
				SetVisibilitySelectionMode(false);
				HViewSelectionLevel lvl = GetViewSelectionLevel();
				SetViewSelectionLevel(HSelectionLevelEntity);
				int res = HC_Compute_Selection(GetDriverPath(), ".", "v", new_pos.x, new_pos.y);
				SetViewSelectionLevel(lvl);
				HC_Set_Heuristics("related selection limit = 5");
				HC_Set_Selectability("everything = off");
				HPoint fa,ra,ta;
				GetViewAxis(&fa, &ta);
				...
				if (res)
				{
					...
					SetVisibilitySelectionMode(selectionmode);
...
{
	HPoint new_target, selection;

	bool const selected = HC_Compute_Selection ("..", ".", "v", mouse_win.x, mouse_win.y) != 0;	
...
void HBaseView::ComputeNewField(float & width, float & height, HPoint const & new_tar,
								 HCamera const & old_camera)
{
	HVector old_ptd = old_camera.target - old_camera.position;
	HVector new_ptd = new_tar - old_camera.position;

	float old_len = HC_Compute_Vector_Length(&old_ptd);
	float new_len = HC_Compute_Vector_Length(&new_ptd);
...
static int isinf_f (float x) { return isnan(x - x); }
static int isinf_d (double x) { return isnan(x - x); }
static int isinf_ld (long double x) { return isnan(x - x); }
#endif
#endif
...
{
    float zDelta = ((float)event.GetMouseWheelDelta()) / GetMouseWheelSensitivity() / 120.0 / 8.0;

	/* Save this setting and restore it later, because visual selection can really hurt zoom performance. */
	bool vis = GetVisibilitySelectionMode();
	SetVisibilitySelectionMode(false);
	...
	HC_Open_Segment_By_Key (GetSceneKey());{
		...
		if(event.Shift()){
			...
			if(!event.Control()) {
				...
				adjusted.field_width *= (1 + zDelta);
				adjusted.field_height *= (1 + zDelta);
			}
			else {
		...
		/* This allows us to zoom out even if we're already beyond the limit (which can happen). */
		if (zDelta > 0)
			maxZoomLevel = 0;
		else
			maxZoomLevel = GetZoomLimit();
		...
		if(valid_point(adjusted.position) &&
			...
			if(MIN(adjusted.field_width, adjusted.field_height) > maxZoomLevel
				&& adjusted.position != adjusted.target){
				PrepareForCameraChange();{
	...
	SetVisibilitySelectionMode(vis);
...
void HBaseView::SetTransparency(char *text, bool fast_z_sort)
{
...
int HBaseView::GetCullingThreshold()
{
	char text[MVO_BUFFER_SIZE];
	HC_Open_Segment_By_Key(GetSceneKey());
	HC_Show_One_Net_Heuristic("culling = maximum extent", text);
	HC_Close_Segment();
	return atoi(text);
}
...
int HBaseView::GetHardExtent()
{
	char text[MVO_BUFFER_SIZE];
	HC_Open_Segment_By_Key(GetSceneKey());
	HC_Show_One_Net_Heuristic("culling = hard extent", text);
	HC_Close_Segment();
	return atoi(text);
}
...
HViewSelectionLevel HBaseView::GetViewSelectionLevel()
{
	...
	HC_Open_Segment_By_Key(m_ViewKey);
		HC_Show_One_Net_Heuristic("selection level", heur);
	HC_Close_Segment();
...
{
	if(enable)
		SetFramerateMode(FramerateTarget, target, threshold, false, steps);
	else if (m_FramerateMode==FramerateTarget)
		SetFramerateMode(FramerateOff, target, threshold, false, steps);
}
...
{		
	...
	switch (mode)
	{
		...
		case FramerateTarget:
		{
			...
			SimpList[1] = new HFrExtent(0, steps);
		
			((HFrExtent *)(SimpList[1]))->SetMultiplier(cullingthreshold / (steps>0 ? steps : 1));
			if (enableLods)
				((HFrExtent *)(SimpList[1]))->SetUseLod(true);
...
{
	if (GetIncludeLinkKey() == INVALID_KEY)
		return false;
	else 
		return true;
}
...
void HBaseView::UpdateFakeHLRColor(HPoint paper_color)
{
	...
	HCLOCALE(sprintf(color, "faces=windows=window contrast=(r=%f g=%f b=%f)", 
...
//static H_FORMAT_TEXT variable_marker_size("invalid");
...
void HBaseView::SetSplatSize(int splat_size)
{
	...
	if (m_SplatSize == 1) {
		...
		if (!m_bSceneBoundingSet) {
			HPoint min, max;
			GetSceneExtents (min, max);
		}

		float length = fabs(MAX(m_SceneBoundingMax.x-m_SceneBoundingMin.x,m_SceneBoundingMax.y-m_SceneBoundingMin.y));

		float splat_size = pow(1.1, (double)m_SplatSize-100.0);

		float adjusted_size = splat_size * length;

		HC_Open_Segment_By_Key(GetSceneKey()); {
			HC_Set_Variable_Marker_Size(H_FORMAT_TEXT("%f wsu", adjusted_size));
		} HC_Close_Segment();
	}
	Update();
}
...
{
	HBaseView * view = (HBaseView *)view_pointer;
...
void HBaseView::VisualizeBounding(bool draw_something, HC_KEY key, int incl_count, HC_KEY * incl_path)
{
	...
	HC_Open_Segment_By_Key(GetOverwriteKey());
		bounding_seg=HC_KOpen_Segment("bounding");
			HC_Flush_Contents(".", "everything");
		HC_Close_Segment();
	HC_Close_Segment();
	...
	if(draw_something && key!=INVALID_KEY)
	{
		...
		if(bounding_info[0]!='\0')
		{
			...
			else if(strstr(bounding_info, "cuboid"))
			{
				...
				int flist [] = {4,0,1,2,3, 4,1,5,6,2, 4,2,6,7,3, 4,3,7,4,0, 4,0,4,5,1, 4,7,6,5,4};
...
void HBaseView::FocusOnSelection(bool onoff)
{
	...
	HCLOCALE(sprintf(dopt, "depth of field = (on, strength = 0.5, near = %lf, far = %lf)", near, far));
	HC_Open_Segment_By_Key(GetViewKey());
		HC_Set_Driver_Options(dopt);
	HC_Close_Segment();
...
void HBaseView::SetupLights()
{
	...
	HC_Open_Segment_By_Key(m_LightsKey);
		...
		for(int i=0; i<m_LightCount; ++i)
		{
			m_SceneLight[i] = HC_KInsert_Distant_Light(light_loc[i].x, light_loc[i].y, light_loc[i].z);
			if(m_bLightFollowsCamera)
