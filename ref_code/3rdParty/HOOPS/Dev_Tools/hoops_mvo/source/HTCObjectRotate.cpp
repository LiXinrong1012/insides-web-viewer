...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HTCObjectRotate.cpp,v 1.20 2008-08-07 00:14:46 jason Exp $
//
...
bool HTCObjectRotate::GetSelection( HEventInfo &event, HC_KEY *key ) 
{
	...
	HC_Open_Segment_By_Key(GetView()->GetViewKey());
		...
		if (HC_Compute_Selection(".","./scene/overwrite","v", new_pos.x, new_pos.y) > 0)
...
{
	return ((HTCObjectRotate*)user_data)->OnSignal(signal);
}
