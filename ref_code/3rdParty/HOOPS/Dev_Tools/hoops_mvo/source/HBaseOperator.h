...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HBaseOperator.h,v 1.75 2008-09-19 18:44:55 reuben Exp $
//
...
public:
	...
	*/
	...
	HPoint const GetFirstPoint( void ) { return m_ptFirst; }         //!< Returns the initial selection point

	void SetNewPoint( HPoint const& new_point ) { m_ptNew = new_point; }     //!< Sets the current selection point
	HPoint const GetNewPoint( void ) { return m_ptNew; }             //!< Returns the current selection point

	HBaseView * GetView( void ) { return m_pView; }                    //!< Returns a pointer to the view that this operator is associated with
	void SetView(HBaseView* new_view) { m_pView = new_view; }         //!< Sets the view that this operator is associated with

	virtual bool const OperatorStarted( void ) { return m_bOpStarted; }          //!< Operator has been started if true, and has not been started if false
