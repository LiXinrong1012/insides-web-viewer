...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HErrorManager.cpp,v 1.8 2010-10-27 23:07:56 boni Exp $
//
...
HErrorNode::HErrorNode(int category, int specific, int severity, int msgc, char **msgv, int stackc, char ** stackv, double t)
{
	int i;
	m_category = category;
	m_specific = specific;
	...
	m_time = t;
	m_message = new char *[m_messagelength];
	for (i=0;i<m_messagelength;i++)
	{
		m_message[i] = new char[strlen(msgv[i])+1];	
		strcpy(m_message[i], msgv[i]);
	}	
	
	m_stacklength = stackc;
	m_stack = new char *[m_stacklength];

	for (i=0;i<m_stacklength;i++)
	{
		m_stack[i] = new char[strlen(stackv[i])+1];	
		strcpy(m_stack[i], stackv[i]);
	}
}
