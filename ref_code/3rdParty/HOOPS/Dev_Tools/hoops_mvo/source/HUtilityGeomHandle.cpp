...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HUtilityGeomHandle.cpp,v 1.97 2010-08-16 17:01:02 jason Exp $
//
...
int LineLineIntersect(
   HPoint p1,HPoint p2,HPoint p3,HPoint p4,HPoint *pa,HPoint *pb,
...
//	HC_Compute_Transform_By_Path(kcount, keys, "world","object", matrix);
...
void HUtilityGeomHandle::ShowBasePosition(HC_KEY handlekey, HPoint &bp)
{
	...
	HC_Find_Contents(type, &markerkey);
...
void HUtilityGeomHandle::Move(HC_KEY handlekey, HPoint newposition, HC_KEY camerasegment, HBaseView *view)
{
    HPoint tvalue;
   	float matrix[16];		
	float imatrix[16];
	GetConstraintParameters(handlekey);
	HPoint xxxpos = newposition;	
	HPoint xyz,xyz2;
	HPoint bposition;	
	float mainmatrix[16];
	float mainmatrixinverse[16];

	HC_KEY keys[MVO_BUFFER_SIZE];
	FindTranslation(handlekey, tvalue);
	int kcount = HUtility::CalculateKeyPath(handlekey, view, keys, MVO_BUFFER_SIZE);
	HC_Compute_Transform_By_Path(kcount, keys, "world","object", mainmatrix);
	HC_Compute_Transform_By_Path(kcount, keys, "object","world", mainmatrixinverse);
	...
	case HConstrainToRay:
		...
		HC_Compute_Transformed_Points(1,&newposition, mainmatrix, &newposition);		
		AdjustPositionToRay(handlekey, newposition, mr1, mr2);		 
		ShowBasePosition(handlekey, bposition);
		HPoint npold = newposition;
		newposition.Set(newposition.x - bposition.x, newposition.y - bposition.y,
		    newposition.z - bposition.z);
		HC_Compute_Transformed_Points(1,&newposition, mainmatrixinverse, &xyz); 			
	...
	case HConstrainToPlane:
		...
		HC_Compute_Transformed_Plane(plane,mainmatrixinverse, newplane);
 		
		AdjustPositionToPlane(handlekey, newposition, newplane[0], newplane[1], newplane[2], newplane[3], camerasegment);			
		HC_Compute_Transformed_Points(1,&newposition, mainmatrix, &newposition);
	...
	case HConstrainToViewplane:
		...
		HC_Open_Segment_By_Key(handlekey);
		HC_Compute_Transformed_Points(1,&bposition, mainmatrixinverse, &bposition);
		HC_Close_Segment();
		
		HC_Open_Segment_By_Key(camerasegment);
		HC_Compute_Coordinates(".", "world", &bposition, "local window", &bposition);
		HC_Compute_Coordinates(".", "world", &newposition, "local window", &newposition);
		newposition.z = bposition.z;
		HC_Compute_Coordinates(".", "local window", &newposition, "world", &newposition);
		HC_Close_Segment();
	...
	HC_KEY ownerkey = GetMMSegment(handlekey);
 	HC_Show_Segment(ownerkey, segment);
 	HC_Open_Segment_By_Key(ownerkey);
	HC_Show_Net_Modelling_Matrix(matrix);
	HC_Compute_Matrix_Inverse(matrix, imatrix);

        HC_Compute_Transformed_Points(1,&xyz, imatrix, &newposition);
	...
	float tm[16], matrixfinal[16];

	HC_Compute_Translation_Matrix(newposition.x - tvalue.x,newposition.y - tvalue.y,newposition.z - tvalue.z, tm);
 
//	HC_Compute_Translation_Matrix(tposition.x-bposition.x,tposition.y-bposition.y,tposition.z-bposition.z, tm);
	
 	HC_Compute_Matrix_Product(tm, matrix, matrixfinal);
	HC_Set_Modelling_Matrix(matrixfinal);
...
HC_KEY HUtilityGeomHandle::SelectHandle(HBaseView *view, HPoint new_pos)
{
	...
	bool oldvismode = view->GetVisibilitySelectionMode();
	view->SetVisibilitySelectionMode(false);

	HC_Open_Segment_By_Key(view->GetViewKey());
		HC_Set_Driver_Options("selection proximity = 0.3");
 		HC_QSet_Heuristics ("./scene/overwrite", "no related selection limit");
		HViewSelectionLevel lvl = view->GetViewSelectionLevel();
		view->SetViewSelectionLevel(HSelectionLevelEntity);
	...
	view->SetDefaultSelectionProximity(view->GetDefaultSelectionProximity());
	view->SetVisibilitySelectionMode(oldvismode);
 	return NoteKey;
}
...
HC_KEY HUtilityGeomHandle::SelectObject(HBaseView *view, HPoint new_pos, HC_KEY oldkey, bool exclude, float proximity)
{
    ...
    int		res;
	HC_KEY    key;
	char dropt[MVO_BUFFER_SIZE];

	if(proximity<0)
		sprintf(dropt, "no selection proximity");
	else
		HCLOCALE(sprintf(dropt, "selection proximity=%f", proximity));
  
 
	HC_KEY ObjectKey = INVALID_KEY;
	HC_Open_Segment_By_Key(view->GetViewKey());
	HC_Set_Driver_Options(dropt);
	HC_Close_Segment();
	bool oldvismode = view->GetVisibilitySelectionMode();
	view->SetVisibilitySelectionMode(false);
 	HC_Open_Segment_By_Key(view->GetViewKey());
		...
		HViewSelectionLevel lvl = view->GetViewSelectionLevel();
		view->SetViewSelectionLevel(HSelectionLevelEntity);
		res = HC_Compute_Selection(view->GetDriverPath(),".","v", new_pos.x, new_pos.y);
		view->SetViewSelectionLevel(lvl);
		HC_QUnSet_Heuristics("./scene/overwrite");
	HC_Close_Segment();
...
void HUtilityGeomHandle::SetupCuttingPlaneManipulator(HC_KEY startkey, HBaseView *view, HPoint *midpoint, bool omit_rotate)
{
	...
	{
			ppp.Set(mid.x,mid.y+0.05,mid.z);
		hkey = ghandle.Insert(ppp, wireframe); 	
		r1.Set(mid.x,mid.y,mid.z);
...
void HUtilityGeomHandle::FindTranslation(HC_KEY handlekey, HPoint &trans)
{
	...
	handlekey = HC_KShow_Owner_By_Key(handlekey);
	HC_Show_Segment(handlekey, text);
	HC_Open_Segment_By_Key(handlekey);
	HC_KEY key = HC_KOpen_Segment("geom");
	if (HUtility::UserOptionExists(key, "gpos",0) != INVALID_KEY)
	{
		HC_Show_One_User_Option("gpos", text);
		HCLOCALE (sscanf(text, "%f %f %f", &trans.x, &trans.y, &trans.z));
	}
	else
		trans.Set(0,0,0);
