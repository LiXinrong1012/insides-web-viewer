...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HBhvSensor.cpp,v 1.17 2008-03-27 21:16:42 jason Exp $
//
...
bool HBhvConditionMouse::IsClicked(HC_KEY target, float x, float y) 
{
	...
	HC_Open_Segment_By_Key(pBehaviorManager->GetActiveView()->GetViewKey());
		...
		HViewSelectionLevel lvl = pBehaviorManager->GetActiveView()->GetViewSelectionLevel();
		pBehaviorManager->GetActiveView()->SetViewSelectionLevel(HSelectionLevelEntity);
		res = HC_Compute_Selection(".","./scene/overwrite","v", x, y);
		pBehaviorManager->GetActiveView()->SetViewSelectionLevel(lvl);
