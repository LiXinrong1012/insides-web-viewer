...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HOpSelectAperture.cpp,v 1.102 2010-10-04 22:07:27 boni Exp $
//
...
{
	return (HBaseOperator *)new HOpSelectAperture(GetView());
}
...
int HOpSelectAperture::OnLButtonDown(HEventInfo &event)
{
	...
	HC_Open_Segment_By_Key(m_pView->GetOverwriteKey());{
		HC_QSet_Heuristics (".", "selection sorting, internal selection limit = 0");
	}HC_Close_Segment();

	HMarkupManager *markupManager;
	if ((markupManager = GetView()->GetMarkupManager()))	
	{
