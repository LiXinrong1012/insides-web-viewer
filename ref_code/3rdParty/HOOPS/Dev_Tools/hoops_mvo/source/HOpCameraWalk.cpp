...
{
	return (HBaseOperator *)new HOpCameraWalk(GetView());
}
...
static HC_KEY ConstructExtrudedBox(HPoint &p1, HPoint &p2, HPoint &u, float width, float height, bool noextrude)
{
	...
	float length = HC_Compute_Vector_Length(&delta);
	if (!noextrude)
...
void HOpCameraWalk::AdjustCameraCollision(HCamera &oldcamera, bool noextrude)
{
  ...
  iterate_closer:
	...
	int selection_count = HC_Compute_Selection_By_Shell("v", "./scene/overwrite", pnum, points,
														fnum, face_list);			
...
void  HOpCameraWalk::SnapToGround()
{
	...
	HC_Open_Segment_By_Key(GetView()->GetViewKey());
		HViewSelectionLevel lvl = GetView()->GetViewSelectionLevel();
		GetView()->SetViewSelectionLevel(HSelectionLevelEntity);
		int selection_count = HC_Compute_Selection_By_Ray("v", "./scene/overwrite", &p, &up_invert);
		GetView()->SetViewSelectionLevel(lvl);
	HC_Close_Segment();
