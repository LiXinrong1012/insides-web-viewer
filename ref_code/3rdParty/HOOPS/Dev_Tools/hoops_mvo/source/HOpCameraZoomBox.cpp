...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HOpCameraZoomBox.cpp,v 1.98 2009-05-27 20:04:05 boni Exp $
//
...
{
	return (HBaseOperator *)new HOpCameraZoomBox(GetView());
}
...
bool HOpCameraZoomBox::ComputeReasonableTarget(HPoint & new_tar, HPoint const & min, HPoint const & max, HPoint const & tar_orig)
{
	...
	bool const selected = HC_Compute_Selection ("..", ".", "v", center.x, center.y) != 0;	
	...
	if(selected){
		...
		/* It's best not to use odd numbers for the steps as we'd just be computing selection
		   on the center of the box again... */
		int const X_STEPS = 8;
		int const Y_STEPS = 8;
		int count = 0;
		HPoint found(0, 0, 0);
		...
		for (int row = 0; row < Y_STEPS; ++row){
			...
			for (int col = 0; col < X_STEPS; ++col){
				...
				if (HC_Compute_Selection ("..", ".", "v", xx, yy)){
					float x, y, z;
		...
		if(count > 0){
			HUtility::AdjustPositionToPlane(GetView(), new_target, found / count);
			anything_selected = true;
		}
		else {
									   ...
									   HPoint const & new_tar)
{
	...
	width = HC_Compute_Vector_Length(&x);
	height = HC_Compute_Vector_Length(&y);
