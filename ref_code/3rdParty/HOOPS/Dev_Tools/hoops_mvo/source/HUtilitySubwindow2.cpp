...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HUtilitySubwindow2.cpp,v 1.6 2008-11-19 21:29:53 jason Exp $
//
...
void HSubwindow2::Insert(float left, float right, float bottom, float top,char *title,float minwidth, float minheight, int type, long appearance)
{
	if (title)
		...
		{	
			if (i>=m_ScrollPos)
			{
				...
				//if (0.9-i2*delta > -1)
				{
					...
					ShowSphere();
					HAbsoluteWindow::SetWindow(m_pView, 0,i2*80,0,80, RelativeLeft, RelativeTop, RelativeRight, RelativeTop);				
			//		HC_Set_Window(-1,0.87,0.9 - i2*delta-delta,0.9-i2*delta);
