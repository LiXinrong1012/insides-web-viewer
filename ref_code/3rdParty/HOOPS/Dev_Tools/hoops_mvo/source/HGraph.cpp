...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HGraph.cpp,v 1.28 2010-08-16 17:00:53 jason Exp $
//
...
int HBaseGraph::AddLegendEntry(const char * str, const char * color, HGraphLegendEntryType ltype)
{
	...
	HC_Open_Segment_By_Key(m_legend.segment);
		m_legend.entries[n]->segment=HC_KOpen_Segment("");
			...
			m_legend.entries[n]->text_key=HC_KInsert_Text(2*fsize,0.,0., str);
			HC_Set_Text_Alignment("<*");
			HC_Set_Color(color_def);
			if(ltype==LegendEntryTypeLine)
				HC_Insert_Line(0., 0., 0., fsize, 0., 0.);
			else
			{
				HPoint mesh_points[4];
				mesh_points[0].Set(0., -fsize/2., 0.);
				mesh_points[1].Set(fsize, -fsize/2., 0.);
				mesh_points[2].Set(0., fsize/2., 0.);
				mesh_points[3].Set(fsize, fsize/2., 0.);
...
int HBaseGraph::AddLegendUnicodeEntry(const unsigned short * str, const char * color, HGraphLegendEntryType ltype)
{
	...
	HC_Open_Segment_By_Key(m_legend.segment);
		m_legend.entries[n]->segment=HC_KOpen_Segment("");
			HC_Show_One_Net_Text_Font("size", text_size);
			HCLOCALE(sscanf(text_size, "%f", &fsize));
			m_legend.entries[n]->text_key=HC_KInsert_Unicode_Text(2*fsize,0.,0., str);
			HC_Set_Text_Alignment("<*");
			HC_Set_Color(color_def);
			if(ltype==LegendEntryTypeLine)
				HC_Insert_Line(0., 0., 0., fsize, 0., 0.);
						   ...
						   HPoint *out_points, HGraphPointFormat out_system)
{
	...
	if(in_system==PointFormatCartesian)
	{
		...
		else if(out_system==PointFormatPolarDegrees)
		{
			...
			{
				tmp.x=pow(in_points[i].x, 2)+pow(in_points[i].y, 2);
				tmp.y=180*atan2(in_points[i].y, in_points[i].x)/_PI;
				tmp.z=in_points[i].z;
	...
	else if(in_system==PointFormatPolarDegrees)
	{
		if(out_system==PointFormatCartesian)
		{
			for(i=0; i<in_count; ++i)
			{
				tmp.x=in_points[i].x*cos(180*in_points[i].y/_PI);
				tmp.y=in_points[i].x*sin(180*in_points[i].y/_PI);
				tmp.z=in_points[i].z;
...
void HBaseGraph::DrawLegend()
{
	...
	d=(m_legend.title!=INVALID_KEY ? ftitle*1.5 : 0.);
	max=m_legend.entries.Count();
	for(i=0; i<max; ++i)
	{
...
void HPlot2D::SetAxisScale(HGraphAxisSelection axis, HGraphAxisScale scale, double factor)
{
	...
	if(axis==X_Axis)
	{
		...
		{
			old_scale=m_x_axis.axis_scale;
			old_factor=m_x_axis.axis_scale_factor;
	...
	else if(axis==Y_Axis) 
	{
		...
		{
			old_scale=m_y_axis.axis_scale;
			old_factor=m_y_axis.axis_scale_factor;
...
void HPlot2D::SetAxisRange(HGraphAxisSelection axis, double min, double max)
{
	...
	DrawGrid();
	DrawFrame();
	SetPlotTitleLocation(HPoint((m_x_axis.max+m_x_axis.min)/2, m_y_axis.max+0.5, m_origin.z));
	points_count=m_points.Count();
	for(i=0; i<points_count; ++i)
		if(m_points[i]!=0) DrawData(i);
}
...
void HPlot2D::DrawGrid()
{
	...
	else if(m_grid_type==GridTypePolar)
	{
		...
		for(i=m_x_axis.grid_frequency; i <= max_dist; i+=m_x_axis.grid_frequency)
		{
			...
			{
				p1=HPoint(m_origin.x+i, m_origin.y, m_origin.z);
				p2=HPoint(m_origin.x, m_origin.y+i, m_origin.z);
				p3=HPoint(m_origin.x-i, m_origin.y, m_origin.z);
			...
			else
			{
				...
				if(x_M==m_x_axis.grid_max && sqrt(pow(m_x_axis.grid_max-m_origin.x, 2)+pow(m_y_axis.grid_max-m_origin.y,2))>i)
				{
					...
					{
						p3.x=x_M;
						theta_2=2*_PI-H_ACOS((x_M-m_origin.x)/i);
						p3.y=m_origin.y+i*sin(theta_2);
						p3.z=0.;
					}

					//compute midpoint of arc
					p2.x=m_origin.x+i*cos((theta_1+theta_2)/2);
					p2.y=m_origin.y+i*sin((theta_1+theta_2)/2);
					p2.z=0.;
				...
				if(y_M==m_y_axis.grid_max && sqrt(pow(m_y_axis.grid_max-m_origin.y, 2)+pow(m_x_axis.grid_min-m_origin.x, 2))>i)
				{
					//get the first endpoint
					p1.y=y_M;
					theta_1=_PI-H_ASIN((y_M-m_origin.y)/i);
					p1.x=m_origin.x+i*cos(theta_1);
					p1.z=0.;
					...
					{
						p3.y=y_M;
						theta_2=2*_PI+H_ASIN((y_M-m_origin.y)/i);
						p3.x=m_origin.x+i*cos(theta_2);
						p3.z=0.;
					}
				...
				if(x_m==m_x_axis.grid_min && sqrt(pow(m_x_axis.grid_min-m_origin.x, 2)+pow(m_y_axis.grid_min-m_origin.y, 2))>i)
				{
					//get the first endpoint
					p1.x=x_m;
					theta_1=2*_PI-H_ACOS((x_m-m_origin.x)/i);
					p1.y=m_origin.y+i*sin(theta_1);
					p1.z=0.;
				...
				if(y_m==m_y_axis.grid_min && sqrt(pow(m_y_axis.grid_min-m_origin.y, 2)+pow(m_x_axis.grid_max-m_origin.x, 2))>i)
				{
					//get the first endpoint
					p1.y=y_m;
					theta_1=H_ASIN((y_m-m_origin.y)/i);
					p1.x=m_origin.x+i*cos(theta_1);
					p1.z=0.;
		...
		for(n=1; (theta_1=n*m_y_axis.grid_frequency)<2*_PI; ++n)
		{
			if(theta_1<_PI/2) //quadrant I
			{
				...
				{
					p2.y=m_y_axis.grid_max;
					p2.x=m_origin.x+((p2.y-m_origin.y)/sin(theta_1))*cos(theta_1);
					p2.z=0.;
				}
			}
...
{
	int count=m_points[data_set]->data_size, i, color_count;
	...
	{
		for(i=0; i<m_points[data_set]->data_size; ++i)
		{

			bar[0].x=data_points[i].x-m_points[data_set]->bar_width/2;
			bar[0].y=m_origin.y;
			bar[0].z=m_origin.z;

			bar[1].x=data_points[i].x+m_points[data_set]->bar_width/2;
			bar[1].y=m_origin.y;
			bar[1].z=m_origin.z;

			bar[2].x=data_points[i].x-m_points[data_set]->bar_width/2;
			bar[2].y=data_points[i].y;
			bar[2].z=m_origin.z;

			bar[3].x=data_points[i].x+m_points[data_set]->bar_width/2;
...
void HPieChart::SetPieRadius(double radius)
{
	...
	DrawPie();
	DrawFrame();
	SetPlotTitleLocation(HPoint(m_origin.x, m_origin.y+m_scale*1.35, m_origin.z));
}
...
void HPieChart::DrawFrame()
{
	...
	HC_Open_Segment_By_Key(m_plot_segment);
		HC_Open_Segment("Frame");
			...
			if(m_show_frame)
			{
				p1=HPoint(m_origin.x+m_scale, m_origin.y, m_origin.z);
				p2=HPoint(m_origin.x, m_origin.y+m_scale, m_origin.z);
				p3=HPoint(m_origin.x-m_scale, m_origin.y, m_origin.z);
...
void HPieChart::DrawPie()
{
	...
	for(i=0; i<max; ++i)
	{
		if(m_pie[i]==0) continue;

		theta_1=theta_2;
		theta_2=theta_1+2*_PI*m_pie[i]->amount/m_pie_total;

		p1.x=m_origin.x+m_scale*cos(theta_1);
		p1.y=m_origin.y+m_scale*sin(theta_1);
		p1.z=m_origin.z;
		p2.x=m_origin.x+m_scale*cos((theta_1+theta_2)/2);
		p2.y=m_origin.y+m_scale*sin((theta_1+theta_2)/2);
		p2.z=m_origin.z;
		p3.x=m_origin.x+m_scale*cos(theta_2);
		p3.y=m_origin.y+m_scale*sin(theta_2);
		p3.z=m_origin.z;
