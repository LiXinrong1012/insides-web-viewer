...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HImCuttingGeometry.cpp,v 1.44 2010-02-23 01:07:59 reuben Exp $
//
...
static bool find_mate (struct vlist_s *line_list, struct vlist_s *bucket_list, float tolerance )
{
	...
	switch( match_mode ) {
		case START_START:
			while((line = (line_t *)vlist_remove_first(line_list))) {
				reverse_line( line );
...
{		
	...
	while((line = (line_t *)vlist_remove_first(m_lines))) {
	...
	while((line_list = (struct vlist_s *)vlist_remove_first(bucket_list))) {
		if( find_mate( line_list, bucket_list, tolerance ) )
			continue;
		vlist_add_first( bucket_list2, line_list );
	}

	while((line_list = (struct vlist_s *)vlist_remove_first(bucket_list2))) {
	...
	if( m_cutting_type == HIM_CUTTING_GEOMETRY_TYPE_POLYLINES ) {
		...
		while( polyline = (polyline_t *) vlist_remove_first( intermediate_polylines ) ) {
		...
		while( polyline = (polyline_t *) vlist_peek_cursor( intermediate_polylines ) ) {
	...
	face->point_count = point_out - face->points;
	face->face_list_length = face_out - face->face_list;
...
void HImCuttingGeometry::CreateCuttingGeometry(HC_KEY source_key, HC_KEY target_key, int cutting_type, int body_level)
{
	...
	HC_Open_Segment_By_Key(target_key);
		...
		while((polyline = (polyline_t *)vlist_remove_first(m_polylines))) {
			HC_Insert_Polyline(polyline->n_pts, polyline->pts);
			free(polyline->pts);
			free(polyline);
		}
		while((face = (face_t *)vlist_remove_first(m_faces))) {
