...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HObjects.cpp,v 1.48 2009-06-13 04:51:41 jason Exp $
//
...
//    HC_Translate_Object(pos.x, pos.y, pos.z);
    ...
    HC_Set_Visibility("lights = off, edges = off, markers = off"); 
    HC_Open_Segment("dummybox");
...
{    
    ...
    if (!event.LButton() && !event.RButton() && !event.MButton() && event.Shift()  && event.Control())
    {
		...
		HC_Open_Segment_By_Key(m_pView->GetOverwriteKey());
			HC_Set_Heuristics("related selection limit = 1");
		HC_Close_Segment();
		...
		HC_Open_Segment_By_Key(m_pView->GetViewKey());
			...
			count = HC_Compute_Selection(".", "./scene/overwrite", "v", new_pos.x, new_pos.y);
...
{
	int res = false;
 	if ((res = vlist_remove(m_ImagePanelList, ip)) && doDelete)
		delete ip;
...
int HImagePanel::OnLButtonDown(HEventInfo &event)
{
	...
	HViewSelectionLevel lvl = m_pView->GetViewSelectionLevel();
	m_pView->SetViewSelectionLevel(HSelectionLevelEntity);
	int res = HC_Compute_Selection(".",path,"v", m_ptLast.x,  m_ptLast.y);
	m_pView->SetViewSelectionLevel(lvl);
	if (res)
	{
		HC_KEY key;
