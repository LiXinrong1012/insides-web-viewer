...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/hoops_mvo/source/HTools.h,v 1.103 2010-03-30 22:07:12 boni Exp $
//
...
#if defined (DEBUG_BUILD) && !defined (_WIN32_WCE)
#include <assert.h>
...
#if defined(_AIX) || defined(__hpux) || defined(__sgi) || defined(__sun)
#   define MVO_BIGENDIAN 1
#else
...
// Advanced selection using conditional styles
#define MVO_HIGHLIGHT_SEGMENT_BASENAME		"mvo_temp_highlight_segment"
#define MAKE_HIGHLIGHT_SEGMENT_NAME(char_buf, key)		\
			sprintf(char_buf,"%s_%p", MVO_HIGHLIGHT_SEGMENT_BASENAME, (void*)key );
#define MAKE_COMMON_HIGHLIGHT_SEGMENT_NAME(char_buf)		\
			sprintf(char_buf,"%s_common", MVO_HIGHLIGHT_SEGMENT_BASENAME);
