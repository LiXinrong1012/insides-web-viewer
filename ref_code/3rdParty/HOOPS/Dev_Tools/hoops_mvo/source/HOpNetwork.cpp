					...
					float x2, float y2, float z2)
{
	...
	HC_Open_Segment(buffer);{
		...
		length_text_key = My_Renumber_Key(length_text_key);
	}HC_Close_Segment();
...
int HOpNetwork::OnLButtonDown(HEventInfo &hevent)
{
	...
	int count = HC_Compute_Selection(view_segment, 0, "v", window.x, window.y);
			...
			if(count > 0){
				...
				if(other_node_key != INVALID_KEY){
					...
					My_Open_Node(other_node_key);{
						...
						ENDIAN(keys.line_key);
						ENDIAN(keys.node_key);
						HC_Set_User_Data(connection_count, &keys, sizeof(keys));
						++connection_count;
						ENDIAN(connection_count);
