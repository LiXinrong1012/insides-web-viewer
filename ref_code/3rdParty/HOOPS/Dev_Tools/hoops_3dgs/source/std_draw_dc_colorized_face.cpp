 ...
 * manner of such use.
 *
 * $Id: obf_tmp.txt 1.37 2009-10-23 23:28:32 trask Exp $
 */
...
local void triangle_action (
	...
	_hoops_AIICS const *			_hoops_CRHCP = (_hoops_AIICS const *)_hoops_CAIGA;
	DC_Point					points[3];
	...
	_hoops_GAPCP (_hoops_CRHCP->_hoops_APHGA, 3, points, _hoops_CRHCP->color, true);
}
...
GLOBAL_FUNCTION void HD_Std_DC_Colorized_Face (
	...
	for (index = 0; index < count; ++index) {
		nice[index].x = _hoops_GHPCP[index].x;
		nice[index].y = _hoops_GHPCP[index].y;
		nice[index].z = _hoops_GHPCP[index].z * scale;
	}
	...
	_hoops_CRHCP._hoops_PIICS = _hoops_GHPCP;
	_hoops_CRHCP._hoops_APHGA = nr;
	_hoops_CRHCP.color = color;
