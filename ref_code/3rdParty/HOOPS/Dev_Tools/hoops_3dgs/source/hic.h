 ...
 * manner of such use.
 *
 * $Id: hic.h,v 1.455 2011-01-13 22:30:14 boni Exp $
 */
 ...
 */
HC_EXTERNAL HIC_API void HIC_Draw_DC_Phong_Polyline(HIC_Rendition const *nr, int count, HIC_DC_Point const *points, HIC_RGBA const *colors, HIC_Plane const *planes);			
 ...
 */
HC_EXTERNAL HIC_API void HIC_Draw_DC_Textured_Polyline(HIC_Rendition const *nr, int count, HIC_DC_Point const *points, HIC_RGBA const *colors, HIC_Plane const *planes, float const *params, int param_width, HIC_Integer32 param_flags);			
 ...
 * \param rowbytes An int.
 * \param blk A const void pointer.
 *
 * This device-independent function draws the entire image stored in the data block and does not provide for Z-buffering tilted images.
 */
...
/*!
  \addtogroup Driver_XXX_Shader_XXXX
  @{
*/


/*!
 Compiles the supplied shader source, and returns a handle to the resulting shader. The type indicates whether 
 it is avertex or pixel shader, and shader_source points to a character buffer containing the shader's source 
 code.
 
 Notes:
 <ul>
	<li>for DX9, the vertex shader's main function must be called "vs_main", and the pixel shader's main function "ps_main".</li>
	<li>for OGL2, the main function of each shader should be just "main".</li>
 </ul>

 * \param nr A pointer to a const HIC_Rendition.
 * \param type A HIC_Shader_Type enum.
 * \param shader_source A pointer to a const char containing the shader source code.
 * \return A HC_POINTER_SIZED_INT, a shader handle.
 ...
 * \param nr A pointer to a const HIC_Rendition.
 * \param shader_handle A HC_POINTER_SIZED_INT, a shader handle.
 *
 * The behavior of this function is driver and platform specific. 
 */
 ...
 * \param type A HIC_Shader_Type enum.
 * \param shader_handle A HC_POINTER_SIZED_INT, a shader handle.
 * \return A bool, true if the shader was newly set.
 *
 */
 ...
 * \param nr A pointer to a const HIC_Rendition.
 * \param type A HIC_Shader_Type enum.
 * \param name A char const pointer to a shader uniform name.
 * \param count An int indicating the number of ints to set.
 * \param data An int pointer to the data to set on the shader.
 *
 */
 ...
 * \param type A HIC_Shader_Type enum.
 * \param name A char const pointer to a shader uniform name.
 * \param count An int indicating the number of floats to set.
 * \param data A float pointer to the data to set on the shader.
 *
 * The behavior of this function is driver and platform specific. 
 */
 ...
 * \param type A HIC_Shader_Type enum.
 * \param name A char const pointer to a shader uniform name.
 * \param width An int specifying the width of the vector.
 * \param count An int indicating the number of vectors to set.
 * \param data A float pointer to the data to set on the shader.
 *
 */
 ...
 * \param type A HIC_Shader_Type enum.
 * \param name A char const pointer to a shader uniform name.
 * \param count An int indicating the number of matrices to set.
 * \param data A float pointer to the data to set on the shader.
 *
 */
...
* \param nr A pointer to a const HIC_Rendition.
 * \param type A pointer to a float array to store the return matrix.
 *
 */
