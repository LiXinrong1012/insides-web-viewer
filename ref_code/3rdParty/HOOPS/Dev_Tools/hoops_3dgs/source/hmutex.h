 ...
 * manner of such use.
 *
 * $Id: hmutex.h,v 1.80 2010-07-29 23:49:19 jason Exp $
 */
...
#	endif
#else 
#	define HMUTEX_POINTER_SIZED_INT POINTER_SIZED_INT
#endif
...
#	include <time.h>
#	define MILLI_SECOND_SLEEP(_x_) Sleep(_x_)
#else
...
#if defined(H_MUTEX_DEBUG_TRACE) || defined(H_RW_LOCK_DEBUG_TRACE)
#  include <stdio.h>
...
#	define	HM_WAIT_SEMAPHORE(_s_)					WaitForSingleObject (_s_, 0xFFFFFFFF)
...
#	ifdef USE_INTERLOCK_MUTEX
#	include <intrin.h>

	typedef struct {
		volatile long	use_count;
		HThreadID		thread;
	}   Interlock_Mutex;
...
#	define		CREATE_MUTEX(_m_)													\
					do {															\
						Interlock_Mutex *	_hm_;									\
						ALLOCATE_MEMORY (_hm_, Interlock_Mutex);					\
						memset (_hm_, 0, sizeof (Interlock_Mutex));					\
						_m_ = (void *)_hm_;											\
						_H_MUTEX_DEBUG_TRACE("CREATE_MUTEX:",THREAD_ID(),_m_);		\
					} while (0)
#	define		DESTROY_MUTEX(_m_)													\
					do {															\
						Interlock_Mutex *	_hm_ = (Interlock_Mutex *)_m_;	\
						_H_MUTEX_DEBUG_TRACE("DESTROY_MUTEX:",THREAD_ID(),_m_);		\
						FREE_MEMORY (_hm_, Interlock_Mutex);						\
						_m_ = 0;													\
					} while (0)
#	define		LOCK_MUTEX(_m_)														\
					do {															\
						...
						if (_hm_->thread != _me_) {									\
							...
							while (_InterlockedCompareExchange (&_hm_->use_count,	\
																1, 0) != 0) {}		\
							_hm_->thread = _me_;									\
...
#	define		UNLOCK_MUTEX(_m_)													\
					do {															\
						...
						else if (_hm_->use_count > 1)								\
							--_hm_->use_count;										\
						else {														\
							_hm_->thread = 0;										\
							_InterlockedExchange(&_hm_->use_count, 0);				\
...
#	define		CREATE_MUTEX(_m_)										\
					do {												\
						HM_Mutex *		_hm_;							\
						ALLOCATE_MEMORY (_hm_, HM_Mutex);				\
						memset (_hm_, 0, sizeof (HM_Mutex));			\
...
#	define		DESTROY_MUTEX(_m_)										\
					do {												\
						HM_Mutex *		_hm_ = (HM_Mutex *)_m_;	\
						_H_MUTEX_DEBUG_TRACE("DESTROY_MUTEX:",THREAD_ID(),_m_);\
						HM_DESTRUCT_SEMAPHORE (_hm_->control);			\
						FREE_MEMORY (_hm_, HM_Mutex);					\
						_m_ = 0;										\
					} while (0)
#	define		LOCK_MUTEX(_m_)											\
					do {												\
...
#define		READ_LOCK(_l_)					HM_Read_Lock ((HM_Lock *)_l_,					__FILE__, __LINE__)
...
HM_DECLARE_FUNCTION void	HM_CDECL	HM_Read_Lock (HM_Lock * hl, char const * f, int l);
...
#	define	EXPAND_PER_THREAD_DATA(hl)	do {										\
	...
	HM_DEFINE_FUNCTION	void HM_CDECL HM_Read_Lock (
		HM_Lock *		hl,
		char const *	f,
		int				l) {
		...
		_H_RW_LOCK_DEBUG_TRACE("HM_Read_Lock: grab control", me, hl, 999, f,l);
		...
		if (ii == hl->active_threads) {	/* new thread */
			...
			hl->threads[ii].id = me;
			hl->threads[ii].use_count = use_count = 1;
