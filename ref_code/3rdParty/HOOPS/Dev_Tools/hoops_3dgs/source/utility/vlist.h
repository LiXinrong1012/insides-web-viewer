...
* manner of such use.
*
* $Id: vlist.h,v 1.68 2010-12-06 22:06:36 jason Exp $
*/
...
#define START_LIST_ITERATION(TYPE, _vl_) VLIST_RESET_CURSOR(_vl_, &((_vl_)->cursor));\
				while (1) {\
				TYPE *temp;\
				if (!(temp = (TYPE *)VLIST_PEEK_CURSOR(&((_vl_)->cursor))))\
					break;

#define END_LIST_ITERATION(_vl_)  VLIST_ADVANCE_CURSOR(&((_vl_)->cursor)); }
