...
GLOBAL_FUNCTION void HI_Reset_Driver_Config_SS (void) {
	...
	HC_Define_Driver_Config("opengl", "*", "*", "");

	/* _hoops_HPCPI _hoops_RHPP */
	HC_Define_Driver_Config("mgl", "*", "*", "");

	HC_Define_Driver_Config("opengl", "Mesa X11", "*", "software, config=(ALLOW_DRAWPIXEL_RESTORE)");
	...
	HC_Define_Driver_Config("opengl", "Quadro FX*", "*", "config=("
		"UNDRAW_OVERLAY_FRONT_AND_BACK, DOUBLE_BUFFER_COPY_ONLY, BAD_FLAT_VERTEX_ARRAYS),"
		"tristrip minimum=5,"
		"display list tristrip minimum=24,"
		"flat tristrip minimum=0,"
		"flat display list tristrip minimum=5");
	...
	HC_Define_Driver_Config("opengl", "Radeon X1300 / X1550 Series", "*", "config=("
							"BAD_KTX)");
...
GLOBAL_FUNCTION void HI_Start_System_Specific (_hoops_AIGPR *_hoops_RIGC) {
	...
	HOOPS_WORLD->_hoops_IARPR = HD_Linux_Load_Dynamic_Driver;
#endif
...
GLOBAL_FUNCTION Driver_Action HD_Linux_Load_Dynamic_Driver (
...
#ifdef _hoops_IPCPI
	printf("%s %s...",_hoops_SARPR, _hoops_GPRPR);
#endif
	...
	if (!_hoops_APRPR){
#ifdef _hoops_IPCPI
		printf("sym linking %s faild: %s\n", _hoops_GPRPR, dlerror());
#endif
...
GLOBAL_FUNCTION void HD_Linux_UnLoad_Dynamic_Driver (
...
GLOBAL_FUNCTION void HI_Sleep(_hoops_GSPGR _hoops_SPAPR) {
	...
	req.tv_nsec = ((_hoops_SPAPR % _hoops_HSPGR) * 1000000000L) / _hoops_HSPGR;
...
local void _hoops_AHCPI(int _hoops_IHHPI)
{
	_hoops_PIAPR (true);
}
...
GLOBAL_FUNCTION bool HI_Enable_Timer_Interrupts(void) {
	...
	_hoops_GHAPR._hoops_HHAPR.tv_usec = ((_hoops_HIAPR % _hoops_HSPGR) * 1000000L) / _hoops_HSPGR;
	...
	ZERO_STRUCT(&_hoops_GIHI, struct _hoops_AHHPI);
	_hoops_GIHI._hoops_CIHPI = _hoops_AHCPI;
