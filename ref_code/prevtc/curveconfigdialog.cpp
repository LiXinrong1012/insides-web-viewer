...
{
	//保存的是用户看到的名
	QString saveName = QInputDialog::getText(tr("New Appearance"),tr("Enter appearance name:"));
	if(saveName.isEmpty())
		return;
...
void CurveConfigDialog::renameConfig()
{
	...
	QString newName = QInputDialog::getText("","Input New Name :");
...
{
	QString saveName = QFileDialog::getSaveFileName(qApp->applicationDirPath(),"*.dat");
	if(saveName.isEmpty())
		return;
...
{
	QString openName = QFileDialog::getOpenFileName(qApp->applicationDirPath(),"*.dat");
	if(openName.isEmpty())
		return;
