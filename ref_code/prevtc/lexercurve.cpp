...
// The ctor.
LexerCurve::LexerCurve(QObject *parent, bool caseInsensitiveKeywords)
	:QsciLexerCPP(parent,caseInsensitiveKeywords)
{
}

LexerCurve::~LexerCurve()
{
}

// Return the set of character sequences that can separate auto-completion
// words.
QStringList LexerCurve::autoCompletionWordSeparators() const
{
	QStringList wl;
	wl << ".";
	return wl;
...
const char *LexerCurve::blockStartKeyword(int *style) const
{
	if (style)
		*style = Keyword;
...
const char *LexerCurve::wordCharacters() const
{
	return "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_#";
}

// Returns the foreground colour of the text for a style.
QColor LexerCurve::defaultColor(int style) const
{
	switch (style)
	{
	case Default:
...
bool LexerCurve::defaultEolFill(int style) const
{
	switch (style)
	{
	case UnclosedString:
...
QFont LexerCurve::defaultFont(int style) const
{
	QFont f;
...
const char *LexerCurve::keywords(int set) const
{
...
QColor LexerCurve::defaultPaper(int style) const
{
	switch (style)
	{
	case UnclosedString:
