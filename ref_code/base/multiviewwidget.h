...
/*!
*	From paraview, the main widget that support multiple views.
*	@author Jerry He
*	@date	2014.3.26
*/
class BASE_EXPORT MultiViewWidget : public QWidget
{
	Q_OBJECT
		typedef QWidget Superclass;
public:
	MultiViewWidget(QWidget * parent=0, Qt::WindowFlags f=0);
	virtual ~MultiViewWidget();

	/// Get/Set the vtkSMViewLayoutProxy instance this widget is using as the layout
	/// manager.
	ViewLayoutManager* layoutManager() const;
	...
	QList<View*> viewProxies() const;

	/// Returns true if the view has been assigned to this layout.
	bool isViewAssigned(View*) const;
...
signals:
	...
	public slots:
		...
		protected slots:
			...
			void makeActive(ViewFrame* frame);
			...
			void markActive(View* view);
			void markActive(ViewFrame* frame);
...
protected:
	...
	virtual ViewFrame* newFrame(View* view);
...
private:
	QWidget* createWidget(int, ViewLayoutManager* layout, QWidget* parentWdg,
		int& maxIndex);

private:
	Q_DISABLE_COPY(MultiViewWidget);
