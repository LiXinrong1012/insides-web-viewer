...
bool HelpViewer::launchWithExternalApp(const QUrl &url)
{
    ...
    if (isLocalUrl(url)) {
        ...
        if (!canOpenPage(path)) {
            ...
            actualTmpFile.write(engine.fileData(resolvedUrl));
            actualTmpFile.close();
...
QString HelpViewer::fixupVirtualFolderForUrl(const HelpEngineWrapper *engine, const QUrl &url, bool *fixed)
{
    ...
    const QString virtualFolder = HelpEngineSingleton::instance()->d->nameSpaceVirtualFolderMap.value(url.host(), QString());
